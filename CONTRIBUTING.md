...
可以通过下面这些方法参与 [Microsoft Graph 文档](http://developer.microsoft.com/graph/docs)：
...
如果提交的是文档和代码示例的新更改或重大更改，需要先签署参与许可协议 (CLA)，然后我们才能接受你的拉取请求（如果你属于以下任意一组的话）：
...
签署参与许可协议 (CLA) 并未授予向主存储库提交内容的权利，但确实表示 Microsoft 团队可以查看并考虑你提交的内容；如果被我们采纳，你将获得相应的奖励。

可以单击[此处](https://cla.microsoft.com)下载或数字签名参与许可协议 (CLA)。 收到和处理你的 CLA 后，我们便会尽力在 10 个工作日内评审你的拉取请求。

## <a name="use-github-git-and-this-repository"></a>使用 GitHub、Git 和此存储库

**注意：** 此部分中的大多数信息都可以在 [GitHub 帮助][]文章中找到。  如果已熟悉 Git 和 GitHub，请跳至“参与和编辑内容”**** 部分，查看此存储库中代码/内容流的详细信息。
...
1. 打开 GitBash。
1. 在提示符处键入 `git pull upstream master:<new branch name>`。 此命令在本地新建分支，该分支是从最新的 *microsoftgraph* 主分支复制而来。 **注意：** 对于内部参与者，将命令中的 `master` 替换为要在特定日期发布的分支。
1. 在提示符处键入 `git push origin <new branch name>`。 此命令将提醒 GitHub 注意这个新分支。 现在，应该可以在 GitHub 存储库的分叉上看到这个新分支。
1. 键入 `git checkout <new branch name>` 切换到新分支。
...
本地计算机上的文件是在本地存储库中创建的新分支的副本，目前正在运行。 提交更改前，在此文件夹的任何更改都不会影响本地存储库。 若要向本地存储库提交更改，在 GitBash 中键入以下命令：
...
`add` 命令将更改添加到暂存区域，以准备将更改提交到存储库。 `add` 命令后面的点号指明要暂存所有已添加或修改的文件，同时以递归方式查看子文件夹。 （如果不希望提交所有更改，可以添加特定的文件）。 还可以撤消提交。 若要获取帮助，请键入 `git add -help` 或 `git status`。）

`commit` 命令将暂存更改应用到存储库。 `-m` 表示命令行中有提交注释。 如果你并不希望在某个特定日期进行发布，可以说“尽快发布”。  -v 和 -a 开关可以省略。 -v 开关用于命令的详细输出，-a 与添加命令的用途相同。
...
1. 在 GitBash 中的命令提示符处键入 `git push origin <new branch name>`。 在本地存储库中，`origin` 是指从中复制本地存储库的 GitHub 存储库。 此命令将新分支的当前状态（包括上述步骤中的所有提交）推送到 GitHub 分支。
...
并对内容进行验证。 `checkout` 命令将磁盘上的文件更新为 X2 分支的当前状态。
签出新分支后，便可以照常更新并提交内容。
但是为了避免误在已合并的分支 (X) 进行操作，最好将其删除（请参阅下列“**删除分支**”部分）。
...
2. 接下来，在命令提示符处键入 `git branch -d <branch name>`。  只有当分支已成功合并到上游存储库中，此命令才会在本地计算机上删除分支。 （可以使用 `–D` 标记重写此行为，但请先确保自己需要这么做。）
...
若要寻找实用编辑器，请尝试 [Visual Studio Code][vscode]。
...
- **编号列表：** 若要创建编号（经过排序的）列表，请在代码行的开头添加 `1. `。 若要在一个列表元素内添加多个元素，请按如下方式设置列表格式：
    ...
    1. 请注意，“.”后面有一个空格。
       ...
       现在，请注意，一个列表元素中的两个段落之间有一个换行符，并且这一行的缩进与上一行的缩进相同。
...
- **内联 HTML：** 使用 Markdown，虽然可以内联 HTML，但应避免这样做。
