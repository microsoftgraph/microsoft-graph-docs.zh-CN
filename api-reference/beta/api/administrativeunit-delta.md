...
如果成功，此方法在响应 `200 OK` 正文中返回 响应代码和 [administrativeUnit](../resources/administrativeunit.md) 集合对象。 该响应还包括 `nextLink`URL 或 `deltaLink`URL。 

- `nextLink`如果返回 URL，则会话中有其他要检索的数据页。 **administrativeUnit** 继续使用 `nextLink` URL 提出请求，直到响应`deltaLink`中包括 URL。

- `deltaLink`如果返回 URL，则不再返回有关资源现有状态的数据。 保留并使用 `deltaLink` URL 了解将来对资源的更改。

有关详细信息和示例，请参阅[使用增量查询和](/graph/delta-query-overview)[获取用户的增量更改](/graph/delta-query-users)。
