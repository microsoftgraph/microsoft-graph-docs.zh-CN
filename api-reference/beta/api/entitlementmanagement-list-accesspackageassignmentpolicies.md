...
此方法支持 `$select` 、 `$filter` 和 `$expand` OData 查询参数来帮助自定义响应。 例如，若要检索具有指定权限的访问包分配显示名称，请包含 `$filter=displayName eq 'Employee sales support'` 到查询中。 若要了解一般信息，请参阅 [OData 查询参数](/graph/query-parameters)。
