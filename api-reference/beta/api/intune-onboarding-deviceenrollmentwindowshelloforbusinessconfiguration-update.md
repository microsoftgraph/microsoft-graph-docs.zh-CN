...
|pinSpecialCharactersUsage|[windowsHelloForBusinessPinUsage](../resources/intune-onboarding-windowshelloforbusinesspinusage.md)|控制在适用于企业 PIN 的 Windows Hello 中使用特殊字符的能力。  允许允许使用特殊字符 (，) 必需可确保它们存在。 如果设置为"不允许"， (不允许) 特殊字符。 可取值为：`allowed`、`required`、`disallowed`。|
