...
description: 获取一个报告，该报告提供 (Microsoft Edge、Microsoft Edge 旧版和Internet Explorer) 用户在所选时段内访问 Microsoft 365 服务时所使用的浏览器。
...
获取一个报告，该报告提供 (Microsoft Edge、Microsoft Edge 旧版和Internet Explorer) 用户在所选时段内访问 Microsoft 365 服务时所使用的浏览器。
...
此方法支持使用 `$format`、`$top` 和 `$skipToken` [OData 查询参数](/graph/query-parameters)自定义响应。 默认输出类型为 `text/csv`。 但是，如果要指定输出类型，可以使用 OData `$format` 查询参数将默认输出 `text/csv` 设置为 或 `application/json`。
