...
可以在邮箱的所有日历或特定日历中的事件或日历 **的日历视图** (事件范围（由日历的开始日期和结束日期) ）的事件集中获取这些增量更改的特定类型的更改。 日历可以是默认日历或用户的其他一些指定日历。 在 **calendarView** 上获取增量更改的情况下，日历也可以是组日历。

**delta** 函数调用类似于指定日历的或请求，只不过通过在这些调用中的一个或多个调用中正确应用状态令牌，可以查询该日历中事件的增量更改。 `GET /events` `GET /calendarview` [](/graph/delta-query-overview#state-tokens) 这允许您维护和同步指定日历中事件的本地存储，而无需每次从服务器提取该日历的所有事件。

下表列出了事件上的 **delta** 函数与日历中 **calendarView** 上的 **delta** 函数之间的差异。

| 事件上的 Delta 函数  | calendarView 上的 Delta 函数  |
|:--------------------------|:---------------------------------------------------------|
| 获取日历中不受开始日期和结束日期范围限制的所有事件的增量更改。 或者，你可以从该日期/时间或之后开始，获取受开始时间限制的日历中事件的增量更改。 | 获取日历视图 的开始日期和结束日期/时间内事件的 **增量更改**。 |
| 出于性能原因，仅 **返回一组** 有限的事件属性。 随后用于扩展 `GET /events/{id}` 任何事件的客户端。 | 服务器端扩展返回一组更 **完整的事件** 属性。 |
| 响应包括单个实例和定期系列母版。 | 响应包括单个实例，以及定期系列的发生和例外。 |
| 适用于用户日历中的事件，但不应用于组日历。 | 适用于用户和组日历中的事件。 |
...
在成功响应或成功响应中返回的查询 URL `nextLink` `deltaLink` 包括状态令牌。 对于任何后续 **delta** 函数调用，请使用查询 URL 中的或 `nextLink` `deltaLink` 前面的查询 URL。

### <a name="delta-function-on-events-in-a-user-calendar-preview"></a>用户日历中事件的 Delta 函数 (预览) 
将 **delta** 函数应用于从特定日期/时间开始或之后的所有事件，在指定的用户日历 () ：

* 若要获取用户邮箱中所有事件或自指定日期/时间之后开始的事件的增量 _更改，请执行以下操作_：
  <!-- { "blockType": "ignored" } -->
  ```http
  GET /me/events/delta 
...
* 若要获取用户默认日历中指定日期/时间或之后开始的所有事件或事件的增量 _更改：_
  <!-- { "blockType": "ignored" } -->
  ```http
...
* 若要获取指定用户日历中指定日期/时间或之后开始的所有事件或 _事件的增量更改_：
  <!-- { "blockType": "ignored" } -->
  ```http
...
* 若要获取指定日历组和日历中指定日期/时间或之后开始的所有事件或事件的增量 _更改_：
  <!-- { "blockType": "ignored" } -->
  ```http
...
* 若要在用户的默认日历的日历视图中获取增量 _更改，请执行以下操作_：
  <!-- { "blockType": "ignored" } -->
  ```http
...
* 若要获取指定用户日历的日历视图中 _的增量更改，请执行以下操作_：
  <!-- { "blockType": "ignored" } -->
  ```http
...
跟踪更改将引发一轮或多次 **delta** 函数调用。 如果要使用任意查询参数（`$deltatoken` 和 `$skiptoken` 除外），则必须在最初的 **delta** 请求中指定它。 Microsoft Graph 自动将指定的任意参数编码为响应中提供的 `nextLink` 或 `deltaLink` URL 的令牌部分。 只需预先指定所需的任何查询参数一次。
在后续请求中，只需复制和应用上一响应中的或 `nextLink` `deltaLink` URL，因为该 URL 已包含编码的所需参数。

| 查询参数      | 类型   |说明|
|:---------------|:--------|:----------|
|startDateTime|String|时间范围的开始日期和时间，以 ISO 8601 格式表示。例如，“2019-11-08T19:00:00-08:00”。<br>时区在参数值的时区偏移部分中指定，如果存在，则不会影响 `Prefer: outlook.timezone` 标头。 如果值中未包含时区偏移量，则将其解释为 UTC。<br>对于 **日历中的事件** 增量，可选。 <br>在 **calendarView** 上 delta **是必需的**。 |
|endDateTime|String|时间范围的结束日期和时间，以 ISO 8601 格式表示。例如，“2019-11-08T20:00:00-08:00”。<br>时区在参数值的时区偏移部分中指定，如果存在，则不会影响 `Prefer: outlook.timezone` 标头。 如果值中未包含时区偏移量，则将其解释为 UTC。<br>_不受_**日历中事件** 上的 delta 支持。 <br>在 **calendarView** 上为 **delta 所必需**。|
...
不支持 `$expand` 、 `$filter` 、 和 `$orderby` `$select` `$search` 。
...
### <a name="delta-function-on-events-preview"></a>事件上的 Delta 函数 (预览) 
如果成功，此方法在响应正文中返回响应代码 `200 OK` 和事件集合。 [](../resources/event.md) 出于 **性能** 原因，响应中的每个事件仅包含 **ID、****类型****、** 开始和结束属性。 随后 `GET /events/{id}` 使用扩展响应中的任意事件。  

### <a name="delta-function-on-calendarview"></a>calendarView 上的 Delta 函数
如果成功，此方法在响应正文中返回响应代码 `200 OK` 和事件集合。 [](../resources/event.md)

期望获取通常从请求获取的所有 `GET /calendarview` 属性。 

## <a name="examples"></a>示例

### <a name="example-1-delta-function-on-events-in-a-calendar-preview"></a>示例 1：日历中事件的 Delta 函数 (预览) 
#### <a name="request"></a>请求
以下示例显示用于获取已登录用户的默认日历中的事件的初始同步请求，该事件发生在指定参数上或之后 `startDateTime` 。 初始请求不包括任何状态令牌。 

请求使用标头将每个响应中的最大事件数限制为 `Prefer: odata.maxpagesize` 1。 继续使用 `delta` 返回的查询调用函数， `@odata.nextLink` 直到收到 `@odata.deltaLink` 响应。
...
如果请求成功，响应将包含状态令牌，该令牌是 _\@ odata.nextLink_ 响应标头) 中的 _skipToken_ (或 _\@ odata.deltaLink_ 响应标头) 中的 _deltaToken_ (。
它们分别指示是继续该轮，还是已完成获取该轮的所有更改。

下面的响应在 _\@ odata.nextLink_ 响应标头中显示 _skipToken。_
...
以下示例显示获取已登录用户指定日历中的事件的初始同步请求，该同步请求在 **calendarView** 指示的日期范围内。 初始请求不包括任何状态令牌。 

请求使用 `Prefer: odata.maxpagesize` 标头将每个响应中的最大事件数限制为 2。 继续使用返回的查询调用函数，直到获取该日历视图中的所有事件和 `delta` `@odata.nextLink` 响应 `@odata.deltaLink` 中的 a。
