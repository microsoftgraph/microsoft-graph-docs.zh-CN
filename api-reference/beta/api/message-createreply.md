...
创建以 JSON 或 MIME 格式答复邮件[](../resources/message.md)发件人的草稿。

使用 JSON 格式时：
- 指定参数的 comment 或 `message` **body** 属性。 指定这两者将返回 HTTP 400 错误请求错误。
- 如果在原始邮件中指定了 **replyTo**，则根据 Internet 邮件格式 ([RFC 2822](https://www.rfc-editor.org/info/rfc2822)) ，您应将答复发送给 **replyTo** 中的收件人，而不是收件人 **的发件人。**
