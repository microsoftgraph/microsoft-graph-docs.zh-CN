...
读取 [跨租户](../resources/crosstenantaccesspolicyconfigurationdefault.md) 访问策略的默认配置。 此默认配置可能是 Azure AD 分配的服务默认配置 (**isServiceDefault** `true`) 或在租户中自定义 (**isServiceDefault**) `false` 。
...
以下响应对象显示从 Azure AD 继承的默认跨租户策略，由 **isServiceDefault** 设置为 `true`标识。
