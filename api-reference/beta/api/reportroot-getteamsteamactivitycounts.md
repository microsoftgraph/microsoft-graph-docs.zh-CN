...
description: 获取跨Microsoft Teams的团队活动数。 活动类型与会议和消息相关。
...
获取跨Microsoft Teams的团队活动数。 活动类型与会议和消息相关。
...
| date      | Date   | 指定要查看团队间活动详细信息的日期。 {date_value} 必须采用格式 YYYY-MM-DD。 因为此报表的有效期仅为过去 30 天，所以 {date_value} 应为这个范围内的日期。 |
