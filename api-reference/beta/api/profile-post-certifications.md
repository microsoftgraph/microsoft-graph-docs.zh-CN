...
在用户配置文件 [中创建新的 personCertification](../resources/personcertification.md) [对象](../resources/profile.md)。
...
下表显示了在用户的配置文件中创建新 [personCertification](../resources/personcertification.md) 对象时可能设置 [的属性](../resources/profile.md)。

|属性|类型|说明|
|:---|:---|:---|
|allowedAudiences|String|能够查看实体中包含的值的访问群体。 继承自 [itemFacet](../resources/itemfacet.md)。 可取值为：`me`、`family`、`contacts`、`groupMembers`、`organization`、`federatedOrganizations`、`everyone`、`unknownFutureValue`。|
|certificationId  |String      |证书的可引用标识符。 |
|description      |String      |认证的说明。                   |
...
|issuedDate       |日期        |认证的发布日期。         |
|issuingAuthority |String      |证书授权的颁发机构。          |
|issuingCompany   |String      |证书授权的颁发机构。          |
|source|[personDataSource](../resources/persondatasource.md)|同步到其他服务时从哪个位置发送值。 继承自 [itemFacet](../resources/itemfacet.md)。|
|startDate        |日期        |认证的有效日期。       |
|thumbnailUrl     |String      |引用认证缩略图的 URL。   |
|webUrl           |String      |引用认证的 URL。                  |

## <a name="response"></a>响应

如果成功，此方法将在响应 `201 Created` 正文中返回 [响应代码和 personCertification](../resources/personcertification.md) 对象。
