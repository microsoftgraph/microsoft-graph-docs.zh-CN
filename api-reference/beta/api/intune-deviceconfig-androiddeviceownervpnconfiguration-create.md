...
|role|字符串|将连接类型设置为 Pulse Secure 时的角色。 继承自 [vpnConfiguration](../resources/intune-deviceconfig-vpnconfiguration.md)|
|realm|String|将连接类型设置为 Pulse Secure 时的领域。 继承自 [vpnConfiguration](../resources/intune-deviceconfig-vpnconfiguration.md)|
...
|alwaysOn|布尔值|是否启用始终启用 VPN 连接。|
|alwaysOnLockdown|布尔值|如果始终启用 VPN 连接，是否在断开 VPN 连接时锁定网络流量。|
...
如果成功，此方法在响应正文中返回 响应代码和 `201 Created` [androidDeviceOwnerVpnConfiguration](../resources/intune-deviceconfig-androiddeviceownervpnconfiguration.md) 对象。
