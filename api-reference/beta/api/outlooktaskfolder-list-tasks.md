...
默认情况下，此操作 (POST、PATCH 和 [完成](../api/outlooktask-complete.md) 任务操作) UTC 格式返回与日期相关的属性。  可以使用请求标头将响应中与日期相关的所有属性表示为与 `Prefer: outlook.timezone` UTC 不同的时区。 请参阅 [获取](outlooktask-get.md#example-2-get-outlook-task-with-date-time-properties-in-pacific-standard-time) 单个任务的示例。 可以以类似方式应用 标头，以获取多个任务。
