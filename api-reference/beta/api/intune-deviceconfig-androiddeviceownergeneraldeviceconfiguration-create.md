...
|appsRecommendSkippingFirstUseHints|布尔|是否推荐所有应用跳过他们可能会添加的任何首次使用提示。|
...
|crossProfilePoliciesAllowCopyPaste|Boolean|指示从一个配置文件复制的文本是否可以 (个人) 另一个配置文件复制的文本。|
...
|microsoftLauncherConfigurationEnabled|布尔|指示是否要配置微软桌面。|
...
|microsoftLauncherSearchBarPlacementConfiguration|[microsoftLauncherSearchBarPlacement](../resources/intune-deviceconfig-microsoftlaunchersearchbarplacement.md)|指示设备上搜索栏放置的配置。 可能的值是：`notConfigured`、`top`、`bottom`、`hide`。|
...
|kioskModeBluetoothConfigurationEnabled|布尔|是否允许用户在展台模式下蓝牙自定义设置。|
...
|kioskModeManagedSettingsEntryDisabled|布尔|是否以展台模式设置托管主屏幕上显示托管应用入口点。|
...
|kioskModeWifiAllowedSsids|字符串集合|用户可以在展台模式下配置的受限 WIFI SS ID 集。 该集合最多可包含 500 个元素。|
...
|kioskModeLockHomeScreen|布尔|是否在展台模式下向最终用户锁定主屏幕。|
...
|kioskModeManagedHomeScreenAutoSignout|布尔|托管主屏幕处于非活动状态后是否自动注销 M分屏和共享设备模式应用程序。|
...
|kioskModeManagedHomeScreenPinRequired|布尔|是否要求用户为托管主屏幕的登录会话设置 PIN。|
...
|stayOnModes|[androidDeviceOwnerBatteryPluggedMode](../resources/intune-deviceconfig-androiddeviceownerbatterypluggedmode.md) 集合|设备屏幕将保持打开状态的模式列表。 此集合最多可包含 4 个元素。 可能的值是：`notConfigured`、`ac`、`usb`、`wireless`。|
...
|personalProfileCameraBlocked|布尔|指示是否禁止在个人配置文件上使用相机。|
|personalProfileScreenCaptureBlocked|布尔|指示是否禁用在个人配置文件上拍摄屏幕截图的功能。|
