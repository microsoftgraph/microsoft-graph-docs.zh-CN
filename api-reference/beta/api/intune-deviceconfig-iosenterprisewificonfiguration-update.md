...
|outerIdentityPrivacyTemporaryValue|String|当 EAP 类型 (EAP 类型（TTLS、EAP) EAP 或 PEAP）时，启用标识隐私FAST外部标识。 此属性会屏蔽输入文本的用户名。 例如，如果使用"匿名"，则使用此连接进行身份验证的每个用户Wi-Fi真实用户名的"匿名"。|
