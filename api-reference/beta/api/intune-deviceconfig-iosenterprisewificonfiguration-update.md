...
|disableMacAddressRandomization|Boolean|如果设置为 true，则强制设备使用此 Wi-fi 配置文件进行连接，以提供实际的 Wlan MAC 地址，而不是随机 MAC 地址。 适用于 iOS 14 和更高版本。 继承自 [iosWiFiConfiguration](../resources/intune-deviceconfig-ioswificonfiguration.md)|
...
|trustedServerCertificateNames|String collection|将 EAP 类型配置为 EAP-TLS/TTLS/FAST 或 PEAP 时受信任的服务器证书名称。 这是受信任的证书颁发机构 (CA) 颁发的证书中使用的常用名称。 如果提供此信息，则可以绕过在最终用户的设备连接到此 Wi-fi 网络时显示的动态信任对话框。|
