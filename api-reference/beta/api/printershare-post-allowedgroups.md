...
向指定的组授予将打印作业提交到关联的 [printerShare](../resources/printershare.md)的权限。
...
在请求正文中，通过使用格式提供对组实体的引用 `@odata.id` ，如下面的示例所示。
...
在请求正文中，通过在 JSON 正文的字段中包含组的 Microsoft Graph URI，提供对组实体的引用 `@odata.id` 。
