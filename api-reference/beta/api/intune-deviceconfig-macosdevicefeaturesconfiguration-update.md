...
|authorizedUsersListHidden|布尔|是否在登录窗口中显示名称和密码对话框或用户列表。|
|authorizedUsersListHideLocalUsers|布尔|是否在登录窗口的授权用户列表中只显示网络和系统用户。|
|authorizedUsersListHideMobileAccounts|布尔|是否在登录窗口的授权用户列表中隐藏移动用户。|
...
|authorizedUsersListShowOtherManagedUsers|布尔|是否在登录窗口的授权用户列表中显示其他用户。|
|shutDownDisabled|布尔|是否在登录窗口中隐藏"关闭"按钮项。|
|restartDisabled|布尔|是否在登录窗口中隐藏"重启"按钮项。|
|sleepDisabled|布尔|是否在登录窗口上隐藏"睡眠"菜单项。|
|consoleAccessDisabled|Boolean|其他用户是否忽略使用 `console` 特殊用户名。|
|shutDownDisabledWhileLoggedIn|布尔|登录窗口上的"关闭"菜单项在用户登录时是否将被禁用。|
...
|associatedDomains|[keyValuePair](../resources/intune-shared-keyvaluepair.md) 集合|已弃用：改为使用 appAssociatedDomains。 获取或设置将应用映射到其关联域的列表。 密钥应匹配应用的 ID，并且值应为"service：domain"形式的字符串，其中 domain 是完全限定的主机名 (例如 webcredentials：example.com) 。 该集合最多可包含 500 个元素。|
...
|contentCachingMaxSizeBytes|Int64|将用于内容缓存的最大磁盘空间字节数。 默认值为 0 (表示) 磁盘空间不受限制。 |
...
|contentCachingForceConnectionSharing|布尔|强制进行 Internet 连接共享。 contentCachingDisableConnectionSharing 会覆盖此设置。|
...
|contentCachingLogClientIdentities|布尔|启用请求缓存内容的客户端的 IP 地址和端口的日志记录。|
...
|contentCachingShowAlerts|布尔|将内容缓存警报显示为系统通知。|
|contentCachingKeepChingke|布尔|如果启用了内容缓存，则防止设备休眠。|
