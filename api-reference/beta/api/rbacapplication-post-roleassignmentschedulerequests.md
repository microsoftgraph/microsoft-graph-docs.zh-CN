...
|principalId|字符串|要向其授予分配的主体的标识符。|
...
|isValidationOnly|布尔值|指定调用是验证还是实际调用。 仅当要在实际提交请求之前检查激活是否受 MFA 等其他规则约束时，才设置此属性。|
|targetScheduleId|字符串|附加到分配的计划对象的 ID。|
