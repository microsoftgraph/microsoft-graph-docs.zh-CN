...
更新 [androidDeviceOwnerGeneralDeviceConfiguration 对象的](../resources/intune-deviceconfig-androiddeviceownergeneraldeviceconfiguration.md) 属性。
...
在请求正文中，为 [androidDeviceOwnerGeneralDeviceConfiguration](../resources/intune-deviceconfig-androiddeviceownergeneraldeviceconfiguration.md) 对象提供 JSON 表示形式。

下表显示了创建 [androidDeviceOwnerGeneralDeviceConfiguration](../resources/intune-deviceconfig-androiddeviceownergeneraldeviceconfiguration.md) 时所需的属性。
...
|azureAdSharedDeviceDataClearApps|[appListItem](../resources/intune-deviceconfig-applistitem.md) 集合|托管应用的列表，这些应用将在全局注销期间将其数据清除AAD共享设备模式。 该集合最多可包含 500 个元素。|
...
|appsDefaultPermissionPolicy|[androidDeviceOwnerDefaultAppPermissionPolicyType](../resources/intune-deviceconfig-androiddeviceownerdefaultapppermissionpolicytype.md)|指示针对运行时权限的请求的权限策略（如果未专门为应用定义权限）。 可取值为：`deviceDefault`、`prompt`、`autoGrant`、`autoDeny`。|
|appsRecommendSkippingFirstUseHints|Boolean|是否建议所有应用跳过他们可能添加的任何首次使用提示。|
...
|microsoftLauncherCustomWallpaperEnabled|Boolean|指示是否在目标设备上配置壁纸。|
...
|factoryResetDeviceAdministratorEmails|字符串集合|在设备在设置出厂重置后需要进行身份验证的 Google 帐户电子邮件列表。|
...
|kioskCustomizationDeviceSettingsBlocked|Boolean|指示用户是否可以在展台模式下访问设备的设置应用。|
...
|kioskModeWallpaperUrl|String|当设备处于展台模式时，用于壁纸的可公开访问的映像的 URL。|
...
|kioskModeFlashlightConfigurationEnabled|Boolean|是否允许用户在展台模式下使用手电筒。|
...
|kioskModeAppPositions|[androidDeviceOwnerKioskModeAppPositionItem](../resources/intune-deviceconfig-androiddeviceownerkioskmodeapppositionitem.md) 集合|展台模式托管主屏幕上项的排序。 该集合最多可包含 500 个元素。|
|kioskModeManagedHomeScreenAutoSignout|Boolean|是否在托管主屏幕处于非活动状态后自动注销 MHS 和共享设备模式应用程序。|
|kioskModeManagedHomeScreenInactiveSignOutDelayInSeconds|Int32|自动将用户注销到托管主屏幕之前通知用户的秒数。 有效值 0 到9999999|
|kioskModeManagedHomeScreenInactiveSignOutNoticeInSeconds|Int32|在自动为托管主屏幕注销用户之前，设备处于非活动状态的秒数。 有效值 0 到9999999|
...
|kioskModeManagedHomeScreenPinRequiredToResume|Boolean|如果托管主屏幕出现屏幕保护程序，是否需要用户输入会话 PIN。|
|kioskModeManagedHomeScreenSignInBackground|String|托管主屏幕登录屏幕的自定义 URL 背景。|
|kioskModeManagedHomeScreenSignInBrandingLogo|String|用于登录屏幕的自定义 URL 品牌徽标和托管主屏幕的会话固定页。|
...
|microphoneForceMute|Boolean|指示是否阻止在设备上取消麦克风。|
...
|passwordBlockKeyguardFeatures|[androidKeyguardFeature](../resources/intune-deviceconfig-androidkeyguardfeature.md) 集合|要阻止的设备密钥防护功能列表。 该集合最多可包含 7 个元素。 可取值为：`notConfigured`、`camera`、`notifications`、`unredactedNotifications`、`trustAgents`、`fingerprint`、`remoteInput`、`allFeatures`。|
...
|vpnAlwaysOnLockdownMode|Boolean|如果指定了 Always on VPN 包名称，则在该 VPN 断开连接时是否锁定网络流量。|
...
|wifiBlockEditPolicyDefinedConfigurations|Boolean|指示是否仅阻止用户编辑策略定义的网络。|
...
如果成功，此方法在响应正文中返回 `200 OK` 响应代码和更新后的 [androidDeviceOwnerGeneralDeviceConfiguration](../resources/intune-deviceconfig-androiddeviceownergeneraldeviceconfiguration.md) 对象。
