...
在 [Azure AD 权利管理](../resources/entitlementmanagement-root.md)中，检索 [accessPackageAssignmentRequest](../resources/accesspackageassignmentrequest.md) 对象的列表。  生成的列表包括所有目录和访问包中调用方有权访问读取的当前和已过期的所有分配请求。
...
此方法支持一些 OData 查询参数来帮助自定义响应。 例如，要检索每个请求的访问包，请包括在 `$expand=accessPackage` 查询中。  若要仅检索特定访问包的请求，在查询中包括筛选器，如 `$expand=accessPackage&$filter=accessPackage/id eq '9bbe5f7d-f1e7-4eb1-a586-38cdf6f8b1ea'` 。  若要检索生成的分配，请 `$expand=accessPackageAssignment` 包括在查询中。
...
如果成功，此方法在响应正文中返回响应代码 `200 OK` 和 [accessPackageAssignmentRequest](../resources/accesspackageassignmentrequest.md) 对象集合。
