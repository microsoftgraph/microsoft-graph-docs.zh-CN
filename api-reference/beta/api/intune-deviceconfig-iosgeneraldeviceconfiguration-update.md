...
|airDropBlocked|布尔|指示设备处于监督模式时是否允许使用 AirDrop。|
...
|appStoreBlocked|布尔|指示是否阻止用户使用应用商店。|
|appStoreBlockInAppPurchases|布尔|指示是否阻止用户进行应用内购买。|
|appStoreBlockUIAppInstallation|布尔|指示是否阻止应用商店应用，而不通过主机应用限制安装。 仅适用于监督模式（iOS 9.0 及更高版本）。|
|appStoreRequirePassword|布尔|指示使用应用商店时是否需要密码。|
...
|enterpriseAppBlockTrustModification|布尔|指示是否阻止用户修改企业应用信任设置。|
|esimBlockModification|布尔|指示是否允许在受监督的设备的 eSIM 卡上添加或删除手机网络计划。|
|faceTimeBlocked|布尔|指示是否阻止用户使用 FaceTime。|
|findMyFriendsBlocked|布尔|指示设备处于监督模式时是否阻止查找我的好友。|
|gamingBlockGameCenterFriends|布尔|指示是否阻止用户在 Game Center 中拥有好友。|
|gamingBlockMultiplayer|布尔|指示是否阻止用户使用多人游戏。|
...
|iCloudBlockActivityContinuation|布尔|指示是否阻止用户在另一个 iOS 或 MacOS 设备上继续从事在 iOS 设备上启动的工作。|
|iCloudBlockBackup|布尔|指示是否阻止 iCloud 备份。|
...
|iTunesBlockExplicitContent|布尔|指示是否阻止用户访问 iTunes 和 App Store 中的显式内容。|
...
|kioskModeAllowAutoLock|布尔|指示在展台模式下是否允许设备自动锁定。|
|kioskModeAllowColorInversionSettings|布尔|指示在展台模式下是否允许访问颜色反转设置。|
|kioskModeAllowRingerSwitch|布尔|指示在展台模式下是否允许使用响铃开关。|
|kioskModeAllowScreenRotation|布尔|指示在展台模式下是否允许屏幕旋转。|
|kioskModeAllowSleepButton|Boolean|指示在展台模式下是否允许使用睡眠按钮。|
|kioskModeAllowTouchscreen|布尔|指示在展台模式下是否允许使用触摸屏。|
|kioskModeAllowVoiceOverSettings|布尔|指示在展台模式下是否允许访问语音过滤设置。|
|kioskModeAllowVolumeButtons|布尔|指示在展台模式下是否允许使用音量按钮。|
...
|kioskModeRequireVoiceOver|布尔|指示在展台模式下是否要求语音过滤。|
...
|lockScreenBlockTodayView|布尔|指示是否阻止用户在锁定屏幕上使用今日视图。|
...
|safariBlocked|布尔|指示是否阻止用户使用 Safari。|
...
|keychainBlockCloudSync|布尔|指示是否阻止 iCloud 密钥链同步。|
