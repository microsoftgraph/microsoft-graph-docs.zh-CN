...
ms.locfileid: "59135688"
---
# <a name="update-macoswirednetworkconfiguration"></a>更新 macOSWiredNetworkConfiguration
...
更新 [macOSWiredNetworkConfiguration 对象](../resources/intune-deviceconfig-macoswirednetworkconfiguration.md) 的属性。
...
在请求正文中，提供 [macOSWiredNetworkConfiguration](../resources/intune-deviceconfig-macoswirednetworkconfiguration.md) 对象的 JSON 表示形式。

下表显示创建 [macOSWiredNetworkConfiguration 时所需的属性](../resources/intune-deviceconfig-macoswirednetworkconfiguration.md)。
...
|trustedServerCertificateNames|字符串集合|将 EAP 类型配置为 EAP-TLS/TTLS/FAST PEAP 时受信任的服务器证书名称。 这是由受信任证书颁发机构颁发给 CA 证书颁发机构颁发的证书 (名) 。 如果提供此信息，可以在最终用户连接到此有线网络时绕过显示在最终用户设备上的动态信任对话框。|
...
|enableOuterIdentityPrivacy|String|将 EAP (EAP 类型) EAP-TTLS、EAP-FAST PEAP 时，启用标识隐私和外部标识。 此属性会屏蔽输入文本的用户名。 例如，如果使用"匿名"，则使用真实用户名对此有线网络进行身份验证的每个用户将显示为"匿名"。|



## <a name="response"></a>响应
如果成功，此方法在响应正文中返回 响应代码和更新的 `200 OK` [macOSWiredNetworkConfiguration](../resources/intune-deviceconfig-macoswirednetworkconfiguration.md) 对象。
