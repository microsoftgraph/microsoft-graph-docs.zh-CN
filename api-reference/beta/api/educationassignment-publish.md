...
如果工作分配计划在将来的日期`draft``scheduled`，您可以将状态从 更改为 。 

只有课堂中的教师才能进行此呼叫。 当 **作业** 在草稿状态时，学生将看不到作业，也不会看到任何提交对象。 调用此 API 时， [将创建 educationSubmission](../resources/educationsubmission.md) 对象，作业将显示在学生列表中。

如果发布 **过程中出现** 任何 `draft` 后端故障，则分配的状态将返回。

若要更新已发布工作分配 **的属性，** 请参阅 [更新工作分配](../api/educationassignment-update.md)。

若要更新已发布工作分配的属性，请参阅 [更新工作分配](../api/educationassignment-update.md)。
