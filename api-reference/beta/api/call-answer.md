...
`participantCpapacity` `Answer`当在基于策略的录制通知的请求中指定时，属于同一策略组的后续参与者加入事件将发送为 [participantJoiningNotification](../resources/participantJoiningNotification.md)`participantCapacity`，而不是新的传入呼叫通知，直到当前呼叫实例处理的参与者数量达到 中指定的号码。
