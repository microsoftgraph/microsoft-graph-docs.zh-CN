...
根据正从中获取扩展属性的资源以及所请求的权限类型 (委派或应用程序) ，下表中指定的权限至少是调用此 API 所需的权限。 若要了解详细信息，包括如何选择权限的信息，请参阅[权限](/graph/permissions-reference)。
...
请确保应用 [筛选器字符串中的 URL](https://www.w3schools.com/tags/ref_urlencode.asp) 编码 - 冒号、正斜杠和空格。
