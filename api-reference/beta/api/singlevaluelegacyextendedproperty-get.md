...
获取支持的资源实例，其中包含与筛选器匹配的数字扩展属性。 筛选器在 **id** 属性上使用 `eq` 运算符，并在 **value** 属性上使用以下运算符之一：`eq`、`ne`、`ge`、`gt`、`le` 或 `lt`。 请确保您应用使确保您应用于[URL 编码](https://www.w3schools.com/tags/ref_urlencode.asp)冒号、 正斜杠和空间下列中的筛选器字符串的字符。

以下语法行显示对 id 使用 `eq` 运算符的筛选器，对属性值使用另一个 `eq` 运算符。 可以使用适用于数值的其他运算符中的任何一个（`ne`、`ge`、`gt`、`le` 或 `lt`）替换 **value** 上的 `eq` 运算符。

获取**消息**实例：<!-- { "blockType": "ignored" } -->
```http
