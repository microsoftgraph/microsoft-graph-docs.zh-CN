...
下表显示时创建[iosVpnConfiguration](../resources/intune-deviceconfig-iosvpnconfiguration.md)所需的属性。
...
|userDomain|String|仅 Zscaler。 输入静态域预填充具有 Zscaler 应用程序中的登录字段。 如果这保留为空，将使用用户的 Azure Active Directory 域。|
|strictEnforcement|Boolean|仅 Zscaler。 块到 Zscaler 应用程序直到用户迹象网络流量。 搕 rue 意味着阻止流量。|
|cloudName|String|仅 Zscaler。 该用户分配给 Zscaler 云。|
|excludeList|String 集合|仅 Zscaler。 不会通过 Zscaler 云发送的网络地址的列表。|



## <a name="response"></a>响应
如果成功，此方法返回`200 OK`响应代码和响应正文中的更新的[iosVpnConfiguration](../resources/intune-deviceconfig-iosvpnconfiguration.md)对象。
