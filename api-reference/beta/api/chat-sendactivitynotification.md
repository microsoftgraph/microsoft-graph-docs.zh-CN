...
此示例演示如何向所有聊天成员发送活动源通知。 此示例与前面的示例类似。 但是，在这种情况下， 是 `recipient` [chatMembersNotificationRecipient](../resources/chatmembersnotificationrecipient.md)。 请注意， `chatId` 中指定的 必须与 `recipient` 请求 `chatId` URL 中指定的 匹配。
