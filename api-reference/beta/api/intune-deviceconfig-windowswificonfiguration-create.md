...
创建新的 [windowsWifiConfiguration](../resources/intune-deviceconfig-windowswificonfiguration.md) 对象。
...
在请求正文中，提供 windowsWifiConfiguration 对象的 JSON 表示形式。

下表显示创建 windowsWifiConfiguration 时所需的属性。
...
|connectAutomatically|布尔值|指定 wifi 连接是否在范围内时自动连接。|
|connectToPreferredNetwork|布尔值|指定 wifi 连接在已连接到此网络时是否应该连接到更首选的网络。  要求 ConnectAutomatically 为 true。|
|connectWhenNetworkNameIsHidden|布尔值|指定即使在 SSID 未广播时 wifi 连接是否应该自动连接。|
...
如果成功，此方法在响应正文中返回 响应代码和 `201 Created` [windowsWifiConfiguration](../resources/intune-deviceconfig-windowswificonfiguration.md) 对象。
