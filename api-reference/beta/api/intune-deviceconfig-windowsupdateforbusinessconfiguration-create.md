...
|skipChecksBeforeRestart|布尔|设置为在重启前跳过所有检查：电池电量 = 40%，用户状态、需要显示、演示模式、全屏模式、电话呼叫状态、游戏模式等。 |
...
|postponeRebootUntilAfterDeadline|布尔|指定设备是否等到使用时段外重启的截止时间|
...
|allowWindows11Upgrade|布尔|允许符合条件的Windows 10设备升级到最新版本的 Windows 11。|
