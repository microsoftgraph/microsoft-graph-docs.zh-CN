...
|keychainBlockCloudSync|布尔值|指示 iCloud 钥匙链同步阻止 (macOS 10.12 及更高版本)。|
|airPrintBlocked|布尔值|指示 AirPrint 阻止 (macOS 10.12 及更高版本)。|
|airPrintForceTrustedTLS|布尔值|指示受信任的证书是否需要 TLS 打印通信 (macOS 10.13 及更高版本)。|
|airPrintBlockiBeaconDiscovery|布尔值|指示阻止 iBeacon 发现 AirPrint 打印机。 这样可以防止在从网络流量 (macOS 10.3 及更高版本) 的网络钓鱼的虚假 AirPrint 蓝牙信号。|
...
|appleWatchBlockAutoUnlock|布尔值|指示是否或阻止用户解锁与 Apple Watch 其 Mac。|
|iTunesBlockFileSharing|布尔值|指示阻止进行文件传输使用 iTunes。|
|iCloudBlockDocumentSync|Boolean|指示是否阻止 iCloud 文档同步。|
|iCloudBlockMail|布尔值|指示阻止 iCloud 从同步邮件。|
|iCloudBlockAddressBook|布尔值|指示阻止 iCloud 从同步联系人。|
|iCloudBlockCalendar|布尔值|指示阻止 iCloud 从同步日历。|
|iCloudBlockReminders|布尔值|指示阻止 iCloud 从同步提醒。|
|iCloudBlockBookmarks|布尔值|指示阻止 iCloud 从同步书签。|
|iCloudBlockNotes|布尔值|指示阻止 iCloud 从同步注释。|
|airDropBlocked|Boolean|指示允许 AirDrop。|
|passwordBlockModification|布尔值|指示允许密码修改。|
