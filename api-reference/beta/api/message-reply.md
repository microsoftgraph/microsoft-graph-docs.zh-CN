...
此外，您可以第一个[创建草稿答复邮件](../api/message-createreply.md)包含评论或更新任何消息属性，然后[发送](../api/message-send.md)答复。

**注意**

- 您可以指定注释或的**body**属性`message`参数。 如果同时指定将返回 HTTP 400 错误请求出错。
- 如果**回复**属性指定在原始邮件中，每个 Internet 邮件格式 ([RFC 2822](https://www.rfc-editor.org/info/rfc2822))，您应在**回复**并不**从**属性中的收件人发送对收件人的答复。 
