...
description: 获取由特定用户主体筛选的 unifiedRoleEligibilityScheduleRequest 对象及其属性的列表
author: rkarim-ms
...
获取 [unifiedRoleEligibilityScheduleRequest](../resources/unifiedRoleEligibilityScheduleRequest.md) 对象及其与当前已登录主体对象关联的属性的列表。 

> [!NOTE]
> 此方法不会检索当前已登录用户所属、具有合格分配的组的对象。
...
|参数|类型|说明|
|:---|:---|:---|
|on|RoleEligibilityScheduleRequestFilterByCurrentUserOptions|筛选以查询当前用户为主体的对象。 允许的值为 `principal`. 必填。 不检索此用户所属组的分配。|
