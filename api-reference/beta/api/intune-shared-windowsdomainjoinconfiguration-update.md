...
更新[windowsDomainJoinConfiguration](../resources/intune-shared-windowsdomainjoinconfiguration.md)对象的属性。
...
下表显示时创建[windowsDomainJoinConfiguration](../resources/intune-shared-windowsdomainjoinconfiguration.md)所需的属性。
...
注意： 请求正文中属性支持取决于呼叫上下文。  并非所有属性都都适用于所有工作流。

## <a name="response"></a>响应
如果成功，此方法返回`200 OK`响应代码和响应正文中的更新的[windowsDomainJoinConfiguration](../resources/intune-shared-windowsdomainjoinconfiguration.md)对象。
...
### <a name="response"></a>响应
下面是一个响应示例。 注意：为简洁起见，可能会截断此处展示的响应对象。 根据上下文而有所不同实际的调用返回的属性。
``` http
HTTP/1.1 200 OK
Content-Type: application/json
Content-Length: 521
