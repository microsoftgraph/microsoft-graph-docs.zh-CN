...
|userDomain|String|仅 Zscaler。 在 Zscaler 应用中输入要预填充登录字段的静态域。 如果此域留空，则Azure Active Directory域。 继承自 [iosVpnConfiguration](../resources/intune-deviceconfig-iosvpnconfiguration.md)|
...
|deadPeerDetectionRate|[vpnDeadPeerDetectionRate](../resources/intune-deviceconfig-vpndeadpeerdetectionrate.md)|确定检查对等连接是否仍处于活动状态频繁。 . 可能的值是：`medium`、`none`、`low`、`high`。|
...
|serverCertificateType|[vpnServerCertificateType](../resources/intune-deviceconfig-vpnservercertificatetype.md)|VPN 服务器将呈现给 VPN 客户端以进行身份验证的证书类型。 可能的值是：`rsa`、`ecdsa256`、`ecdsa384`、`ecdsa521`。|
|sharedSecret|String|在选择"共享密码身份验证"时使用|
|tlsMaximumVersion|字符串|要与 EAP-TLS 身份验证一同使用的最大 TLS 版本|
