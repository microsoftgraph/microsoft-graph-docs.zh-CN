...
由于 **组** 资源 [支持扩展](/graph/extensibility-overview)`PATCH`，因此可以使用 操作在现有组实例的扩展的自定义属性中添加、更新或删除你自己的特定于 **应用的数据。**
...
>
> - 只有一部分与核心组管理和管理相关的组 API 才同时支持应用程序权限和委派权限。 组 API 的所有其他成员（包括更新 **autoSubscribeNewMembers**）仅支持委派权限。 有关示例，请参阅[已知问题](/graph/known-issues#groups)。
>
...
可以使用列表标签获取要应用于组Microsoft 365 [ID。](informationprotectionpolicy-list-labels.md) 然后，可以使用标签 ID 更新组的 [assignedLabels](../resources/assignedlabel.md) 属性。
