---
title: 邮件：取消订阅
description: 代表登录用户提交电子邮件请求，以取消订阅电子邮件通讯组列表。 使用 标头 `List-Unsubscribe` 中的信息。
author: abheek-das
...
代表登录用户提交电子邮件请求，以取消订阅电子邮件通讯组列表。 使用 标头 `List-Unsubscribe` 中的信息。

邮件发件人可以通过包括收件人选择退出的选项，以用户友好的方式使用邮件列表。他们可以通过在 `List-Unsubscribe` [RFC-2369](https://www.faqs.org/rfcs/rfc2369.html)之后的每封邮件中指定头来这样做。

**注意** 特别是，若要 **使取消订阅操作** 正常工作，发件人必须指定而不是 `mailto:` 基于 URL 的取消订阅信息。

设置该邮件头还会将邮件实例的 **unsubscribeEnabled** 属性设置为 ，将 [](../resources/message.md) `true` **unsubscribeData** 属性设置为头数据。

如果 **邮件的 unsubscribeEnabled** 属性为 ，您可以使用取消订阅操作取消订阅用户与邮件发件人管理的类似未来 `true` 邮件。 
