...
|remotePassportEnabled|Boolean|控制 Remote Windows Hello for Business 的使用。 远程 Windows Hello for Business 使可移植的已注册设备能够用作桌面身份验证的配套设备。 桌面必须加入 Azure AD，配套设备必须具有适用于Windows Hello PIN。|
...
|pinExpirationInDays|Int32|控制在系统 (PIN) 之前可以使用 PIN 的时间（以天表示）。 这必须设置为 0 到 730 之间（包含两者）。 如果设置为 0，则用户的 PIN 永不过期|
