...
停止[访问评审阶段](../resources/accessReviewStage.md)。`inProgress` 访问评审阶段停止后，阶段 **状态** 将为 `Completed` ，审阅者将无法再提供输入。 如果后续阶段依赖于已完成的阶段，则将创建下一阶段。 

无论阶段的状态如何， [accessReviewInstanceDecisionItem](../resources/accessreviewinstancedecisionitem.md) 对象始终反映在给定时间记录的所有阶段的最后决策。
