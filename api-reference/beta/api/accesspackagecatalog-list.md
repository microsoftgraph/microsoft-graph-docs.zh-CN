...
此方法支持一些 OData 查询参数来帮助自定义响应。  例如，若要检索每个目录中的访问包，请包括在 `$expand=accessPackages` 查询中。 若要搜索具有特定名称的访问包目录，请包含筛选器（ `$filter=contains(tolower(displayName),'staff')` 如 在查询中）。  若要了解一般信息，请参阅 [OData 查询参数](/graph/query-parameters)。
