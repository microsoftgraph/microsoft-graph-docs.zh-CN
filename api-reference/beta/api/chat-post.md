...
如果成功，此方法在响应 `201 Created` 正文中返回 响应 [代码和新建](../resources/chat.md) 的聊天资源。

### <a name="response-for-creating-a-one-on-one-chat-with-installed-apps"></a>使用已安装的应用创建一对一聊天的响应
如果成功，此方法将返回 响应 `202 Accepted` 代码和 Location 标头，其中包含 [指向 teamsAsyncOperation 的链接](../resources/teamsasyncoperation.md)。 该链接可用于获取操作状态和详细信息。 有关详细信息，请参阅 [获取聊天操作](teamsasyncoperation-get.md#example-get-operation-on-chat)。
...
异步操作已启动，响应包含 Location 标头，其中包含指向 [teamsAsyncOperation 的链接](../resources/teamsasyncoperation.md)。 该链接可用于获取操作状态和详细信息。 有关详细信息，请参阅 [获取聊天操作](teamsasyncoperation-get.md#example-get-operation-on-chat)。
...
### <a name="example-5-create-a-group-chat-with-tenant-guest-user"></a>示例 5：创建与租户来宾用户的群聊
