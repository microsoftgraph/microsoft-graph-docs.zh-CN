...
1) 如果将 allowUpdateConfigurationOverride 设置为 true，则将跳过租户设置的更新配置，并且代理将在下一版本的代理可用时收到更新（优先级为1）。
2) 如果设置了延迟更新，将不会更新代理，直到延迟更新日期时间（优先级2）。
3) 如果设置了 "更新" 窗口，则代理将仅在24小时内（优先级为3）的该时间段内更新。
