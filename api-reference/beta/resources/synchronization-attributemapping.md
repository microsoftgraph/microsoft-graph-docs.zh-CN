...
|flowBehavior               | 枚举字符串      |定义此属性时应可导出到目标目录。 可能的值为：`FlowWhenChanged`和`FlowAlways`。 默认值为 `FlowWhenChanged`。 |
|flowType                   | ennum 字符串          |定义此属性时应更新目标目录中。 可能的值为： `Always` （默认）， `ObjectAddOnly` （仅当创建新对象）， `MultiValueAddOnly` （仅当更改正在添加新值到多值属性）。 |
...
|source                     |[attributeMappingSource](synchronization-attributemappingsource.md)     | 定义如何值应为源对象中提取 （或转换）。 |
