...
| 属性   | 类型 |说明|
|:---------------|:--------|:----------|
|signInType|字符串| 指定目录中的用户登录类型，如`emailAddress` `userName`或。 `federated` 此处， `federated`表示颁发者的用户的唯一标识符，该标识符可以采用颁发者选择的任何格式。 当登录类型设置为**** `emailAddress`或`userName`时，对 issuerAssignedId 强制执行其他验证。 此属性还可以设置为任何自定义字符串，例如，允许多个电子邮件地址。<br><br>支持`$filter`。|
|常用|字符串|指定标识的颁发者，例如`facebook.com`。<br>对于本地帐户（其中**signInType**不`federated`是），此属性是本地 B2C 租户默认域名（例如`contoso.onmicrosoft.com`）。<br>对于来自其他 Azure AD 组织的外部用户，这将是联合组织的域，例如`contoso.com`。<br><br>支持`$filter`。 512字符限制。|
