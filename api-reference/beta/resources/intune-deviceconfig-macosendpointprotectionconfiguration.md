...
|advancedThreatProtectionRealTime|[enablement](../resources/intune-shared-enablement.md)|确定是否在 macOS 上为 Microsoft Defender for Endpoint 启用实时保护。 可取值为：`notConfigured`、`enabled`、`disabled`。|
...
|advancedThreatProtectionExcludedExtensions|String 集合|从 macOS 上的 Microsoft Defender for Endpoint 防病毒扫描中排除的文件扩展名列表。|
