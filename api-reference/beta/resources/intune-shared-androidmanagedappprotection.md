...
|exemptedAppPackages|[keyValuePair](../resources/intune-shared-keyvaluepair.md) 集合|此列表中的应用包将不受策略约束，并且能够从托管应用接收数据。|
...
|appActionIfAndroidDeviceManufacturerNotAllowed|[managedAppRemediationAction](../resources/intune-mam-managedappremediationaction.md)|如果不允许指定的设备制造商，则定义托管应用行为（阻止或擦除）。 可取值为：`block`、`wipe`、`warn`。|
...
|appActionIfAndroidSafetyNetDeviceAttestationFailed|[managedAppRemediationAction](../resources/intune-mam-managedappremediationaction.md)|如果指定的 Android SafetyNet 证明查询失败，则定义托管应用行为（警告或阻止）。 可取值为：`block`、`wipe`、`warn`。|
...
|appActionIfAndroidSafetyNetAppsVerificationFailed|[managedAppRemediationAction](../resources/intune-mam-managedappremediationaction.md)|如果指定的 Android 应用验证查询失败，则定义托管应用行为（警告或阻止）。 可取值为：`block`、`wipe`、`warn`。|
