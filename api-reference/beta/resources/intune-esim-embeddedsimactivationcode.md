...
|属性|类型|说明|
|:---|:---|:---|
|integratedCircuitCardIdentifier|String|由移动运营商提供的嵌入的 SIM 激活代码 (ICCID) 的集成电路卡标识符。
输入必须与以下正则表达式匹配： ' ^ \[ 0-9 \] {19} \[ 0-9 \] ？ $ '。|
|matchingIdentifier|String|MatchingIdentifier (MatchingID) 在 GSMA Association SGP RSP 技术规范部分中指定4.1。
输入必须与以下正则表达式匹配： ' ^ \[ Z0-9 \- \] * $ '。|
