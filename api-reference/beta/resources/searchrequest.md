...
|fields|String collection |包含要为在 **entityTypes**中指定的搜索资源对象返回的字段，允许自定义默认情况下返回的字段（包括来自 SharePoint 和 OneDrive 的其他字段，也可以是 **externalItem** 中的自定义托管属性）图表连接器的内容引入中的自定义字段。 可选。|
...
|stored_fields (已弃用) |String collection |现在，此属性将替换为 **fields** 属性。 |
