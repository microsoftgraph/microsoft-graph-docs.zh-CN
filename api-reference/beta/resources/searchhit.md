...
|_id (已弃用) |String| 重命名为 **hitId**。 项的内部标识符。|
|_score (弃用的) |Int32|已重命名为 **排名**。 结果的分数或顺序。|
|_summary (已弃用) |String|重命名为 **摘要**。 如果摘要可用，则结果摘要 () 。|
|_sortField (弃用的) |String|此属性已删除。|
|_source (已弃用) |[实体](entity.md)|已重命名为 **资源**。 搜索结果的基础 Graph 表示形式。|
