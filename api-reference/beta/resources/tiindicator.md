...
有关受支持的指示器类型和每个租户的指标计数限制的详细信息，请参阅[Manage 指示器](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/manage-indicators)。
...
| [Update](../api/tiindicator-update.md) | [tiIndicator](tiindicator.md) | 更新 tiIndicator 对象。 |
...
|targetProduct|字符串|一个字符串值，表示应应用指标的单个安全产品。 可接受的值`Azure Sentinel`为`Microsoft Defender ATP`：、。 **Required**|
...
|fileName|字符串|如果该标记是基于文件的，则为该文件的名称。 可以用逗号分隔多个文件名。 |
...
|userAgent|字符串|来自 web 请求的用户代理字符串，可能表明已泄露。|
