...
Azure Active Directory (Azure AD) 标识同步 (也称为"预配") 允许你从以下任一项自动执行 (标识的 (创建、维护) 和取消预配 (删除) 操作：
...
还可以在示例租户或你自己的租户 [的 Graph 资源管理器](https://developer.microsoft.com/graph/graph-explorer) 中试用 API。
...
同步架构定义将同步哪些对象以及如何同步它们。 同步架构包含特定同步作业的多数安装信息。 通常，您将自定义某些[属性](synchronization-attributemapping.md)映射，或添加范围筛选器以仅同步满足[](synchronization-filter.md)特定条件的对象。
...
使用同步 API 主要涉及访问 [synchronizationJob 和](synchronization-synchronizationjob.md) [synchronizationSchema](synchronization-synchronizationschema.md) 资源。 若要查找 [synchronizationJob](synchronization-synchronizationjob.md) 资源，需要知道同步作业所属的服务主体对象的 ID。 以下示例显示如何使用 **synchronizationJob** 和 **synchronizationSchema** 资源。
