...
|operationType|[win32LobPowerShellScriptRuleOperationType](../resources/intune-apps-win32lobpowershellscriptruleoperationtype.md)|脚本输出比较操作类型。 如果规则用于检测，则使用 NotConfigured （默认值）。 可取值为：`notConfigured`、`string`、`dateTime`、`integer`、`float`、`version` 或 `boolean`。|
