...
|notificationChannelUrl|String|用于指定用于帖子 **分配** 发布通知的 [通道](channel.md) URL 的可选字段。 如果未指定或为 null，则 `General` 默认为通道。 此字段仅适用于 **assignTo** 值为 [educationAssignmentClassRecipient](educationassignmentclassrecipient.md) 的 **作业**。 发布 **分配** 后，不允许更新 **notificationChannelUrl**。|
...
| 关系 | 类型   |说明|
|:---------------|:--------|:----------|
|resources|[educationAssignmentResource](educationassignmentresource.md) 集合| Learning与此 **分配** 关联的对象。  只有教师才能修改此列表。 可为 NULL。|
