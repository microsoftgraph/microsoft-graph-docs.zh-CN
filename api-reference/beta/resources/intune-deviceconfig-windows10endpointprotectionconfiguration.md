...
|xboxServicesEnableXboxGameSaveTask|布尔值|此设置确定 xbox 游戏保存是 (1) 0 (禁用) 。|
...
|localSecurityOptionsAdministratorAccountName|字符串|定义要与帐户"Administrator" (SID) 相关联的其他帐户名称。|
...
|defenderSecurityCenterHelpPhone|字符串|向用户Skype的电话号码或 ID。|
|defenderSecurityCenterHelpURL|字符串|向用户显示的帮助门户 URL。|
...
|defenderDisableCloudProtection|布尔值|为了最好地保护你的电脑，Windows Defender向 Microsoft 发送有关它所发现任何问题的信息。 Microsoft 将分析该信息，了解有关影响您和其他客户的问题的更多信息，并提供改进的解决方案。|
|defenderAllowCloudProtection|布尔值|为了最好地保护你的电脑，Windows Defender向 Microsoft 发送有关它所发现任何问题的信息。 Microsoft 将分析该信息，了解有关影响您和其他客户的问题的更多信息，并提供改进的解决方案。|
...
|defenderCloudBlockLevel|[defenderCloudBlockLevelType](../resources/intune-deviceconfig-defendercloudblockleveltype.md)|已添加到Windows 10版本 1709 中。 此策略设置确定Windows Defender 防病毒和扫描可疑文件时的主动性。 值类型为整数。 此功能需要启用"加入 Microsoft MAPS"设置才能运行。 可取值为：`notConfigured`、`high`、`highPlus`、`zeroTolerance`。|
...
|defenderDaysBeforeDeletingQuarantinedMalware|Int32|时间段 (以天) 隔离项目将存储在系统。 有效值为 0 至 90|
