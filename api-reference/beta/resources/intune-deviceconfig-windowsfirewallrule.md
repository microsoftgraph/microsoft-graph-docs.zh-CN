...
|属性|类型|说明|
|:---|:---|:---|
|displayName|字符串|规则的显示名称。 不需要是唯一的。|
|说明|字符串|规则的说明。|
|packageFamilyName|字符串|受防火墙规则影响的 Microsoft Store 应用程序的程序包系列名称。|
|路径|字符串|受防火墙规则影响的应用程序的完整文件路径。|
|serviceName|字符串|当服务（而不是应用程序）发送或接收通信时使用的名称。|
|协议|Int32|0-255 表示 IP 协议 (TCP = 6，UDP = 17) 的数字。 如果未指定，则默认值为 All。 有效值为0至255|
|localPortRanges|字符串集合|本地端口范围的列表。 例如，"100-120"，"200"，"300-320"。 如果未指定，则默认值为 All。|
|remotePortRanges|字符串集合|远程端口范围的列表。 例如，"100-120"，"200"，"300-320"。 如果未指定，则默认值为 All。|
|localAddressRanges|字符串集合|规则所涵盖的本地地址的列表。 默认值为任意地址。 有效令牌包括：<ul><li>"*" 表示任何本地地址。 如果存在此标记，则必须是包含的唯一标记。</li><li>可以使用子网掩码或网络前缀表示法指定子网。 如果不指定子网掩码和网络前缀，则子网掩码默认为255.255.255.255。</li><li>有效的 IPv6 地址。</li><li>不包含空格的 IPv4 地址范围，格式为 "起始地址-结束地址"。</li><li>不包含空格的 IPv6 地址范围，格式为 "起始地址-结束地址"。</li></ul>|
|remoteAddressRanges|字符串集合|指定规则所涵盖的远程地址的令牌列表。 标记不区分大小写。 默认值为任意地址。 有效令牌包括：<ul><li>"*" 表示任何远程地址。 如果存在此标记，则必须是包含的唯一标记。</li><li>"Defaultgateway"</li><li>LDHCP</li><li>DN</li><li>首选</li><li>Windows 版本1809上支持 "Intranet" (+) </li><li>Windows 版本 1809 (支持 "RmtIntranet") </li><li>Windows 版本1809和) 支持的 "Internet" (</li><li>Windows 版本 1809 (支持 "Ply2Renders") </li><li>"LocalSubnet" 指示本地子网上的任何本地地址。</li><li>可以使用子网掩码或网络前缀表示法指定子网。 如果不指定子网掩码和网络前缀，则子网掩码默认为255.255.255.255。</li><li>有效的 IPv6 地址。</li><li>不包含空格的 IPv4 地址范围，格式为 "起始地址-结束地址"。</li><li>不包含空格的 IPv6 地址范围，格式为 "起始地址-结束地址"。</li></ul>|
...
|localUserAuthorizations|字符串|指定应用程序容器的授权本地用户的列表。 这是安全描述符定义语言 (SDDL) 格式的字符串。|
