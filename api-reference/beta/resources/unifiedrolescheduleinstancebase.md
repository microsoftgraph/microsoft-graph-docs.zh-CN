...
|principal|[directoryObject](../resources/directoryobject.md)|通过请求获取角色分配主体。 启用检索主体，同时使用 `$expand` 获取角色分配。 只读。|
