...
|[Publish](../api/bookingbusiness-publish.md)|无|将此业务的计划页提供给外部的客户。 **IsPublished**属性设置为 true，并**publicUrl**属性设为计划页的 URL。|
...
|displayName|String|业务，接口与客户的名称。 此名称将显示在业务计划页的顶部。|
|email|字符串|业务电子邮件地址。|
|id|字串符号|业务唯一的编程标识符。 只读。|
|IsPublished|Boolean|计划页上进行了可对外部客户。 使用**发布**和**取消发布**操作来设置此属性。 只读。|
|phone|String|业务电话号码。 **电话**属性，以及**地址**和**webSiteUrl**，显示页脚中的企业计划页。|
|publicUrl|String|计划页，其中您[发布](../api/bookingbusiness-publish.md)或[取消公开](../api/bookingbusiness-unpublish.md)后页上设置的 URL。 只读。|
|schedulingPolicy|[bookingSchedulingPolicy](bookingschedulingpolicy.md)|指定如何创建此业务预订。|
|websiteUrl|String|业务网站的 URL。 **WebSiteUrl**属性，以及**地址**，**电话**，显示在业务计划页的页脚。|
