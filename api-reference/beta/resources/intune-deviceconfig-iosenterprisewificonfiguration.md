...
|disableMacAddressRandomization|Boolean|如果设置为 true，则强制使用此 Wi-Fi 配置文件连接的设备显示其实际的 mac Wi-Fi 地址，而不是随机 MAC 地址。 适用于 iOS 14 及更高版本。 继承自 [iosWiFiConfiguration](../resources/intune-deviceconfig-ioswificonfiguration.md)|
|preSharedKey|字符串|这是 WPA 个人共享网络的预共享Wi-Fi密钥。 继承自 [iosWiFiConfiguration](../resources/intune-deviceconfig-ioswificonfiguration.md)|
|eapType|[eapType](../resources/intune-deviceconfig-eaptype.md)|可扩展身份验证协议 (EAP) 。 指示在 Wi-Fi 终结点上设置的 EAP 协议 (路由器) 。 可取值为：`eapTls`、`leap`、`eapSim`、`eapTtls`、`peap`、`eapFast`。|
|eapFastConfiguration|[eapFastConfiguration](../resources/intune-deviceconfig-eapfastconfiguration.md)|EAP-FAST EAP-FAST选择"EAP 类型"时选择"EAP 配置选项"。 可能的值是：`noProtectedAccessCredential`、`useProtectedAccessCredential`、`useProtectedAccessCredentialAndProvision`、`useProtectedAccessCredentialAndProvisionAnonymously`。|
...
|outerIdentityPrivacyTemporaryValue|String|当 EAP (EAP 类型) TTLS、EAP 或 PEAP 时，启用标识隐私FAST外部标识。 此属性会屏蔽输入文本的用户名。 例如，如果使用"匿名"，则使用此连接进行身份验证的每个用户Wi-Fi真实用户名的"匿名"。|
