...
|ssid|字符串|这是广播到所有Wi-Fi网络的名称。 继承自 [iosWiFiConfiguration](../resources/intune-deviceconfig-ioswificonfiguration.md)|
...
|outerIdentityPrivacyTemporaryValue|String|将 EAP (EAP 类型) EAP - TTLS、EAP 或 PEAP 时，启用标识隐私FAST外部标识。 此属性会屏蔽输入文本的用户名。 例如，如果使用"匿名"，则使用此连接进行身份验证的Wi-Fi其真实用户名显示为"匿名"。|
...
|identityCertificateForClientAuthentication|[iosCertificateProfileBase](../resources/intune-deviceconfig-ioscertificateprofilebase.md)|将 EAP 类型配置为 EAP-TLS、EAP-TTLS (（具有证书身份验证) ）或 PEAP (证书身份验证时用于客户端身份验证的标识证书) 。|
