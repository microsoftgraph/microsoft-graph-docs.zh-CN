...
> **Microsoft Teams 和 Office 365 组支持组协作**。 大多数的 Office 365 组 API 都可与 Microsoft Teams 结合使用。 若要创建[团队](team.md)，首先要[创建组](../api/group-post-groups.md)，然后[向组添加团队](../api/team-put-teams.md)。 有关详细信息，请参阅 [Microsoft Teams 概述](teams-api-overview.md)。
...
|assignedLicenses|[assignedLicense](assignedlicense.md) 集合|分配给该组的许可证。 只读。|
...
|expirationDateTime|DateTimeOffset| 设置的组的过期时间戳。 值无法修改，并在组创建时自动填充。 时间戳类型表示采用 ISO 8601 格式的日期和时间信息，始终采用 UTC 时区。 例如，2014 年 1 月 1 日午夜 UTC 如下所示：`'2014-01-01T00:00:00Z'`。 只读。 |
|groupTypes|String collection| 指定要创建的组的类型。 可能的值是 `Unified`（创建 Office 365 组）或 `DynamicMembership`（创建动态组）。  对于其他所有组类型（如启用安全机制的组和启用电子邮件的安全组），请勿设置此属性。|
...
|licenseProcessingState|String|指示对所有组成员的组许可证分配的状态。 只读。 可能的值是：`QueuedForProcessing`、`ProcessingInProgress` 和 `ProcessingComplete`。|
...
|membershipRule|String|组为动态组时（groupTypes 包含 `DynamicMembership`），用于确定该组成员的规则。 有关成员身份规则语法的详细信息，请参阅[成员身份规则语法](https://azure.microsoft.com/zh-CN/documentation/articles/active-directory-accessmanagement-groups-with-advanced-rules/)|
|membershipRuleProcessingState|String|指示动态成员身份处理正在进行中，还是已暂停。 可能的值为：“正在进行”或“已暂停”。|
|onPremisesLastSyncDateTime|DateTimeOffset|指示对象最后一次与本地目录同步的时间。时间戳类型表示使用 ISO 8601 格式的日期和时间信息，并且始终采用 UTC 时区。 例如，2014 年 1 月 1 日午夜 UTC 如下所示：`'2014-01-01T00:00:00Z'`。 只读。 支持 $filter。|
...
|preferredLanguage|String|Office 365 组的首选语言。 应遵循 ISO 639-1 代码；例如“EN-US”。|
...
|theme|String|指定 Office 365 组的颜色主题。 可能的值为：`Teal`、`Purple`、`Green`、`Blue`、`Pink`、`Orange` 或 `Red`。|
|unseenConversationsCount|Int32|自登录用户上次访问该组以来已传递一个或多个新帖子的对话计数。 此属性与 **unseenCount** 相同。|
|unseenCount|Int32|自登录用户上次访问该组以来已传递一个或多个新帖子的对话计数。 此属性与 **unseenConversationsCount** 相同。|
|unseenMessagesCount|Int32|自登录用户上次访问该组以来已传递到组对话的新帖子计数。|
