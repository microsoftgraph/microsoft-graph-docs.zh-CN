...
| 属性     | 类型   |说明|
|:---------------|:--------|:----------|
|customerEmailAddress|String|预订约会[bookingCustomer](bookingcustomer.md) SMTP 地址。|
|customerId|String|该约会的[bookingCustomer](bookingcustomer.md)的 ID。 如果未指定 ID 创建约会时，将创建一个新的**bookingCustomer**对象。 设置后，您应考虑**customerId**变。|
...
|optOutOfCustomerEmail|Boolean|True 表示该约会的[bookingCustomer](bookingcustomer.md)不希望接收该约会的确认。|
...
|Price|双精度|指定[bookingService](bookingservice.md)约会正则价格。|
...
|服务 Id|String|与此约会相关联的[bookingService](bookingservice.md) ID。|
...
|serviceNotes|String|从[bookingStaffMember](bookingstaffmember.md)备注。 此属性的值时，可仅读取此**bookingAppointment**由其 id。|
