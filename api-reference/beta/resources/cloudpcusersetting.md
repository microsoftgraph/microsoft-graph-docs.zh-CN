...
|localAdminEnabled|布尔|指示是否已启用本地管理员选项。 默认值为 `false`。 若要启用本地管理员选项，将设置更改为 `true` 。 |
|selfServiceEnabled|布尔|指示是否已启用自助服务选项。 默认值为 `false`。 若要启用自助服务选项，将设置更改为 `true` 。 |
...
|关系|类型|说明|
|:---|:---|:---|
|assignments|[cloudPcUserSettingAssignment](../resources/cloudpcusersettingassignment.md) 集合|Office 365 Azure AD 中的安全组可以分配一组用户设置。|
