...
通过在此配置文件中提供配置，你可以指示 Windows 10 设备 (桌面或移动) 连接到所需的 VPN 终结点。 通过指定 VPN 终结点预期的身份验证方法和安全类型，可以使最终用户无缝进行 VPN 连接。
...
|方法|返回类型|说明|
|:---|:---|:---|
|[列出 windows10VpnConfigurations](../api/intune-deviceconfig-windows10vpnconfiguration-list.md)|[windows10VpnConfiguration](../resources/intune-deviceconfig-windows10vpnconfiguration.md) 集合|列出 [windows10VpnConfiguration 对象的属性和](../resources/intune-deviceconfig-windows10vpnconfiguration.md) 关系。|
...
|supportsScopeTags|布尔|指示基础设备配置是否支持分配范围标记。 当此值为 false 且实体对范围用户不可见时，不允许向 ScopeTags 属性赋值。 对于在 Silverlight 中创建的旧策略，会发生此情况，可通过在 Azure 门户中删除和重新创建策略来解决此问题。 此属性是只读的。 继承自 [deviceConfiguration](../resources/intune-shared-deviceconfiguration.md)|
...
|enableDnsRegistration|布尔|使用内部 DNS 启用 IP 地址注册。|
|dnsSuffixes|字符串集合|指定要添加到 DNS 搜索列表的 DNS 后缀，以正确路由短名称。|
...
|enableConditionalAccess|布尔|启用条件访问。|
|enableSingleSignOnWithAlternateCertificate|布尔|使用备用证书为 SSO (单一) 登录。|
...
|仅AssociatedAppsCanUseConnection|布尔|只有关联的应用可以使用每个应用 VPN (连接) 。|
|windowsInformationProtectionDomain|String|Windows 信息保护 (WIP) 域，以与此连接关联。|
...
|cryptographySuite|[cryptographySuite](../resources/intune-deviceconfig-cryptographysuite.md)|Windows 10 及以上版 IKEv2 VPN 的加密套件安全设置 |
...
|identityCertificate|[windowsCertificateProfileBase](../resources/intune-deviceconfig-windowscertificateprofilebase.md)|身份验证方法是证书时用于客户端身份验证的标识证书。|
