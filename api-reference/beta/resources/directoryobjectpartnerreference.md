...
|:---------------|:--------|:----------|
|说明|字符串| 返回的对象的说明。 只读。 |
|displayName|字符串| 目录对象返回，如组或应用程序的名称。 此为只读属性。 |
|externalPartnerTenantId|Guid| 合作伙伴租户的租户标识符。 此为只读属性。 |
|id|字符串| 资源的唯一标识符。 继承自 [directoryObject](directoryobject.md?view=graph-rest-beta)。 此为只读属性。 |
|objectType|String| 合作伙伴租户中引用的对象的类型。 此为只读属性。 |
