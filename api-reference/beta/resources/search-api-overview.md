...
一旦返回包含 [searchBucket](searchBucket.md) 对象集合的响应，就可以将搜索请求细化为只包含在一个 [searchBucket](searchBucket.md)中的匹配元素。这可以通过在后续 [searchRequest](./searchrequest.md) 的 **aggregationFilters** 属性中传回 **aggregationsFilterToken** 值来实现。
