...
| 方法           | 返回类型    |说明|
|:---------------|:--------|:----------|
|获取工作表 | [Worksheet](worksheet.md) |读取 worksheet 对象的属性和关系。|
|创建图表 |[Chart](chart.md)| 通过发布到图表集合创建新的图表。|
|列出名称 |[NamedItem](nameditem.md) 集合| 获取与工作表关联的命名项的集合。|
|列出图表 |图表 集合| 获取 Chart 对象集合。|
|创建表 |[Table](table.md)| 通过发布到表集合创建新表。|
|列出表 |[表](table.md) 集合| 获取 Table 对象集合。|
...
|List pivotTables |workbookPivotTable 集合| 获取一组 workbookPivotTable 对象。|
