...
|[列表](../api/governanceroleassignment-list.md) | [governanceRoleAssignment](../resources/governanceroleassignment.md) 集合|列出资源上的角色分配集合。 |
...
|roleDefinitionId|String|必需。 角色定义的 ID，角色分配角色定义。 |
|subjectId|String       |必需。 与用户关联的角色分配的 ID。 |
|linkedEligibleRoleAssignmentId|字符串|如果这是 和 由于 激活而创建的 ，则它表示 的 `active assignment` `eligible assignment` `eligible assignment` ID;否则，值为 `null` 。 |
...
|endDateTime|DateTimeOffset|对于非永久性角色分配，此时间将角色分配过期。 时间戳类型表示采用 ISO 8601 格式的日期和时间信息，始终采用 UTC 时区。 例如，2014 年 1 月 1 日午夜 UTC 为 `2014-01-01T00:00:00Z`|
