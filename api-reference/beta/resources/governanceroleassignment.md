...
| 方法          | 返回类型 |说明|
|:------------|:--------|:--------|
|[获取](../api/governanceroleassignment-get.md) |  [governanceRoleAssignment](../resources/governanceroleassignment.md) |读取实体的属性和角色分配关系。|
...
|subjectId|String       |必填。 与用户关联的角色分配的 ID。 |
...
|memberType|String      |成员的类型。 该值可以是： (如果 角色分配 继承自父资源范围) ，则 (如果 角色分配 不是继承的，但来自组分配) 的成员身份;如果 角色分配 既不继承也不从组分配) 继承 `Inherited` `Group` `User` (。|
