...
|pdfFitToPage|布尔|默认的 pdfFitToPage 设置。 如果为 True，则将 PDF 文档的每个页面放置到一个物理纸上;假以让打印机决定如何布置印象。|
|presentationDirection|printPresentationDirection|每个工作表打印多个页面时对页面进行布局的默认方向。 有效值如下表所述。|
|printColorConfiguration|printColorConfiguration|打印文档时使用的默认颜色模式。 有效值如下表所述。|
|printQuality|printQuality|打印文档时使用的默认质量。 有效值如下表所述。|
|duplexConfiguration|printDuplexConfiguration|打印文档时使用的默认双面打印（双面）配置。 有效值如下表所述。|
