...
ms.locfileid: "60695802"
---
# <a name="privilegedroleassignmentrequest-resource-type-deprecated"></a>privilegedRoleAssignmentRequest 资源类型 (已弃) 
...
表示 PIM 角色分配中Privileged Identity Management (操作) 。
...
|[Create](../api/privilegedroleassignmentrequest-post.md)|  [privilegedroleassignmentrequest](../resources/privilegedroleassignmentrequest.md)|创建管理现有或新用户生命周期角色分配。|
...
|assignmentState|字符串| 工作分配的状态。 该值可用于符合条件的分配 - 如果直接由管理员分配，或由用户对符合条件的分配 `Eligible` `Active` `Active` 进行激活。|
...
|status|String| 只读。请求角色分配状态。 值可以是 `NotStarted` `Completed` `RequestedApproval` `Scheduled` `Approved` `ApprovalDenied` 、、、、、、、、。 `ApprovalAborted` `Cancelling` `Cancelled` `Revoked` `RequestExpired`|
|ticketNumber|字符串| 角色分配 的 ticketNumber。 |
|ticketSystem|String| 用于此系统的 ticketSystem 角色分配。|
