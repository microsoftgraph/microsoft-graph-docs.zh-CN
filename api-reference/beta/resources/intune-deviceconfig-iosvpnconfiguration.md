...
|strictEnforcement|布尔|仅 Zscaler。 阻止网络流量，直到用户登录到 Zscaler 应用程序。 "True" 表示阻止流量。|
|cloudName|字符串|仅 Zscaler。 用户分配到的 Zscaler 云。|
|excludeList|字符串集合|仅 Zscaler。 不通过 Zscaler 云发送的网络地址的列表。|
|targetedMobileApps|[appListItem](../resources/intune-deviceconfig-applistitem.md) 集合|目标移动应用。 该集合最多可包含 500 个元素。|
|microsoftTunnelSiteId|字符串|Microsoft 隧道站点 ID。|
