...
|kioskModeManagedSettingsEntryDisabled|布尔值|是否在展台模式下在托管的主屏幕上显示托管设置入口点。|
|kioskModeDebugMenuEasyAccessEnabled|布尔值|是否允许用户在展台模式下轻松访问 "调试" 菜单。|
|kioskModeShowAppNotificationBadge|布尔值|是否在展台模式下显示应用程序通知徽章。|
