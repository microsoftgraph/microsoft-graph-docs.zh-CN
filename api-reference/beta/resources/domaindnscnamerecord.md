...
表示添加到租户中特定域的 DNS 区域文件的 CNAME 记录。 继承自 [DomainDnsRecord](domaindnsrecord.md) 实体。
...
|isOptional|布尔| 如果为 false，则客户必须在 DNS 主机上配置 CNAME 记录，Microsoft Online Services该域正常运行。 不可为 null |
|label|String| 在 DNS 主机上配置 CNAME 记录的 *别名/主机/* 名称时所使用的值。 |
