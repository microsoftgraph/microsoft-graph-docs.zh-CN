...
|clientAppUsed|字符串|用于登录活动的旧客户端。 例如，浏览器、Exchange Active Sync、新式客户端、IMAP、MAPI、SMTP 或 POP。|
...
|originalRequestId|字符串|身份验证序列中的第一个请求的请求标识符。|
...
|tokenIssuerName|字符串|标识提供程序的名称。 例如，`sts.microsoft.com`。|
