...
|[删除 macOSLobApp](../api/intune-apps-macoslobapp-delete.md)|None|删除 [macOSLobApp](../resources/intune-apps-macoslobapp.md)。|
...
|buildNumber|String| (LoB) 应用的 MacOS 业务线的内部版本号。|
...
|ignoreVersionDetection|Boolean|控制应用的版本是否将用于检测安装在设备上的应用的布尔值。 将 macOS 业务线 (LoB) 使用自更新功能的应用设置为 true。|
|installAsManaged|布尔|一个布尔值，用于控制应用是否将安装为托管 (需要 macOS 11.0 和其他 PKG 限制) 。|
