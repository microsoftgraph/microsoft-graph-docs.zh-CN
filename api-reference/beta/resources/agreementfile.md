...
| 属性     | 类型        | Description |
|:-------------|:------------|:------------|
|fileData|[agreementFileData](agreementfiledata.md)|表示使用 PDF 文档中的条款的数据。 此为只读属性。|
|fileName|String|协议文件 (例如，TOU.pdf) 的名称。 此为只读属性。|
|id|String|只读。|
|isDefault|布尔|指示是否这是默认协议文件，是否无区域性匹配的客户端首选项。 如果没有任何文件被标记为默认值，第一个将被视为默认值。 此为只读属性。|
