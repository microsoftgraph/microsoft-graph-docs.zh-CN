...
|[更新](../api/range-update.md) | [range](range.md)   |更新 Range 对象。 |
|[Boundingrect](../api/range-boundingrect.md)|[range](range.md)|获取包含指定区域的最小 range 对象。例如，“B2:C5”和“D10:E15”的 GetBoundingRect 为“B2:E16”。|
|[单元格](../api/range-cell.md)|[range](range.md)|根据行和列编号获取包含单个单元格的 range 对象。单元格可以位于父区域外部，只要其保持在工作表网格内即可。返回的单元格位于相对于区域左上角的单元格的位置。|
...
|[Entirecolumn](../api/range-entirecolumn.md)|[range](range.md)|获取表示区域整列的对象。|
|[Entirerow](../api/range-entirerow.md)|[range](range.md)|获取表示区域整行的对象。|
|[Intersection](../api/range-intersection.md)|[range](range.md)|获取表示指定区域的矩形交集的 range 对象。|
...
|[Usedrange](../api/range-usedrange.md)|[range](range.md)|返回指定 range 对象的所用区域。|
|[清除](../api/range-clear.md)|None|清除区域值、格式、填充、边框等。|
|[删除](../api/range-delete.md)|None|删除与区域相关的单元格。|
|[插入](../api/range-insert.md)|[range](range.md)|将单个单元格或一系列单元格插入到工作表中取代此区域，并移动其他单元格以留出空间。在现在空白的空间返回新的 Range 对象。|
