...
description: 包含 Office365 套件应用的属性和继承属性。
author: dougeby
...
|[删除 officeSuiteApp](../api/intune-apps-officesuiteapp-delete.md)|None|删除 [officeSuiteApp](../resources/intune-apps-officesuiteapp.md)。|
...
|autoAcceptEula|Boolean|在终结器设备上自动接受 EULA 的值。|
...
|useSharedComputerActivation|Boolean|表示是否将共享计算机激活用于 Office365 应用套件的属性。|
|updateChannel|[officeUpdateChannel](../resources/intune-apps-officeupdatechannel.md)|表示 Office365 更新通道的属性。 可取值为：`none`、`current`、`deferred`、`firstReleaseCurrent`、`firstReleaseDeferred`、`monthlyEnterprise`。|
|officePlatformArchitecture|[windowsArchitecture](../resources/intune-apps-windowsarchitecture.md)|表示 Office365 应用套件版本的属性。 可取值为：`none`、`x86`、`x64`、`arm`、`neutral`、`arm64`。|
|localesToInstall|字符串集合|表示安装 Office365 应用时安装的区域设置的属性。 它使用标准 RFC 6033。 裁判： https://technet.microsoft.com/library/cc179219(v=office.16).aspx|
...
|shouldUninstallOlderVersionsOfOffice|Boolean|用于确定是否将 Office365 应用套件部署到设备时卸载现有 Office MSI 的属性。|
|targetVersion|String|表示应保留在设备上部署的 Office365 应用套件的特定目标版本的属性。|
|updateVersion|String|表示适用于 Office365 应用套件的特定目标版本的更新版本的属性。|
|officeConfigurationXml|Binary|表示可为 Office ProPlus 应用指定的 XML 配置文件的属性。 优先于所有其他属性。 如果存在，XML 配置文件将用于创建应用。|
