...
| 属性     | 类型        | 说明 |
|:-------------|:------------|:------------|
|contentTypes|String collection|支持的内容 (MIME) 类型的列表，打印机支持这些类型。 不能保证通用打印服务支持打印所有这些 MIME 类型。|
|isColorPrintingSupported|布尔|如果打印机支持彩色打印，则为 True;否则为 false。 只读。|
|feedOrientations|printerFeedOrientation 集合|打印机支持的源方向的列表。|
|isPageRangeSupported|布尔|如果打印机支持按页面范围打印，则为 True; 否则为 false。否则为 false。|
...
|collation|布尔|如此如果打印机支持在打印多页文档的 muliple 副本时进行排序;否则为 false。|
