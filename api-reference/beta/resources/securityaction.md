...
|azureTenantId|字符串|实体的 Azure 租户 ID，用于确定实体属于多租户 (租户支持) 。 azureTenantId 应从身份验证令牌中提取，而不是由调用应用程序手动输入。|
...
|name|String| 操作名称。|
|parameters|[keyValuePair](keyvaluepair.md) 集合| 参数集合 (键值对) 调用操作所必需的，例如 URL 或 fileHash to block.) 。 必需。|
...
|用户|String| 在 POST 中提交的已登录用户的用户主体 () 操作。 用户应从身份验证令牌中提取，而不是由调用应用程序手动输入。|
