...
|domainName|String|已加入设备的混合设备Azure AD域名。 此属性由 Intune 设置。|
...
|isManaged|Boolean|`true` 如果设备由移动设备管理中心管理， (MDM) 应用;否则为 `false` 。 这仅可通过 Intune 针对任何设备操作系统类型进行更新，或由适用于 Windows 操作系统设备的已批准[MDM](/windows/client-management/mdm/azure-active-directory-integration-with-mdm)应用更新。 支持 `$filter`（`eq`、`ne`、`not`）。 |
...
|operatingSystemVersion|String| 设备的操作系统版本。 必填。 支持 `$filter` `eq` `ne` `not` (、、、、 和 `ge` `le` `startsWith` `eq` 值 `null`) 。 |
...
|name| String | 设备的友好名称。 仅在用户使用 Microsoft 帐户登录作为 Project 的一部分时返回。 |
|状态 | String| 设备是 `online` `offline` 或 。 仅在用户使用 Microsoft 帐户登录作为 Project 的一部分时返回。 |
|平台 |String|设备平台。 仅在用户使用 Microsoft 帐户登录作为 Project 的一部分时返回。 仅在用户使用 Microsoft 帐户登录作为 Project 的一部分时返回。|
|kind| String| 设备的外形要求。 仅在用户使用 Microsoft 帐户登录作为 Project 的一部分时返回。 |
|model| String| 设备型号。 仅在用户使用 Microsoft 帐户登录作为 Project 的一部分时返回。 |
|manufacturer| String| 设备制造商。 仅在用户使用 Microsoft 帐户登录作为 Project 的一部分时返回。 |
