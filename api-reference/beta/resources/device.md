...
|[List memberOf](../api/device-list-memberof.md) |[directoryObject](directoryobject.md) 集合| 列出设备直接成员所参与的组和管理单元。 |
|[列出 transitive memberOf](../api/device-list-transitivememberof.md) |[directoryObject](directoryobject.md) collection| 列出设备是其中一个成员的组和管理单元。 此操作是可传递的。 |
...
| [getMemberGroups](../api/directoryobject-getmembergroups.md) | String 集合 | 返回设备是成员的所有组。 检查是可传递的。 |
|[checkMemberObjects](../api/directoryobject-checkmemberobjects.md) | String 集合 | 检查组、目录角色或管理单元对象列表中的成员身份。 |
|[getMemberObjects](../api/directoryobject-checkmemberobjects.md) | String 集合 | 返回设备是成员的所有组、管理单元和目录角色。 检查是可传递的。 |
|**开放扩展**| | |
...
|approximateLastSignInDateTime|DateTimeOffset| 时间戳类型表示使用 ISO 8601 格式的日期和时间信息，并且始终采用 UTC 时间。 例如，2014 年 1 月 1 日午夜 UTC 为 `2014-01-01T00:00:00Z`。 只读。 支持`$filter` (`eq`、`ne`、`not`、`ge`、 `le``eq` 和 的值`null`) 和 `$orderBy`。 |
...
|deviceMetadata|String| 仅供内部使用。 `null`设置为 。 |
|deviceOwnership|字符串|设备的所有权。 此属性由 Intune 设置。 可取值为：`unknown`、`company`、`personal`。|
...
|onPremisesLastSyncDateTime|DateTimeOffset|最后一次将对象与本地目录同步的时间。 时间戳类型表示采用 ISO 8601 格式的日期和时间信息，始终采用 UTC 时区。 例如，2014 年 1 月 1 日午夜 UTC 为 `2014-01-01T00:00:00Z` 只读。 支持 `$filter` （`eq`、 `ne`、 `not`、 `ge`、 `le`、 `in`）。 |
...
|operatingSystem|String| 设备上操作系统的类型。 必需。 支持`$filter` (`eq`、`ne`、`not`、`ge`、 `eq` `le``startsWith`和 值`null`) 。 |
|operatingSystemVersion|String| 设备的操作系统版本。 必需项。 支持`$filter` (`eq`、`ne`、`not`、`ge`、 `eq` `le``startsWith`和 值`null`) 。 |
|physicalIds|String collection| 仅供内部使用。 不可为 null。 支持 `$filter` （`eq`、 `not`、 `ge`、 `le`、 `startsWith`）。 |
|profileType|字符串|设备的配置文件类型。 可能的值： (`RegisteredDevice` 默认值) 、、`SecureVM`、`Printer`、`Shared``IoT`。|
...
|trustType|String| 加入设备的信任类型。 只读。 可能的值： `Workplace` ( `AzureAd` 自带的个人设备) 、 (仅加入云的设备) `ServerAd` 、 (加入 Azure AD) 的加入本地域的设备。 如需了解更多详情，请参阅 [Azure Active Directory 中的设备管理简介](/azure/active-directory/device-management-introduction) |
|name| String | 设备的友好名称。 仅在用户使用 Microsoft 帐户登录作为 Rome 的一Project返回。 |
|状态 | String| 设备是 `online` 或 `offline`。 仅在用户使用 Microsoft 帐户登录作为 Rome 的一Project返回。 |
|平台 |String|设备平台。 仅在用户使用 Microsoft 帐户登录作为 Rome 的一Project返回。 仅在用户使用 Microsoft 帐户登录作为 Rome 的一Project返回。|
|kind| 字符串| 设备的外形要求。 仅在用户使用 Microsoft 帐户登录作为 Rome 的一Project返回。 |
...
|memberOf|[directoryObject](directoryobject.md) collection|此设备是其中一个成员的组和管理单元。 只读。 可为 NULL。 支持 `$expand`。 |
...
|transitiveMemberOf |[directoryObject](directoryobject.md) collection| 此设备是其中一个成员的组和管理单元。 此操作是可传递的。 支持 `$expand`。  |
