...
|enrollmentProfileName|String|应用到设备的注册配置文件。 例如，`Apple Device Enrollment Profile``Device enrollment - Corporate device identifiers`或 `Windows Autopilot profile name`. 此属性由Intune设置。|
...
|id|字符串|设备唯一标识符。 继承自 [directoryObject](directoryobject.md)。 密钥，不可为 NULL。 只读。 支持 `$filter` （`eq`、 `ne`、 `not`、 `in`）。 |
