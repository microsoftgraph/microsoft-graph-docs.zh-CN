...
|supports|string| 同步更改通知支持的 Shifts 实体。 Shifts 将调用此处添加的实体上的客户端更改时提供的 URL。 默认情况下，更改通知不支持任何实体。 可能的值是： `none` `shift` ， ， ， `swapRequest` `openshift` ， `openShiftRequest``userShiftPreferences`|
