...
|名称 | 类型 | 说明|
|:----|:-----|:-----------|
|userTimezone | String | 可选。 在其中用于生成活动的用户的设备已位于在活动创建时; timezone为了支持跨平台表示形式作为 Olson Id 提供的值。|
...
|appDisplayName | String | 可选。 用于生成使用活动情况下，用户的本地设备上未安装应用程序时应用程序的简短的文本说明。|
|activationUrl | String | 必需。 用于启动最佳本机体验由 appId 中的活动 URL。 如果没有的本机应用程序存在，则可能会启动一个基于 web 的应用程序。|
|fallbackUrl | String | 可选。 用于启动基于 web 的应用程序中的活动如果可用的 URL。|
|contentUrl | String | 可选。 使用在事件的内容可以呈现之外的本机或基于 web 的应用程序体验 （例如，RSS 源中的项目的指针）。|
