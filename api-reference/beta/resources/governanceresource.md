...
|status             |字符串     |给定资源的状态。 例如，它可能表示是否资源被锁定或未 (值： `Active` / `Locked`)。 注意： 该属性可能进行扩展，以便将来以支持更多方案。|
...
|父          |[governanceResource](../resources/governanceresource.md)           |此为只读属性。 父资源。 为`pimforazurerbac`方案中，它可以表示资源所属的订阅。|
