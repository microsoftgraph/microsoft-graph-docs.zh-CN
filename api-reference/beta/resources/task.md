...
表示可以跟踪和完成的任务，如一部分工作或个人项目。 任务 **始终** 包含在任务 [列表中](basetasklist.md)。 
...
|属性|类型|说明|
|:---|:---|:---|
|body|[itemBody](../resources/itembody.md)|通常包含有关任务的信息的任务正文。 继承自 [baseTask](../resources/basetask.md)。|
...
|personalProperties|[personalTaskProperties](../resources/personaltaskproperties.md)|用户个人的属性，如 reminderDateTime。 继承自 [baseTask](../resources/basetask.md)。|
...
|状态|taskStatus_v2|指示任务的状态或进度。 可能的值是 `notStarted` `inProgress` `completed` ：、、、。 `unknownFutureValue` 继承自 [baseTask](../resources/basetask.md)。|
