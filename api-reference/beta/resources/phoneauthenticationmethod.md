...
电话具有以下三种类型之一：移动、备用移动或办公室。 用户可以为每种类型注册一个号码，并且在添加备用移动电话之前，必须有一个移动电话。 在使用电话进行多重身份验证（MFA）或自助密码重置（SSPR）时，移动电话是默认设置，备用移动电话是备份。 
...
|phoneType|string|此电话的类型。 可能的值包括`mobile`： `alternateMobile`、或`office`。|
|smsSignInState|string|电话是否已准备好用于短信登录。 可能的值包括`notSupported`： `notAllowedByPolicy`、 `notEnabled`、 `phoneNumberNotUnique` `ready`、、或`notConfigured`。|
