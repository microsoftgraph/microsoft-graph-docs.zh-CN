...
|securityRequireIntuneAppIntegrity|Boolean|如果设置为 true，则检查在完全托管、专用或企业拥有的工作配置文件 Android Enterprise 注册设备上安装的 Intune 应用是否是由 Microsoft 从托管 Google Playstore提供的。 如果检查失败，设备将报告为不兼容。|
