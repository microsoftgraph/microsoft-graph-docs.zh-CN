...
|homeScreenGridWidth|Int32|获取或设置配置 iOS 主屏幕布局设置时要呈现的列数。 如果配置了此值，则还必须配置 homeScreenGridHeight。|
|homeScreenGridHeight|Int32|获取或设置配置 iOS 主屏幕布局设置时要呈现的行数。 如果配置了此值，则还必须配置 homeScreenGridWidth。|
...
|singleSignOnSettings|[iosSingleSignOnSettings](../resources/intune-deviceconfig-iossinglesignonsettings.md)|使接收设备上的应用能够顺利进行身份验证的 Kerberos 登录设置。|
|wallpaperDisplayLocation|[iosWallpaperDisplayLocation](../resources/intune-deviceconfig-ioswallpaperdisplaylocation.md)|壁纸显示位置说明符。 可能的值是：`notConfigured`、`lockScreen`、`homeScreen`、`lockAndHomeScreens`。|
|wallpaperImage|[mimeContent](../resources/intune-shared-mimecontent.md)|壁纸映像必须采用 PNG 或 JPEG 格式。 它需要具有 iOS 8 或更高版本的监督设备。|
|singleSignOnExtension|[singleSignOnExtension](../resources/intune-deviceconfig-singlesignonextension.md)|获取或设置单一登录扩展配置文件。 已弃用：改用 IOSSingleSignOnExtension。|
...
|singleSignOnExtensionPkinitCertificate|[iosCertificateProfileBase](../resources/intune-deviceconfig-ioscertificateprofilebase.md)|使用单一登录扩展设置进行身份验证的 PKINIT 证书。|
