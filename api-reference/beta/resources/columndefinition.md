...
| **enforceUniqueValues** | 布尔 | 如果 `true` 为 ，则没有两个列表项对此列具有相同的值。
...
| **propagateChanges**     | 布尔 | 如果 `true` 为 ，则对此列所做的更改将传播到实现该列的列表。 
...
>**注意：** 这些属性对应于 [SPFieldType SharePoint。][]
请注意，最常见的字段类型在上表中表示。 但是，此 beta API 仍缺少一些。
