...
|[extendFeatureUpdatesPause 操作](../api/intune-deviceconfig-windowsupdateforbusinessconfiguration-extendfeatureupdatespause.md)|None|扩展功能更新暂停业务圈Windows 更新。|
...
|skipChecksBeforeRestart|布尔|设置为在重启前跳过所有检查：电池级别 = 40%，用户状态、所需显示、演示模式、全屏模式、电话呼叫状态、游戏模式等。 |
...
|featureUpdatesRollbackStartDateTime|DateTimeOffset|更新回滚开始日期时间的功能|
...
|postponeRebootUntilAfterDeadline|Boolean|指定设备是否应等到在活动时间之外重新启动的截止时间|
...
|userPauseAccess|[支持](../resources/intune-deviceconfig-enablement.md)|指定是否启用最终用户访问暂停软件更新。 可取值为：`notConfigured`、`enabled`、`disabled`。|
|userWindowsUpdateScanAccess|[支持](../resources/intune-deviceconfig-enablement.md)|指定是否禁用用户访问扫描Windows 更新。 可取值为：`notConfigured`、`enabled`、`disabled`。|
...
|allowWindows11Upgrade|布尔|允许符合条件的Windows 10设备升级到最新版本的Windows 11。|
...
|deviceUpdateStates|[windowsUpdateState](../resources/intune-deviceconfig-windowsupdatestate.md) 集合|适用于业务配置设备的 Windows 更新状态。 该集合最多可包含 500 个元素。|
