...
通过提供此配置文件中的配置，你可以指示 iOS 设备连接到所需的 IKEv2 VPN 终结点。
...
|[更新 iosikEv2VpnConfiguration](../api/intune-deviceconfig-iosikev2vpnconfiguration-update.md)|[iosikEv2VpnConfiguration](../resources/intune-deviceconfig-iosikev2vpnconfiguration.md)|更新 [iosikEv2VpnConfiguration 对象](../resources/intune-deviceconfig-iosikev2vpnconfiguration.md) 的属性。|
...
|userDomain|String|仅 Zscaler。 输入静态域以在 Zscaler 应用中预填充登录字段。 如果此域留空，则改为使用用户的 Azure Active Directory 域。 继承自 [iosVpnConfiguration](../resources/intune-deviceconfig-iosvpnconfiguration.md)|
|strictEnforcement|布尔|仅 Zscaler。 阻止网络流量，直到用户登录 Zscaler 应用。 "True"表示流量被阻止。 继承自 [iosVpnConfiguration](../resources/intune-deviceconfig-iosvpnconfiguration.md)|
|cloudName|String|仅 Zscaler。 用户分配到的 Zscaler 云。 继承自 [iosVpnConfiguration](../resources/intune-deviceconfig-iosvpnconfiguration.md)|
|excludeList|字符串集合|仅 Zscaler。 未通过 Zscaler 云发送的网络地址列表。 继承自 [iosVpnConfiguration](../resources/intune-deviceconfig-iosvpnconfiguration.md)|
...
|deadPeerDetectionRate|[vpnDeadPeerDetectionRate](../resources/intune-deviceconfig-vpndeadpeerdetectionrate.md)|确定检查对等连接是否仍处于活动状态的频繁。 . 可取值为：`medium`、`none`、`low`、`high`。|
|disableMobilityAndMultihoming|布尔|禁用 MOBIKE|
|disableRedirect|布尔|禁用重定向|
|enableCertificateRevocationCheck|布尔|启用最佳吊销检查;服务器响应超时不会导致失败|
|enableEAP|布尔|启用仅 EAP 身份验证|
|enablePerfectForwardSecrecy|布尔|启用 PFS (完全) 。|
...
|serverCertificateIssuerCommonName|String|身份验证中使用的 IKEv2 服务器证书颁发者公用名|
|serverCertificateType|[vpnServerCertificateType](../resources/intune-deviceconfig-vpnservercertificatetype.md)|VPN 服务器将呈现给 VPN 客户端进行身份验证的证书类型。 可取值为：`rsa`、`ecdsa256`、`ecdsa384`、`ecdsa521`。|
|sharedSecret|String|在选择共享密码身份验证时使用|
|tlsMaximumVersion|String|要与 EAP-TLS 身份验证一起使用的最大 TLS 版本|
|tlsMinimumVersion|String|要与 EAP-TLS 身份验证一起使用的最低 TLS 版本|
|allowDefaultSecurityAssociationParameters|布尔|允许使用安全关联参数，除非明确指定，否则所有参数都设置为设备默认值。|
|allowDefaultChildSecurityAssociationParameters|布尔|允许使用子安全关联参数，除非明确指定，否则所有参数都设置为设备默认值。|
...
|enableAlwaysOnConfiguration|布尔|确定是否启用始终启用 VPN|
