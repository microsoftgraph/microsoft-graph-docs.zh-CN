...
|energySaverOnBatteryThresholdPercentage|Int32|此设置允许你指定节电模式打开的电池充电级别。 使用电池时，在达到指定的电池充电 (低于) 自动打开节电模式。 有效输入范围 (0-100) 。 有效值为 0 至 100|
...
|microsoftAccountSignInAssistantSettings|[signInAssistantOptions](../resources/intune-deviceconfig-signinassistantoptions.md)|控制 wlidsvc Sign-In NT (的 Microsoft) 助理。 可取值为：`notConfigured`、`disabled`。|
...
|displayAppListWithGdiDPIScalingTurnedOn|字符串集合|已打开 GDI DPI 缩放的旧应用程序列表。|
|displayAppListWithGdiDPIScalingTurnedOff|字符串集合|已关闭 GDI DPI 缩放的旧应用程序列表。|
...
|printerNames|字符串集合|根据打印机的名称自动预配 (网络主机) 。|
...
|securityBlockAzureADJoinedDevicesAutoEncryption|Boolean|指定当设备已加入 Azure AD 时，是否允许在 OOBE 期间自动进行设备加密 (桌面版) 。|
...
|systemTelemetryProxyServer|String|获取或设置代理服务器的 FQDN (或 IP 地址的完全限定域名) 或 IP 地址，以转发连接用户体验和遥测请求。|
|edgeTelemetryForMicrosoft365Analytics|[edgeTelemetryMode](../resources/intune-deviceconfig-edgetelemetrymode.md)|指定向 Microsoft 365 Analytics 发送的遥测 (、Intranet、internet 和) 类型。 可取值为：`notConfigured`、`intranet`、`internet`、`intranetAndInternet`。|
...
|edgeBlockTabPreloading|Boolean|配置 Edge 在启动时是否预加载Windows页。|
|edgeBlockPrelaunch|Boolean|决定是否Microsoft Edge启动时预启动Windows启动。|
...
|defenderPotentiallyUnwantedAppAction|[defenderPotentiallyUnwantedAppAction](../resources/intune-deviceconfig-defenderpotentiallyunwantedappaction.md)|获取或设置 Defender 对可能不需要的应用程序 (PUA) 的操作，其中包括具有广告注入行为、软件捆绑、持续付款或订阅费用等的软件。Defender 在下载 PUA 或尝试自行安装时向用户发出警报。 在桌面Windows 10中添加。 可取值为：`deviceDefault`、`block`、`audit`。|
|defenderPotentiallyUnwantedAppActionSetting|[defenderProtectionType](../resources/intune-deviceconfig-defenderprotectiontype.md)|获取或设置 Defender 对可能不需要的应用程序 (PUA) 的操作，其中包括具有广告注入行为、软件捆绑、持续付款或订阅费用等的软件。Defender 在下载 PUA 或尝试自行安装时向用户发出警报。 在桌面Windows 10中添加。 可取值为：`userDefined`、`enable`、`auditMode`、`warn`、`notConfigured`。|
...
|dataProtectionBlockDirectMemoryAccess|Boolean|此策略设置允许你阻止 DMA (DMA) 所有可插入 PCI 下游端口，直到用户登录到 Windows。|
|appManagementPackageFamilyNamesToLaunchAfterLogOn|字符串集合|以分号分隔的程序包系列名称的列表Windows应用。 列出的Windows应用将在登录后启动。|
