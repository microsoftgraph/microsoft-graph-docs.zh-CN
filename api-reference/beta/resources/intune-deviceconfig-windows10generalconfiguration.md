...
|assignedAccessMultiModeProfiles|[windowsAssignedAccessProfile](../resources/intune-deviceconfig-windowsassignedaccessprofile.md)集合|此策略设置允许定义包含多台应用程序模式已分配的访问配置文件的列表。|
|privacyAccessControls|[windowsPrivacyDataAccessControlItem](../resources/intune-deviceconfig-windowsprivacydataaccesscontrolitem.md)集合|指示应用程序与隐私数据类别，通过其访问控制级别和/或每个类别的默认访问级别的列表。|
