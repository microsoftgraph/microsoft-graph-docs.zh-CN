...
|方法|返回类型|说明|
|:---|:---|:---|
|[列出 windows10GeneralConfigurations](../api/intune-deviceconfig-windows10generalconfiguration-list.md)|[windows10GeneralConfiguration](../resources/intune-deviceconfig-windows10generalconfiguration.md) 集合|列出 [windows10GeneralConfiguration](../resources/intune-deviceconfig-windows10generalconfiguration.md) 对象的属性和关系。|
|[获取 windows10GeneralConfiguration](../api/intune-deviceconfig-windows10generalconfiguration-get.md)|[windows10GeneralConfiguration](../resources/intune-deviceconfig-windows10generalconfiguration.md)|读取 [windows10GeneralConfiguration](../resources/intune-deviceconfig-windows10generalconfiguration.md) 对象的属性和关系。|
|[创建 windows10GeneralConfiguration](../api/intune-deviceconfig-windows10generalconfiguration-create.md)|[windows10GeneralConfiguration](../resources/intune-deviceconfig-windows10generalconfiguration.md)|创建新的 [windows10GeneralConfiguration](../resources/intune-deviceconfig-windows10generalconfiguration.md) 对象。|
|[删除 windows10GeneralConfiguration](../api/intune-deviceconfig-windows10generalconfiguration-delete.md)|无|删除 [windows10GeneralConfiguration](../resources/intune-deviceconfig-windows10generalconfiguration.md)。|
|[更新 windows10GeneralConfiguration](../api/intune-deviceconfig-windows10generalconfiguration-update.md)|[windows10GeneralConfiguration](../resources/intune-deviceconfig-windows10generalconfiguration.md)|更新 [windows10GeneralConfiguration](../resources/intune-deviceconfig-windows10generalconfiguration.md) 对象的属性。|
...
|enableAutomaticRedeployment|布尔值|允许具有管理权限的用户在设备锁屏的屏幕上使用 CTRL + Win + R 删除所有用户数据和设置, 以便可以自动重新配置设备并将其重新注册到管理中。|
...
|authenticationAllowSecondaryDevice|布尔值|允许辅助身份验证设备使用 Windows。|
...
|cryptographyAllowFipsAlgorithmPolicy|布尔值|指定是否允许或禁止联邦信息处理标准 (FIPS) 策略。|
...
|messagingBlockSync|布尔值|指示是否在所有位置阻止短信备份和还原和消息传递。|
|messagingBlockMMS|布尔值|指示是否阻止设备上的 MMS 发送/接收功能。|
|messagingBlockRichCommunicationServices|布尔值|指示是否阻止设备上的 RCS 发送/接收功能。|
|printerNames|String 集合|根据打印机的名称 (网络主机名称) 自动预配打印机。|
|printerDefaultName|String|已安装的打印机的名称 (网络主机名称)。|
|printerBlockAddition|布尔值|阻止用户安装来自打印机设置的其他打印机。|
...
|searchDisableUseLocation|布尔值|指定搜索是否可以使用位置信息。|
|searchDisableLocation|布尔值|指定搜索是否可以使用位置信息。|
...
|securityBlockAzureADJoinedDevicesAutoEncryption|布尔值|在设备已加入 Azure AD 时, 指定是否允许在 OOBE 期间自动设备加密 (仅限桌面)。|
...
|inkWorkspaceBlockSuggestedApps|布尔值|指定是否在墨迹工作区中显示建议的应用建议。|
...
|bluetoothBlockPromptedProximalConnections|布尔值|是否阻止用户使用 Swift 对和其他基于邻近的应用场景。|
...
|edgeBlockPrinting|布尔值|将 Edge 配置为允许或阻止打印。|
...
|edgeBlockSavingHistory|布尔值|将 Edge 配置为允许保存浏览历史记录或从不保存浏览历史记录。|
|edgeBlockFullScreenMode|布尔值|允许或阻止边缘进入全屏模式。|
|edgeBlockWebContentOnNewTabPage|布尔值|将配置为在 Edge 中加载空白页面, 而不是默认的新选项卡页面, 并防止用户对其进行更改。|
|edgeBlockTabPreloading|布尔值|配置边缘是否在 Windows 启动时预加载新的选项卡页。|
|edgeBlockPrelaunch|布尔值|确定 Microsoft Edge 在 Windows 启动时是否为 prelaunched。|
...
|edgePreventCertificateErrorOverride|布尔值|允许或阻止用户覆盖证书错误。|
...
|cellularBlockVpnWhenRoaming|布尔值|通过手机网络漫游时是否阻止用户使用 VPN。|
...
|defenderScheduleScanEnableLowCpuPriority|布尔值|启用后, 在计划扫描期间将使用较低的 CPU 优先级。|
|defenderDisableCatchupQuickScan|布尔值|当被阻止时, 将关闭计划快速扫描的追赶扫描。|
|defenderDisableCatchupFullScan|布尔值|当被阻止时, 计划的完全扫描的追赶扫描将被禁用。|
...
|defenderBlockOnAccessProtection|布尔值|允许或禁止 Windows Defender 访问保护功能。|
...
|privacyDisableLaunchExperience|布尔值|此策略可防止在用户登录时为新的和已升级的用户启动隐私体验。|
|privacyBlockInputPersonalization|布尔值|指示是否阻止 Cortana、Dictation 或 Store 应用程序使用基于云的语音服务。|
|privacyBlockPublishUserActivities|布尔值|阻止共享体验和发现任务切换器等中的最近使用的资源。|
|privacyBlockActivityFeed|布尔值|阻止 Cortana、听写或存储应用程序的基于云的语音服务的使用。|
...
|settingsBlockEaseOfAccessPage|布尔值|指示是否阻止在“设置”应用中访问“轻松使用”。|
...
|edgeBlockSearchEngineCustomization|布尔值|指示是否阻止用户添加新的搜索引擎或更改默认搜索引擎。|
|edgeBlockSendingIntranetTrafficToInternetExplorer|布尔值|指示是否将 intranet 流量从边缘切换到 Internet Explorer。 注意: 此属性的名称是误导性的;属性已过时, 请改用 EdgeSendIntranetTrafficToInternetExplorer。|
|edgeSendIntranetTrafficToInternetExplorer|布尔值|指示是否将 intranet 流量从边缘切换到 Internet Explorer。|
...
|smartScreenBlockPromptOverride|布尔值|指示用户是否可以替代有关潜在恶意网站的 SmartScreen 筛选器警告。|
...
|windowsStoreEnablePrivateStoreOnly|布尔值|指示是否启用“仅限专用应用商店”。|
...
|appManagementMSIAllowUserControlOverInstall|布尔值|此策略设置允许用户更改通常仅供系统管理员使用的安装选项。|
|appManagementMSIAlwaysInstallWithElevatedPrivileges|布尔值|此策略设置指示 Windows Installer 在系统上安装任何程序时使用提升的权限。|
|dataProtectionBlockDirectMemoryAccess|布尔值|通过此策略设置, 您可以阻止所有热插拔 PCI 下游端口的直接内存访问 (DMA), 直到用户登录 Windows。|
...
|privacyAccessControls|[windowsPrivacyDataAccessControlItem](../resources/intune-deviceconfig-windowsprivacydataaccesscontrolitem.md)集合|指示具有高于隐私数据类别的访问控制级别和/或每个类别的默认访问级别的应用程序的列表。|
