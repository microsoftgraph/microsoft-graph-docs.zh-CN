...
| 方法   | 返回类型|说明|
|:---------------|:--------|:----------|
|[获取警报](../api/alert-get.md) | [Alert](alert.md) |读取属性和警报对象的关系。|
|[更新警报](../api/alert-update.md) | [Alert](alert.md) |更新警报对象。 |
...
|id |字串符号|提供程序生成的 GUID/唯一标识符。 只读。 必需。|
...
|Severity |alertSeverity|警报严重性-供应商/服务提供商设置。 可取值为：`unknown`、`informational`、`low`、`medium`、`high`。 必需。|
...
|标记|String 集合|用户可定义标签可以应用于通知和可以充当筛选条件 （例如"HVA"、"是"等。）（支持[更新](../api/alert-update.md)）。|
|title |字符串|通知的标题。 必需。|
