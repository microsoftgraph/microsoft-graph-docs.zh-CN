...
description: 通过提供此配置文件中的配置，可以指示 iOS 设备连接到所需的Wi-Fi终结点。 通过指定Wi-Fi终结点所需的身份验证方法和安全类型，可以使最终用户的Wi-Fi连接无缝。 此配置文件提供比企业Wi-Fi配置文件更有限、更简单的安全类型。
author: dougeby
...
通过提供此配置文件中的配置，可以指示 iOS 设备连接到所需的Wi-Fi终结点。 通过指定Wi-Fi终结点所需的身份验证方法和安全类型，可以使最终用户的Wi-Fi连接无缝。 此配置文件提供比企业Wi-Fi配置文件更有限、更简单的安全类型。
...
|[删除 iosWiFiConfiguration](../api/intune-deviceconfig-ioswificonfiguration-delete.md)|None|删除 [iosWiFiConfiguration](../resources/intune-deviceconfig-ioswificonfiguration.md)。|
|[更新 iosWiFiConfiguration](../api/intune-deviceconfig-ioswificonfiguration-update.md)|[iosWiFiConfiguration](../resources/intune-deviceconfig-ioswificonfiguration.md)|更新 [iosWiFiConfiguration 对象的](../resources/intune-deviceconfig-ioswificonfiguration.md) 属性。|
...
|connectWhenNetworkNameIsHidden|布尔|当网络未 (SSID) 广播其名称时进行连接。 设置为 true 时，此配置文件会强制设备连接到未将其 SSID 广播到所有设备的网络。|
|wiFiSecurityType|[wiFiSecurityType](../resources/intune-deviceconfig-wifisecuritytype.md)|指示Wi-Fi终结点是否使用基于 EAP 的安全类型。 可取值为：`open`、`wpaPersonal`、`wpaEnterprise`、`wep`、`wpa2Personal`、`wpa2Enterprise`。|
...
|disableMacAddressRandomization|Boolean|如果设置为 true，则强制使用此Wi-Fi配置文件进行连接的设备显示其实际Wi-Fi MAC 地址，而不是随机 MAC 地址。 适用于 iOS 14 及更高版本。|
