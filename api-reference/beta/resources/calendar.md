...
|[getSchedule （预览）](../api/calendar-getschedule.md) |[scheduleInformation](scheduleinformation.md)集合|获取闲/忙时间集的用户、 通讯组列表或资源的信息指定的时间段。 |
...
|hexColor|字符串|表示日历颜色。 颜色由此 6 位数、 3 字节的十六进制数。 每个字节表示范围中 00 到 FF 以十六进制颜色的红色、 绿色和蓝色组件之一。 |
|id|String|组的唯一标识符。只读。|
|isDefaultCalendar|布尔|如果该日历否则是用户的默认日历，false。|
|isShared |Boolean |如果用户具有共享日历与其他用户，则返回 false 否则为 true。 由于只有创建日历的用户可以共享其**isShared**和**isSharedWithMe**不能为相同的用户，则返回 true。 |
|isSharedWithMe |布尔 |如果用户已为否则共享此日历 false。 此属性始终为 false 的日历所有者。  |
|name|String|日历名称。|
