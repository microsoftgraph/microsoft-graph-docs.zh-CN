...
| [列表](../api/termsofusecontainer-list-agreements.md) | [协议](agreement.md) 集合 | 获取协议对象集合。 |
...
|id|字符串| 只读。 支持 `$filter`（`eq`）。|
|isPerDeviceAcceptanceRequired|布尔值|此设置使你能够要求最终用户在从其访问它的每个设备上接受此协议。 如果最终用户尚未注册设备，Azure AD注册设备。 支持 `$filter`（`eq`）。|
