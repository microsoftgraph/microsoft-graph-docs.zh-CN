...
description: 云电脑 Azure 网络连接运行状况检查的结果。
author: AshleyYangSZ
...
|成员|说明|
|:---|:---|
|dnsCheckFqdnNotFound|域名的 DNS 解析失败。 可能的错误原因：1. 指定的 Azure vNet DNS 服务器无法解析域名。 请使用相应的 DNS 服务器更新 vNet;2. 提供的域名不存在或不正确。 请使用正确的域名更新 Azure 网络连接。 请确保 Azure 网络连接中定义的 vNet 可以解析域名。|
|dnsCheckNameWithInvalidCharacter|DNS 检查失败，因为输入的域名包含不受支持的字符。 请确保域名仅包含支持的字符。|
|dnsCheckUnknownError|域名的 DNS 解析失败。 可能的错误原因：1. 指定的 Azure vNet DNS 服务器无法解析域名。 请使用相应的 DNS 服务器更新 vNet;2. 提供的域名不存在或不正确。 请使用正确的域名更新 Azure 网络连接。 请确保 Azure 网络连接中定义的 vNet 可以解析域名。|
|adJoinCheckFqdnNotFound|域加入检查失败，因为找不到域名。 请确保可通过 Azure 网络连接中定义的 vNet 联系域名的域控制器。|
|adJoinCheckIncorrectCredentials|域加入检查失败，因为为域提供的凭据不正确。 请使用正确的凭据更新 Azure 网络连接。|
...
|adJoinCheckComputerObjectAlreadyExists|在 Azure 网络连接中提供 (OU) 找不到计算机帐户，但域中已存在计算机名称。 在计算机对象从 Azure 网络连接中配置的 OU 中移出之后，通常会发生这种情况。 请将计算机对象移回目标 OU。|
|adJoinCheckAccessDenied|域加入检查失败，因为提供的用户帐户没有足够的权限加入域。 请确保提供的帐户具有足够的权限或更改 Azure 网络连接属性中定义的用户帐户。 所需的权限：*创建计算机对象和**删除计算机对象*。|
|adJoinCheckCredentialsExpired|域加入检查失败，因为域加入用户的密码已过期。 请首先更新密码，然后使用新凭据更新 Azure 网络连接。|
...
|adJoinCheckUnknownError|由于未知错误，域加入检查失败。 请确保 Azure 网络连接可以使用提供的详细信息成功加入域。|
...
|endpointConnectivityCheckIntuneUrlNotAllowListed|预配期间，无法Intune一个或多个所需的 URL。 请确保所有所需的 URL 都可以通过防火墙和代理。|
...
|azureAdDeviceSyncCheckScpNotConfigured|由于 SCP Azure Active Directory (Azure AD) 服务配置点配置错误，混合 (连接) 。 请确保 SCP 配置有效且可用于执行混合Azure AD加入。 可以在"管理向导"中创建和配置Azure AD 连接 SCP。|
...
|resourceAvailabilityCheckNoSubnetIP|提供的子网没有可用的 IP 地址。 请确保 Azure 网络连接中提供的子网具有足够的可用 IP 地址。 请展开当前选定的子网或选择要用于设置的不同子网。|
...
|resourceAvailabilityCheckResourceGroupInvalid|选定的 Azure 资源组无效或未找到。 请确保所选的 Azure 资源组可用于预配资源。 或者，使用另一个资源组更新此 Azure 网络连接。|
|resourceAvailabilityCheckVNetInvalid|所选的 Azure 虚拟网络无效。 请确保所选虚拟网络可用且正常运行。 或者，通过另一个虚拟网络更新此 Azure 网络连接。|
|resourceAvailabilityCheckSubnetInvalid|所选 Azure 子网无效。 请确保所选子网可用且正常运行。 或者，通过另一个子网更新此 Azure 网络连接。|
|resourceAvailabilityCheckResourceGroupBeingDeleted|正在删除所选的 Azure 资源组。 请确保所选的 Azure 资源组可用于预配资源。 或者，使用另一个资源组更新此 Azure 网络连接。|
|resourceAvailabilityCheckVNetBeingMoved|正在移动选定的 Azure 虚拟网络。 请确保虚拟网络不会更改或移动，然后重试。 或者，通过另一个 vNet 更新此 Azure 网络连接。|
...
|resourceAvailabilityCheckSubnetWithExternalResources|由于所选子网包含外部资源，因此它不能使用。 请删除可能导致冲突的任何资源，然后重试。 或者，通过另一个子网更新此 Azure 网络连接。|
...
|internalServerErrorUnableToRunDscScript|预配期间，某些 PowerShell DSC 脚本在云电脑上执行。 无法在运行状况检查期间下载或执行这些 DSC 脚本。 请确保 vNet 可以不受限制地访问所需的终结点，并且不会在环境中或环境中阻止 PowerShell 组策略。|
