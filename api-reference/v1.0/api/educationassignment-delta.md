...
description: 获取新创建或更新的分配的列表，而无需对集合执行完整读取。
...
ms.locfileid: "66446300"
---
# <a name="educationassignment-delta"></a>educationAssignment： delta
命名空间：microsoft.graph

获取新创建或更新 [的分配](../resources/educationassignment.md) 的列表，而无需对集合执行完整读取。

教师或具有应用程序权限的应用程序可以看到该类的所有 **作业** 对象。 学生只能看到分配给他们 **的作业** 。
...
如果成功，此函数在响应正文中返回 `200 OK` 响应代码和 [educationAssignment](../resources/educationassignment.md) 集合。

## <a name="examples"></a>示例

### <a name="example-1-get-assignments-with-delta-query-support"></a>示例 1：获取具有增量查询支持的分配

#### <a name="request"></a>请求

下面展示了示例请求。

`$top`使用该参数指定要返回的分配数。 该参数是可选的。 如果有一长串工作分配，请使用它;否则，你将获得该类中的所有作业。
...
>**注意：** 从响应中 `@odata.nextLink` 获取另一个调用，并获取下一组分配。
...
### <a name="example-2-get-next-set-of-assignments-with-delta-query-support"></a>示例 2：使用增量查询支持获取下一组分配
...
>**注意：** 必须继续对后续调用使用 `@odata.nextLink` 该值，直到在响应中获取该 `@odata.deltaLink` 属性。
...
### <a name="example-3-get-the-created-and-modified-assignments-using-delta-token"></a>示例 3：使用增量令牌获取创建和修改的分配
...
>**注意：** 必须继续使用它 `@odata.deltaLink` 来获取自初始增量调用以来新创建的或修改的分配。

>**注意：** 增量响应可能很大，在这种情况下返回一个 `@odata.nextLink` 。 继续提取更改，直到再次命中 `@odata.deltaLink` 。
