...
> **注意：** 此 API 目前需要 `Directory.Read.All` 权限或更高版本。 使用 Group.Read.All 权限，单独使用或与`User.` 权限结合使用，均返回错误。 这是一个已知 bug。
...
| 参数           | 类型    | 说明                                                                                                                                                                                                                                                                         |
| :------------------ | :------ | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| securityEnabledOnly | 布尔 | **true** 指定仅应返回用户是其成员的安全组；**false** 指定应返回用户是其成员的所有组。注意：仅当对用户调用这个方法时，才支持将此参数设置为 **true**。 |
