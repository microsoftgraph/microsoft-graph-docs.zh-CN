...
验证 Microsoft 365 组的显示名称或邮件昵称是否符合命名策略。  客户端可以使用此 API 在尝试创建 Microsoft 365 显示名称或邮件昵[](group-post-groups.md)称是否有效。 若要验证现有组的属性，请使用 [组：validateProperties](group-validateproperties.md) 函数。
...
此 API 仅返回遇到的第一个验证失败。 如果属性未能通过多次验证，则仅返回第一个验证失败。 但是，如果仅验证前缀和后缀命名策略显示名称，您可以验证邮件昵称和别名，并接收验证错误集合。 若要了解有关配置命名策略的更多信息，请参阅["配置命名策略"。](/azure/active-directory/users-groups-roles/groups-naming-policy#configure-naming-policy-in-powershell)
