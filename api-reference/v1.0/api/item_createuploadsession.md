<span data-ttu-id="c325d-p118">使用 320 KiB（320 \* 1024 字节）倍数的文件片段大小。如果使用的片段大小不是 320 KiB 的倍数，可能会在上载最后一个文件片段后导致大文件传输失败。</span><span class="sxs-lookup"><span data-stu-id="c325d-p118">Use a fragment size that is a multiple of 320 KiB (320 \* 1024 bytes). Failing to use a fragment size that is a multiple of 320 KiB can result in large file transfers failing after the last fragment is uploaded.</span></span>
