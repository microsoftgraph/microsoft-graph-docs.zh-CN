...
| 属性       | 类型    |说明|
|:---------------|:--------|:----------|
|appliedCategories|[plannerAppliedCategories](../resources/plannerappliedcategories.md)|此任务已应用的类别。有关可能的值，请参阅[已应用的类型](../resources/plannerappliedcategories.md)。|
|assigneePriority|String|用于为列表视图中的此类型项目排序的提示。此格式在[使用规划器中的排序提示](../resources/planner_order_hint_format.md)定义中。|
|工作分配|[plannerAssignments](../resources/plannerassignments.md)|分配到任务的用户集合。|
|bucketId|String|此任务所属的存储桶 id。存储桶需要位于任务所在的计划中。长度为 28 个字符，区分大小写。[格式验证](../resources/planner_identifiers_disclaimer.md)在服务上完成。 |
...
|orderHint|String|用于为列表视图中的此类型项目排序的提示。此格式在[使用规划器中的排序提示](../resources/planner_order_hint_format.md)定义中。|
|PercentComplete|Int32|任务完成百分比。当设置为 `100` 时，表示任务已完成。 |
