...
获取 RBAC [提供程序的 unifiedRoleAssignment](../resources/unifiedroleassignment.md) 对象列表。
...
此操作需要查询 `$filter` 参数来查询受支持的 RBAC 提供程序的角色分配。

对于目录提供程序，您必须筛选 **roleDefinitionId** 或 **principalId** 属性。 **roleDefinitionId** 属性可以是角色对象 ID 或 **templateId 属性的值**。

对于权利管理提供程序，您必须筛选 **roleDefinitionId**、 **principalId** 或 **appScopeId** 属性。
...
### <a name="example-3-request-using-filter-for-role-assignments-on-an-access-package-catalog-and-expand-the-principal-object"></a>示例 3：请求$filter包目录上的角色分配，并展开主体对象
