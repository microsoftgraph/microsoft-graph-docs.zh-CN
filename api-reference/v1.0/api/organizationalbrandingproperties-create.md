...
如果已配置品牌，PUT 将覆盖所有现有值，无论请求正文中有什么内容。 PATCH 仅覆盖请求正文中包含的值，保留未包含的值不变。
...
在这种情况下，将设置默认品牌对象。 本地化品牌也会由于 Content-Language 标头而设置，即使该响应中未返回 `en-US`  `en-US` 品牌设置。 请求 **中的 Content-Language** 标头是可选的，如果不存在，则仅设置默认品牌。
