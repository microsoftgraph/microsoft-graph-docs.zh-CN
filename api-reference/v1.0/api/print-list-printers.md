...
* 和 `$expand` `select` 运算符受导航属性支持，但 `share` 不支持 `jobs` 。
...
如果成功，此方法在响应正文中返回响应代码和打印机 `200 OK` 对象集合。 [](../resources/printer.md)

>**注意**：响应将不包含"defaults"和"capabilities"属性。 可以使用"获取打印机" [请求查询这些属性](printer-get.md) 。
