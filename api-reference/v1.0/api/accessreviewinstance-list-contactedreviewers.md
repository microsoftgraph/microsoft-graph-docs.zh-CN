...
此 API 的默认页面大小为 100 **accessReviewReviewer** 对象。 若要提高效率并避免由于大型结果集而超时，`$skip``$top`请通过使用 和 查询参数应用分页。 有关详细信息，请参阅[在应用中对 Microsoft Graph 数据进行分页](/graph/paging)。
...
如果成功，此方法在响应 `200 OK` 正文中返回 响应代码和 [accessReviewReviewer](../resources/accessreviewreviewer.md) 对象集合。
