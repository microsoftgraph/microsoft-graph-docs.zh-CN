...
|国家|字符串|用户所在的国家/地区；例如，“美国”或“英国”。支持 $filter。|
...
|学校|字符串集合|供用户枚举其学习过的学校列表。|
|技能|字符串集合|供用户枚举其技能的列表。|
|省份|字符串|用户地址中的省/市/自治区或省。支持 $filter。|
|streetAddress|字符串|用户公司地点的街道地址。|
|姓|字符串|用户的姓氏。支持 $filter。|
...
| 值    | #  |说明|
|:---------------|:--------|:----------|
|空|0|默认值，无 `ageGroup` 已经作为用户设置。|
|minorWithoutParentalConsent |1|(预留备用)|
|minorWithParentalConsent|2| 基于其所在国家/地区的年龄相关规定，用户被视为未成年人，同时帐户管理员已从f父母之一或监护人获取适当许可。|
|成人|3|基于所在国家或地区的年龄相关法规，用户被视为成人。|
...
| 值    | #  |说明|
|:---------------|:--------|:----------|
|空|0|默认值，无 `consentProvidedForMinor` 已经作为用户设置。|
|已获授权|1|用户已经获得配置账户许可。|
|拒绝|2|已为用户获得许可配置账户。|
...
| 关系 | 类型   |说明|
|:---------------|:--------|:----------|
|活动|[userActivity](projectrome_activity.md) 集合|跨设备的用户的活动。 只读。 可为空。|
...
|inferenceClassification | [inferenceClassification](inferenceClassification.md) | 基于显式指定的用户邮件相关性分类，可以替代推导的相关性或重要性。 |
|licenseDetails|[licenseDetails](licensedetails.md) 集合|此用户的许可证详细信息集合。 可为空。|
...
|人员|[人员](person.md)集合| 人员 集合 只读。 可为空。
|照片|[profilePhoto](profilephoto.md)| 用户的个人资料照片。只读。|
|计划员|[plannerUser](planneruser.md)| plannerUser 只读。|
