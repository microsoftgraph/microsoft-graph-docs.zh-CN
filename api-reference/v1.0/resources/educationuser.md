...
|externalSource|`educationExternalSource`| 创建此用户的位置。 可能的值为： `sis`， `manual`， `unkownFutureValue`。|
...
|primaryRole|educationUserRole| 用户的默认角色。 用户的角色在各课程中可能有所不同。 可能的值为： `student`， `teacher`， `unknownFutureValue`。 支持 $filter。|
...
|assignments| [educationAssignment](/graph/api/resources/educationassignment?view=graph-rest-beta)| 为用户分配列表。 可为 Null。|
|user|[user](user.md)| 对应于此用户目录用户。|
