...
description: 表示系统中的用户。 这是用户的教育特定变体，其 ID 与 Microsoft Graph/users 终结点返回的 ID 相同。
author: mlafleur
...
ms.locfileid: "64588538"
---
# <a name="educationuser-resource-type"></a>educationUser 资源类型

命名空间：microsoft.graph

系统中的用户。 这是用户的教育特定变体，其 **ID** 与 Microsoft Graph非教育特定终结点返回的 ID 相同`/users`。 此对象提供核心用户对象中的一组目标属性，[](../resources/user.md)并添加一组特定于教育的属性，如 **primaryRole**、**student** 和 **teacher** 数据。
...
| [列出更新类](../api/educationuser-list-taughtclasses.md) | [educationClass](../resources/educationclass.md) 集合 | 从 **educationClasses** 导航属性 **获取 educationClass** 资源。                       |
...
| onPremisesInfo       | [educationOnPremisesInfo](../resources/educationonpremisesinfo.md) | 用于将用户与 Azure Active Directory对应的 Active Directory 关联的其他信息。                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| preferredLanguage    | String                                                             | 用户应遵循 ISO 639-1 代码的首选语言，例如， `en-US`。                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| residenceAddress     | [physicalAddress](../resources/physicaladdress.md)                 | 用户所居住在的地址。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| showInAddressList    | Boolean                                                            | `True`如果全局Outlook列表应包含此用户;否则为 `false`。 如果未设置，则将其视为 `true`。 对于通过邀请管理器邀请的用户，此属性将设置为 `false`。                                                                                                                                                                                                                                                                                                               |
...
| userPrincipalName    | String                                                             | 用户的用户主体名称 (UPN)。 UPN 是用户基于 Internet 标准 RFC 822 的 Internet 样式登录名。 按照惯例，此名称应映射到用户的电子邮件名称。 常规格式为 `alias@domain`，其中 domain 必须存在于租户的已验证域集合中。 创建用户时此属性是必需的。 可以从组织的 **verifiedDomains** 属性访问租户的已验证 [域](organization.md)。 支持 `$filter` 和 `$orderby`。 |
| userType             | String                                                             | 可用于对目录中的用户类型进行分类的字符串值，例如`Member``Guest`。 支持 `$filter`。                                                                                                                                                                                                                                                                                                                                                                                                              |

> [!IMPORTANT]
> 使用委派权限范围时，Microsoft Graph 将仅返回一组有限的属性：**id**、**primaryRole**、**accountEnabled**、**displayName**、**givenName**、**surname**、**userPrincipalName**、**userType**、**onPremisesInfo**、**student/externalId**、**teacher/externalId**。 如果应用程序需要其他属性，则必须使用应用程序权限范围。
...
| 用户          | [user](../resources/user.md)                                          | 对应于此用户的目录用户。 |
