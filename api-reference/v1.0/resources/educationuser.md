...
|externalSource|`educationExternalSource`| 创建此用户的位置。 可能的值包括 `sis`、`manual`、`unkownFutureValue`。|
...
|primaryRole|educationUserRole| 用户的默认角色。 用户的角色在各课程中可能有所不同。 可能的值包括 `student`、`teacher`、`unknownFutureValue`。 支持 $filter。|
