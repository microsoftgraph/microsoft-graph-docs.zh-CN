...
|supportedServices|字符串集合| 分配给域的功能。 可以包含`0``1`或包含以下值的更多内容：`Email`、`Sharepoint`、、`EmailInternalRelayOnly``OfficeCommunicationsOnline`、`SharePointDefaultDomain`、`FullRedelegation`、`SharePointPublic`、、`OrgIdAuthentication`、。 `Intune``Yammer` 可以使用图形 API添加/删除的值包括： `Email`， ， `OfficeCommunicationsOnline`。 `Yammer` 不可为 null|
