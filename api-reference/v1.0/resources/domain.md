...
4. [表示](../api/domain_update.md)你计划与域配合使用的支持的服务。
...
|id|String| 域的完全限定的名称。密钥、不可变、不可为 Null、唯一 |
...
|supportedServices|字符串集合| 分配到域的功能。<br><br>可以包含下列值中的 0 个、1 个或更多个：*电子邮件*、*Sharepoint*、*EmailInternalRelayOnly*、*OfficeCommunicationsOnline*、*SharePointDefaultDomain*、*FullRedelegation*、*SharePointPublic*、*OrgIdAuthentication*、*Yammer*、*Intune*<br><br> 你可以使用 Graph API 添加/删除的值包括：*电子邮件*、*OfficeCommunicationsOnline*、*Yammer*<br>不可为 Null|
