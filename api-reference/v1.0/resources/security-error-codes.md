...
description: Microsoft Graph安全 API 中的错误使用标准 HTTP 206 部分内容状态代码返回，并且通过警告标头传递。
author: preetikr
...
ms.locfileid: "54205203"
---
# <a name="microsoft-graph-security-api-error-responses"></a>Microsoft Graph 安全 API 错误响应

命名空间：microsoft.graph

Microsoft Graph安全 API 中的错误使用标准 HTTP 206 部分内容状态代码返回，并且通过警告标头传递。

## <a name="errors"></a>错误

Microsoft Graph 安全性 API 是一种联合服务，可接收来自所有数据提供程序的多个响应。 当 Microsoft Graph 安全 API 收到 HTTP 错误时，它将以以下格式发送回警告标头：
<!-- { "blockType": "ignored" } -->
...
- 如果提供程序出现问题，警告标头中 (504) HttpStatusCode.GatewayTimeout。
...
> **注意：** 每个 HTTP 标头都是子网站的集合，因此用户可以枚举 Warning 标头并检查所有项目。
...
此限制的一个工作是使用 OData 查询参数和 Microsoft Graph 安全 API 中的 警报实体的 ，将 dateTime 值替换为最后一个 (`$filter` `eventDateTime` `?$filter=eventDateTime gt {YYYY-MM-DDT00:00:00.000Z}` 1500) 警报。 您还可以为 ;例如 `eventDateTime` *，alerts？$filter=eventDateTime **gt** 2018-11-**11** T00：00：00.000Z&eventDateTime **lt** 2018-11-**12** T00：00：00.000Z*
