...
| 属性     | 类型   |说明|
|:---------------|:--------|:----------|
|action|onenotePatchActionType|要在目标元素上执行的操作。 可能的值为：`replace`、`append`、`delete`或 `insert``prepend`。|
...
|target|字符串|要更新的元素。 必须是`#<data-id>`元素的或生成`<id>`的，或者`body``title`是关键字。|
