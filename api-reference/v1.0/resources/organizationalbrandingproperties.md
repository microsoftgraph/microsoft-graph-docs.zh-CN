...
| bannerLogoRelativeUrl | 字符串 | **bannerLogo** 属性的相对 URL，与 **cdnList** CDN基 URL 结合使用以提供由 CDN 提供的只读CDN。 只读。 |
...
| squareLogo | Stream | 在 OOBE Windows 10中显示公司徽标的正方形 (OOBE) 时Windows Autopilot 进行部署。 允许的类型为 PNG 或 JPEG，大小不超过 240 x 240 像素和不超过 10 KB。 我们建议使用透明图像，徽标周围没有填充。 |
