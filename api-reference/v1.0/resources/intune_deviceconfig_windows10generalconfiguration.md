...
|startMenuLayoutEdgeAssetsXml|Binary|此策略设置允许你导入 Edge 资产以与 startMenuLayoutXml 策略一起使用。 开始布局可以包含查找 Edge 本地资产文件的 Edge 应用中的辅助磁贴。 在这种情况下，Edge 本地资产不存在并导致 Edge 辅助磁贴显示为空。 仅当修改 startMenuLayoutXml 策略时才应用此策略。 该值应该是一个 UTF-8 Base64 编码的字节数组。|
...
|connectedDevicesServiceBlocked|Boolean|是否阻止能够发现并连接到其他设备、远程消息、远程应用会话和其他跨设备体验的联网设备服务。|
...
|safeSearchFilter|String|指定需要的安全搜索过滤级别。 可取值为：`userDefined`、`strict`、`moderate`。|
...
|edgeRequireSmartScreen|Boolean|指示是否要求用户使用智能屏幕过滤器。|
...
|settingsBlockAddProvisioningPackage|Boolean|指示是否阻止用户安装配置软件包。|
|settingsBlockRemoveProvisioningPackage|Boolean|指示是否阻止运行时配置代理删除配给程序包。|
...
|windowsStoreBlocked|Boolean|指示是否阻止用户使用 Windows Store。|
|appsAllowTrustedAppsSideloading|String|指示是否可以旁加载使用可信证书签名的来自 AppX 软件包的应用。 可取值为：`notConfigured`、`blocked`、`allowed`。|
|windowsStoreBlockAutoUpdate|Boolean|指示是否阻止从 Windows Store 自动更新应用。|
...
|appsBlockWindowsStoreOriginatedApps|Boolean|指示是否禁用启动预先安装或已下载的 Windows Store 中的所有应用。|
