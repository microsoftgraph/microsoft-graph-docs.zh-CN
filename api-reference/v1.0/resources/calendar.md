...
> **注意：** 可以与用户日历和群组日历进行交互的方式有一些细微差异：

 - 你可以仅整理 [calendarGroup](calendargroup.md) 中的用户日历。
 - Outlook 可代表组自动接受所有会议请求。 你可以仅为用户日历[接受](../api/event_accept.md)、[暂时接受](../api/event_tentativelyaccept.md)或[拒绝](../api/event_decline.md)会议请求。
  - Outlook 不支持为群组事件的提醒。 你可以仅[暂停](../api/event_snoozereminder.md)或[消除](../api/event_dismissreminder.md)用户日历的[提醒](reminder.md)。
...
|[创建日历](../api/user_post_calendars.md) |[日历](calendar.md)| 在默认日历组或用户的指定日历组中新建日历。|
|[获取日历](../api/calendar_get.md) | [日历](calendar.md) |获取 **calendar** 对象的属性和关系。 可以是用户的日历，也可以是 Office 365 组的默认日历。 |
|[更新](../api/calendar_update.md) | [日历](calendar.md)  |更新 **calendar** 对象的属性。 可以是用户的日历，也可以是 Office 365 组的默认日历。 |
...
|ID|字符串|组的唯一标识符。只读。|
|名称|字符串|日历名称。|
|负责人 |[emailAddress](emailaddress.md) | 如果设置，则表示创建或添加日历的用户。对于用户创建或添加的日历，将 **owner** 属性设置为用户。对于与用户共享的日历，将 **owner** 属性设置为与此用户共享该日历的人员。 |

## <a name="relationships"></a>关系
| 关系 | 类型   |说明|
|:---------------|:--------|:----------|
|calendarView|[事件](event.md) 集合|日历的日历视图。导航属性。只读。|
|事件|[事件](event.md) 集合|日历中的事件。导航属性。只读。|
