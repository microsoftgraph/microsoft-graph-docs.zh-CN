...
|countryLookupMethod|countryLookupMethodType|确定用于确定用户所在的国家/地区的方法。 可能的值为 (`clientIpAddress` 和) `authenticatorAppGps` 默认值。 注意 `authenticatorAppGps` ：目前尚不支持Microsoft Cloud for US Government。|
