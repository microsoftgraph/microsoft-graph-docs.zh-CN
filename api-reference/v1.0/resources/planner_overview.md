...
可以使用在 Microsoft Graph 中使用 Planner API 创建任务并将其分配给 Office 365 中的某个组中的用户。

开始使用 Planner API 之前，需要了解主对象互相之间以及与 Office 365 组的关系。

## <a name="groups"></a>组

Office 365 组是 Planner API 中计划的所有者。
为 [获取由一个组所拥有的计划](../api/plannergroup_list_plans.md)，请进行以下 HTTP 请求。

``` http
GET /groups/{id}/planner/plans
```

当[创建新的计划](../api/planner_post_plans.md)，通过设置计划对象的 `owner` 属性可以使组成为其所有者。 计划必须由组作为其所有者。

>**注意：** 创建计划的用户必须是将拥有计划的组的成员。 当使用[新建组](../api/group_post_groups.md)创建新组时，你不会作为成员添加到该组。 创建组后，需使用[组发布成员](../api/group_post_members.md)将自己添加为成员。

## <a name="plans"></a>计划

[计划](plannerplan.md)是[任务](plannertask.md)的容器。 要[在计划中创建任务](../api/planner_post_tasks.md)，请在创建任务时将任务对象的 `planId` 属性设置为计划的 ID。
当前无法在没有计划的情况下创建任务。
要[检索计划中的任务](../api/plannerplan_list_tasks.md)，请进行以下 HTTP 请求。
...
| :---------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------ |
| 简单列表（计划中的任务）                                                               | `orderHint` 任务的属性                                                   |
| 简单列表（分配给用户的任务）                                                      | `assigneePriority` 任务的属性                                            |
