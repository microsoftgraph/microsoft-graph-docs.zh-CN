...
|属性|类型|说明|
|:---|:---|:---|
|accessReviewId|字符串|accessReviewInstance 父项的标识符。 支持 `$select`。 只读。|
...
|principalLink|字符串|指向主体对象的链接。 例如，`https://graph.microsoft.com/v1.0/users/a6c7aecb-cbfd-4763-87ef-e91b4bd509d9`。 只读。|
|建议|字符串|针对基于上次交互式登录租户的审批决策的系统生成的建议。 如果登录是自审查开始起 30 天内，建议批准。 如果登录自审查开始超过 30 天，建议拒绝。 否则，建议不可用。 可能的值 `Approve` ：、 `Deny` 或 `NoInfoAvailable` 。 仅 `$select` `$orderby` 支持 、 (`$filter` 和 `eq`) 。 只读。|
