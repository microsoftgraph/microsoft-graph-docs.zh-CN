# <a name="use-the-microsoft-graph-api-to-integrate-with-outlook-mail"></a>使用 Microsoft Graph API 与 Outlook 邮件集成
...
你可以使用已知的文件夹名称，如 `Inbox`、`Drafts`、`SentItems` 或 `DeletedItems`，来识别某些对所有用户默认存在的邮件文件夹。例如，你可以从已登录用户的 Outlook **已发送邮件**文件夹收取邮件，无需事先获取文件夹 ID：
```
