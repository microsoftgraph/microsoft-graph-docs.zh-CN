...
| 属性     | 类型   |说明|
|:---------------|:--------|:----------|
|accountEnabled|Boolean| 启用帐户时为 `true`，否则为 `false`。 必填。 默认值为“`true`”。 <br/><br/> 支持 `$filter`（`eq`、`ne`、`not`、`in`）。 只有全局管理员和云设备管理员角色中的呼叫者才能设置此属性。|
...
|operatingSystem|String| 设备上操作系统的类型。 必需。 支持 `$filter` `eq` `ne` `not` (、、、、、、和值 `ge` `le` `startsWith` `eq` `null`) 。 |
|operatingSystemVersion|String|设备上操作系统的版本。 必需。 支持 `$filter` `eq` `ne` `not` (、、、、、、和值 `ge` `le` `startsWith` `eq` `null`) 。 |
...
|memberOf|[directoryObject](directoryobject.md) 集合|此设备是其中一个成员的组。 只读。 可为空。 支持 `$expand`。 |
