...
* 文件夹具有[**文件夹 Facet**](folder.md)
* 文件夹具有[**文件 Facet**](file.md)。
* 除了文件 Facet，图像还具有[**图像 facet**](image.md)。
* 使用照相机拍摄的图像（照片）具有[**照片 Facet**](photo.md)，用于将项标识为照片，并提供照片的拍摄时间和拍摄所用设备等属性。
...
| [列出驱动器中的更改](../api/item_delta.md)        | `GET /drive/root/delta`
...
在 OneDrive for Business 或 SharePoint 文档库中，如果 **driveItem** 具有[文件夹](folder.md) Facet，则不返回 **cTag** 属性。
