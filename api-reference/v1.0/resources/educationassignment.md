...
| 方法           | 返回类型    |说明|
|:---------------|:--------|:----------|
|[创建作业资源](../api/educationassignment-post-resource.md) |[educationAssignmentResource](educationassignmentresource.md)| 通过发布到资源集合来创建新的 **educationAssignmentResource** 。|
...
|[Delete](../api/educationassignment-delete.md) | 无 |删除 **educationAssignment** 对象。 |
|[发布](../api/educationassignment-publish.md)|[educationAssignment](educationassignment.md)|将 **educationAssignment** 对象的状态从草稿更改为已发布。|
|[设置工作分配资源文件夹](../api/educationassignment-setupresourcesfolder.md)| string| 在预定义的位置下创建SharePoint文件夹 () 上传文件作为分配资源。|
...
|[删除类别](../api/educationassignment-remove-category.md) |无| 从此作业中删除属于该类 **的 educationCategory** 。|
|[附加 rubric](../api/educationassignment-put-rubric.md)|无|将现有 **educationRubric** 附加到此作业。|
|[删除 rubric](../api/educationassignment-delete-rubric.md)|无|将 **educationRubric** 与此作业分离。|
|[Get delta](../api/educationassignment-delta.md)|[educationAssignment](../resources/educationassignment.md) 集合|获取新创建或更新的 **educationAssignment 对象的** 列表，而无需对集合执行完整读取。|
...
|createdBy|[identitySet](identityset.md)| Who创建了 **分配**。 |
...
|指示|[itemBody](itembody.md)| 有关分配的说明。  这连同显示名称一起告诉学生该怎么做。 |
|lastModifiedBy|[identitySet](identityset.md)| Who上次修改 **分配**。 |
...
|notificationChannelUrl|String|用于指定用于帖子 **分配** 发布通知的 [通道](channel.md) URL 的可选字段。 如果未指定或为 null，则 `General` 默认为通道。 此字段仅适用于 **assignTo** 值为 [educationAssignmentClassRecipient](educationassignmentclassrecipient.md) 的 **作业**。 发布分配后，不允许更新 **notificationChannelUrl** 。|
...
| 关系 | 类型   |说明|
|:---------------|:--------|:----------|
|resources|[educationAssignmentResource](educationassignmentresource.md) 集合| Learning与此分配关联的对象。  只有教师才能修改此列表。 可为 NULL。|
