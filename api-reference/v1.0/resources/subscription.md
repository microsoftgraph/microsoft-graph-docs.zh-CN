...
| resource | string | 必需。 指定将监视的更改的资源。 不包含的基 URL (`https://graph.microsoft.com/v1.0/`)。 |
...
| clientState | string | 可选。 指定服务为每个通知发送的 `clientState` 属性的值。 最大长度为 128 个字符。 通过对比与订阅一起发送的 `clientState` 属性值和与每个通知一起接收的 `clientState` 属性值，客户端可以检查通知是否是由服务发送。 |
