...
|displayName|字符串|分区名称。 |
|pagesUrl|字符串|可以在其中获取分区中所有页面的详细信息的 `pages` 终结点。只读。|
|self|字符串|可以在其中获取关于分区的详细信息的终结点。只读。|
...
| 方法           | 返回类型    |说明|
|:---------------|:--------|:----------|
|[获取分区](../api/section_get.md) | [OnenoteSection](section.md) |读取分区的属性和关系。|
|[创建页面](../api/section_post_pages.md) |[页面](page.md)| 通过发布到指定分区中的页面集合创建页面。|
|[列出页面](../api/section_list_pages.md) |[页面](page.md)集合| 获取指定分区中的页面集合。|
