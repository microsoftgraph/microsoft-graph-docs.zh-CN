...
|links|[SectionLinks](sectionlinks.md)|用于打开分区的链接。 如果 `oneNoteClientURL` 已安装，链接将在 OneNote 本机客户端中打开分区。 链接 `oneNoteWebURL` 在 OneNote 网页版中打开分区。|
|displayName|字符串|节的名称。 |
|pagesUrl|字符串|`pages`终结点，可在其中获取分区中所有页面的详细信息。 只读。|
|self|字符串|终结点，您可在此处获取关于节的详细信息。 只读。|
