...
|isLocationOnline|布尔值|如果 `true` 为 ，则指示约会将联机进行。 默认值为 `false`。|
|joinWebUrl|String|约会的在线会议 URL。|
|maximumAttendeesCount|Int32|约会中允许的最大客户数。|
|optOutOfCustomerEmail|布尔值|如果 `true` 指示此约会的 [bookingCustomer](bookingcustomer.md) 不希望收到有关此约会的确认。|
