...
|[Delete androidGeneralDeviceConfiguration](../api/intune_deviceconfig_androidgeneraldeviceconfiguration_delete.md)|无|删除 [androidGeneralDeviceConfiguration](../resources/intune_deviceconfig_androidgeneraldeviceconfiguration.md)。|
...
|cellularBlockWiFiTethering|Boolean|指示是否阻止同步 Wi-Fi 绑定。|
...
|compliantAppListType|String|位于 CompliantAppsList 中的列表类型。 可取值为：`none`、`appsInListCompliant`、`appsNotInListCompliant`。|
...
|passwordRequiredType|String|必需的密码类型。 可取值为：`deviceDefault`、`alphabetic`、`alphanumeric`、`alphanumericWithSymbols`、`lowSecurityBiometric`、`numeric`、`numericComplex`、`any`。|
...
|voiceAssistantBlocked|Boolean|指示是否阻止语音助手的使用。|
...
|webBrowserCookieSettings|String|Web 浏览器内的 Cookie 设置。 可取值为：`browserDefault`、`blockAlways`、`allowCurrentWebSite`、`allowFromWebsitesVisited`、`allowAlways`。|
