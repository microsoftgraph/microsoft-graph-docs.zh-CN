...
若要调用 Microsoft Graph，则将访问令牌附加为 HTML 请求中授权标头的持有者令牌。例如，以下为返回已登录用户的个人资料信息的调用（为了不影响可读性，已截断访问令牌）：
...
有关 Microsoft Graph 权限的完全列表以及委派权限和应用程序权限之间的差异，请参阅[权限引用](permissions_reference.md)。
...
* 不可从[独立 Web API](https://docs.microsoft.com/azure/active-directory/develop/active-directory-v2-limitations#restrictions-on-app-types) 调用 Microsoft Graph。 
* 不可调用云解决方案提供商应用。
...
>**重要说明**
>
>**在做出关于开发生产应用时使用哪种终结点的决定前，请查阅[我应该使用 v2.0 终结点吗？](https://docs.microsoft.com/azure/active-directory/develop/active-directory-v2-limitations)。**
...
### <a name="microsoft-graph-connect-samples"></a>Microsoft Graph Connect 示例 

Microsoft 为适用于各种平台的 Microsoft Graph 发布 Connect 示例，包括：Android、Angular.JS、ASP.NET、iOS（Obj C 和 Swift）、Node.JS、PHP、Python、Ruby、UWP 和 Xamarin。你可以通过这些示例检查使用各种身份验证库从 Azure AD 获取令牌的代码。目前，大多数示例使用第三方身份验证库；但是，ASP.NET 和 UWP 示例则使用 Microsoft 库。

- [生成你的第一个应用](get-started.md)部分包含详细的文章，为你演示如何使用 Azure AD v2.0 终结点创建 Connect 应用，并包括在各个平台上所使用的身份验证库。平台和身份验证终结点列出了可用示例。
- 若要快速获取所选平台上的运行示例，请参阅 [Microsoft Graph 快速入门](https://developer.microsoft.com/graph/Quick-Start)。
- 请访问 GitHub 上的 [Microsoft Graph 存储库](https://github.com/microsoftgraph)以查看 Microsoft Graph 的所有可用示例。 
