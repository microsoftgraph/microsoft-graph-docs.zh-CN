...
| type | type | 预期响应的可选类型。 默认值为 `json`。 仅在 (终结点上支持 `json` `image` 或支持 `/photo/value$`) 。 |
| 不适用 | 响应 | 如果请求成功，Microsoft Graph只读响应。  |
| 不适用 |error| 如果请求未成功，Microsoft Graph只读错误。 |
...
| 值 | 返回的数组的数据 `value` 项 | 当预期来自图形的响应包含一组项目（如邮件、文件或用户）时，请使用模板而不是 `value` `default` **模板**。   将自动 `value` 为资源返回的每个项目重复模板。 与默认模板模板不同，一旦准备好，模板 `value` (即可开始呈现) 。|
