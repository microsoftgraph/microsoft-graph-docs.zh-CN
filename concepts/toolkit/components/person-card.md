---
title: Person-Card Microsoft Graph 工具包中的组件
...
---
# <a name="person-card-component-in-the-microsoft-graph-toolkit"></a>Person-Card Microsoft Graph 工具包中的组件
...
该 `MgtPersonCard` 类公开一个静态 `config` 对象，该对象配置应用程序中的所有人员卡组件。 配置对象配置人员卡使用哪些部分和哪些 API 从 Microsoft Graph 中提取有关用户的详细信息。
...
| lock-tab-navigation | `boolean` - 允许使用选项卡锁定导航，使其不会流出卡片部分。 默认值为 `false`。  |
...
Person-Card组件允许用户联系目标人员，包括通过 Teams 聊天。 如果在 Teams 选项卡应用中使用组件，可以确保组件直接链接到聊天，而不是通过设置 `microsoftTeamsLib` 其中 `TeamsProvider`打开浏览器窗口。

如果Person-Card组件无法检测到 Teams lib，则组件将尝试改为打开 Teams Web 客户端。
...
有关提供程序的 `TeamsProvider` 详细信息，请参阅 [Microsoft Teams 提供程序](../providers/teams.md)。
...
------|-------------------|--------------|:-----------:|:---------:|:---------------------------:|
`expanded` | 用户已打开卡片的展开详细信息部分 | 无 | 否 | 是 | 是，除非重写默认模板
...
| 配置 | 权限 | API | 节 |
| --- | ---------- | ------- | --------- |
| `personDetails` 使用用户的 `id` 但不带电子邮件的设置、 `userId` 设置或 `personQuery` 设置为 `me` | User.ReadBasic.All | [/users/{id}](/graph/api/user-list-people)、 [/users/{id}/photo/$value](/graph/api/profilephoto-get) | 默认值 |
...
| `sections.profile` 已启用 (默认)  | User.Read.All | [/users/{id}/profile](/graph/api/profile-get?view=graph-rest-beta&preserve-view=true) | 配置文件 |
...
> [!IMPORTANT]
> 组 `mgt-person-card` 件从父 `mgt-person` 组件中检索基本人员数据，而无需调用 Microsoft Graph。 当单独使用时 `mgt-person-card` ，它将检索必要的数据本身并缓存它。 卡片部分中显示的数据是单独检索的，不会缓存。
...
有关如何配置缓存的更多详细信息，请参阅 [缓存](../customize-components/cache.md) 。
