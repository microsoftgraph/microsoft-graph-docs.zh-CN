...
您可以使用多个 propertiesto 来自定义组件。

| 属性 | 属性 | 说明 |
| --- | --- | --- |
| `showName` | `show-name` | 设置用于显示人员显示名称的标志-默认`false`为。 |
| `showEmail` | `show-email` | 设置用于显示个人电子邮件的标志- `false`默认为。 |
...
| 数据类型 | 数据上下文 | 说明 |
| --- | --- | --- |
| `default` | `person`: person 对象 | 默认模板会将整个组件替换为您自己的组件。 |
