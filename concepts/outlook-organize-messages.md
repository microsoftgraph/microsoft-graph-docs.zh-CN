...
以编程方式，邮件文件夹由 [mailFolder](../api-reference/v1.0/resources/mailfolder.md) 资源表示，“收件箱”是文件夹结构根目录中的一个文件夹。 `/users/{id}/mailfolders` 快捷方式表示根。 

每个 **mailFolder** 都由其文件夹 ID 标识，并具有可写的 **displayName** 属性。 Outlook 默认情况下会为客户创建其他几个文件夹。 可以按文件夹 ID 或其众所周知的名称引用这些默认文件夹：`ArchiveRoot`、`ConversationHistory`、`DeletedItems`、`Drafts`、`Inbox`、`JunkEmail`、`Outbox` 和 `SentItems`。 
...
在子文件夹级别，可以[列出“收件箱”或任何其他文件夹下的子文件夹](../api-reference/v1.0/api/user_list_mailfolders.md)。 **childFolderCount** 属性表示直接子文件夹的数量。
...
Outlook 允许客户设置规则，以在满足某些预确定的条件时自动对传入邮件执行特定操作。 可以为“收件箱”[创建一个规则](../api-reference/beta/api/mailfolder_post_messagerules.md)作为 [messageRule](../api-reference/beta/resources/messagerule.md)，以在特定条件下将邮件复制或移动到特定文件夹。 条件是 [messageRulePredicates](../api-reference/beta/resources/messagerulepredicates.md)。 它们可以包括包含特定文本的邮件主题或正文、从某些电子邮件地址发送的邮件或标记为重要的邮件等等。 
