...
---
# <a name="microsoft-bookings-api-overview-preview"></a>Microsoft 预订 API 概述 （预览）
...
当不在电话旁边或下班时间，企业经营者也不会错过客户预订。 客户可以直接在计划页上的业务网站或 Facebook 任何时间[看到可用的服务](/graph/api/bookingbusiness-list-services?view=graph-rest-beta)和[书籍约会](/graph/api/bookingbusiness-post-appointments?view=graph-rest-beta)。 

企业经营者可以在任何地方（Web 上或移动应用上）亲自或通过电话办理预订。 他们可以[重新安排](/graph/api/bookingappointment-update?view=graph-rest-beta)、[取消](/graph/api/bookingappointment-cancel?view=graph-rest-beta)现有预订或将其[重新分配](/graph/api/bookingappointment-update?view=graph-rest-beta)给其他可用的员工成员。 
...
请参阅[Microsoft Graph beta 中的 Microsoft 预订 API](/graph/api/resources/booking-api-overview?view=graph-rest-beta)。
