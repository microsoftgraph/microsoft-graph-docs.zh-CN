...
- **delta** 函数。
- 上一个 GET **delta** 函数调用的[状态令牌](./delta_query_overview.md)（*deltaToken* 或 *skipToken*）。
...
如果成功，此方法在响应正文中返回 `200 OK` 响应代码和[组](../api-reference/v1.0/resources/group.md)集合对象。 如果整个组集过大而无法适应一个响应，那么还将包括一个包含状态令牌的 `nextLink`。
...
>**注意：**`members@delta` 属性包含在第一个组对象 (TestGroup1) 中并包含该组的两个当前成员。 TestGroup2 不包含该属性，因为该组不具有任何成员。
