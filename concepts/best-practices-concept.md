...
- **使用最小特权**。 只请求绝对必要的权限，并且只在需要时请求。 对于应用程序调用的 API，查看方法主题中的权限部分（例如，查看[创建用户](../api-reference/v1.0/api/user_post_users.md)），然后选择最小特权权限。 有关权限的完整列表，请参阅[权限引用](permissions_reference.md)。
...
使用 [webhook 通知](../api-reference/v1.0/resources/webhooks.md)，在数据发生变化时获得推送通知。
...
使用[增量查询](delta_query_overview.md)以有效保持数据最新。
...
使用 [webhook 通知](../api-reference/v1.0/resources/webhooks.md)作为触发器以触发增量查询调用。 此外，还应确保应用程序有一个支持轮询阈值，以防触发通知。

### <a name="batching"></a>批处理

JSON 批处理使你能够通过将多个请求合并为一个单一 JSON 对象优化应用程序。 将各个请求合并到一个单独的批请求中，可以使应用程序不受严重网络延迟的影响，并且可以节省连接资源。

使用[批处理](json_batching.md)时，严重的网络延迟会对性能产生很大的影响。
