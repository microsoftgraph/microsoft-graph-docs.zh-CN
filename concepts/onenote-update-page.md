...
> [!NOTE]
> 在 OneNote 页面上更新图像时，不能使用 www 链接。 服务不会尝试下载随机资源。 相反，映像必须是请求的一部分，无论是通过图像数据 URL 还是多部分请求的部分名称。
...
> [!NOTE]
> API 会丢弃在 create-page 和 update-page 请求的 [输入 HTML](onenote-input-output-html.md) 中定义的所有 **ID** 值。
...
> [!NOTE]
> 可以通过发出 GET 请求 `https://graph.microsoft.com/v1.0/users`来获取用户 ID。
