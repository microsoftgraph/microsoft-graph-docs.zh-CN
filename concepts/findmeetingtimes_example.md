...
**FindMeetingTimes** 操作使你可以指定条件，如会议日期/时间范围、持续时间、可选或必选的与会者，以及活动性质 (**activityDomain**)。该操作考虑到与会者和组织者正常的工作计划和忙/闲状态，并建议适合参与者和活动类型的时间。例如，在组织者和与会者工作期间经常发生的与工作相关的活动的建议以及在哪里可获得需要的与会者的建议都被列在建议列表的前半部分。
