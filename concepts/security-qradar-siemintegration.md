...
---
# <a name="integrate-microsoft-graph-security-api-alerts-with-ibm-qradar-siem-using-azure-monitor"></a>与 IBM QRadar SIEM 使用 Azure 监视器集成 Microsoft Graph 安全 API 通知

Microsoft Graph 安全提供程序可以通过单个的 REST 终结点。 到[Azure 监视器](https://docs.microsoft.com/en-us/azure/monitoring-and-diagnostics/)，支持连接到多个 SIEM 产品的连接器，可以配置此终结点。 步骤 1 和 2 的本文中的说明引用所有 Azure 监视器连接器的支持通过事件集线器消耗。 本文介绍[QRadar](https://www.ibm.com/us-en/marketplace/ibm-qradar-siem) SIEM 连接器的端到端集成。

此集成过程包含下列步骤：

1. [Azure 事件集线器接收租户的安全警告设置](#step-1-set-up-an-event-hubs-namespace-in-azure-to-receive-security-alerts-for-your-tenant)。
2. [配置 Azure 监视器发送从到事件集线器租户的安全警告](#step-2-configure-azure-monitor-to-send-security-alerts-from-your-tenant-to-the-event-hub)。
3. [下载并安装 QRadar 以使用安全警告](#step-3-download-and-install-the-qradar-to-consume-security-alerts)。

完成这些步骤后，您 IBM QRadar 将使用安全警报所有 Microsoft Graph 集成安全产品为其授权您的租户。 许可的任何新安全产品也将通过此连接发送警报（在同一架构且无需进一步集成）。
...
1. 在您 Azure 订阅中 （可以在"所有服务"下，找到），为资源提供程序注册"microsoft.insights"（Azure 监视器）。  
...
## <a name="step-3-download-and-install-the-qradar-to-consume-security-alerts"></a>步骤 3： 下载并安装 QRadar 以使用安全警告

1. 下载并安装[IBM QRadar](https://www.ibm.com/us-en/marketplace/ibm-qradar-siem)。 **7.2.8 修补程序 7 或更高版本，则需要**从 Microsoft Azure 事件集线器读取事件。
...
3. 最后，按照[配置 QRadar 以收集中使用的 Microsoft Azure 事件集线器协议的 Microsoft Azure 事件集线器事件](https://www.ibm.com/support/knowledgecenter/SS42VS_DSM/t_logsource_microsoft_azure_event_hubs.html)开始显示安全警报中的步骤。
  
 > **注意：** 与 IBM QRadar Microsoft Azure 集成支持[Microsoft Azure DSM 规范](https://www.ibm.com/support/knowledgecenter/SS42VS_DSM/c_dsm_guide_microsoft_azure_DSM_specs.html)中列出的事件。 我们当前正在与 IBM QRadar 添加完整 Microsoft Graph 安全 API 通知的支持。 目前，您将能够接收的 Microsoft 安全图形 API 警报和查看它们在 IBM QRadar 控制台。 您可以使用[DSM 编辑器](https://www.ibm.com/support/knowledgecenter/SS42VS_7.2.8/com.ibm.qradar.doc/c_qradar_adm_dsm_ed_overview.html)启用分析 Microsoft 安全图形 API 通知。  
