...
使用 Windows 更新企业部署服务，Windows Azure AD 租户中的设备部署更新。 如今，部署服务[支持部署](windowsupdates-deployments.md)Windows 10更新和加速安全更新。 本主题重点介绍快速安全更新的部署。 有关部署功能更新的信息，请参阅 [部署功能更新](windowsupdates-deploy-update.md)。

通过更新来替代Windows更新的延迟策略，以便尽快安装更新。 当出现关键安全事件并且需要比正常情况更快速地部署最新更新时，它非常有用。 但是，虽然它可以帮助实现针对特定安全更新的合规性目标，但它并非旨在每月使用一次。 相反，请考虑使用 [更新的合规性截止时间](https://docs.microsoft.com/windows/deployment/update/wufb-compliancedeadlines)。

将加速安全更新部署到设备时，Windows更新会向设备提供最新的适用更新（如果设备尚未收到具有指定发布日期的更新）。 例如，如果将 2021 Windows 10 2021 年 4 月 13 日发布的安全更新部署到当前没有更新的设备，该设备将收到加速更新。 如果设备已具有指定的更新或更新版本，则它不会收到加速更新。
...
* 设备已安装[KB4023057 -](https://support.microsoft.com/topic/kb4023057-update-for-windows-10-update-service-components-fccad0ca-dc10-2e46-9ed1-7e392450fb3a) Windows 10 Update Service 组件的更新 (更新) 。
...
下面是一个查询所有 Windows 10 安全更新的示例，这些安全更新可通过部署服务作为快速更新进行部署。 Microsoft 建议只显示三个最新的更新，因此该示例包括 `$top=3` 。
...
将加速安全更新部署到设备时，Windows更新会提供一个更新，将设备超过指定的最低合规性级别。 根据每个设备扫描和更新时间，某些设备可能会收到更新 (例如，如果有比所需最低合规性级别) 更新更新的更新，但所有设备都符合指定的安全更新合规性标准。 这种提供最新适用更新的行为（由设置为默认值的属性等效项指示）有助于尽可能确保设备安全，并阻止设备在几天后收到快速更新，后跟另一个 `latestSecurity` 定期更新。
...
在将设备添加到部署访问群体 (的成员或排除集合时，会自动向服务注册设备，也就是说，如果 [azureADDevice](/graph/api/resources/windowsupdates-azureaddevice) 对象) 。
...
在最初向部署访问群体分配的所有设备都提供更新后，由于设备连接等因素，并非所有设备都启动或完成了更新。 只要部署仍然存在，它就会继续确保 Windows 更新在重新连接时为分配的设备提供更新。
