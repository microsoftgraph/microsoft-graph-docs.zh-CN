...
了解使用云通信 Api 对 buid 服务应用程序 ([bot](https://microsoftgraph.github.io/microsoft-graph-comms-samples/docs/articles/calls/register-calling-bot.html?q=create%20bot)) 的好处。

### <a name="handle-incoming-calls"></a>处理传入呼叫

当工作人员收到大量业务呼叫且无法回答所有问题时，可能会非常困难。 Bot 可用作前台助理，并通过拒绝似乎像垃圾邮件呼叫那样来处理这些呼叫，然后重定向 (转发) 对不同号码的特定呼叫。
...
无论您是拥有一家大型支持服务还是小型店面，都可能很难处理多个客户请求，尤其是在您没有任何上下文正在尝试提前解决的问题时。 通过 **交互式语音响应** 处理来自客户的传入呼叫 (IVR) 系统，机器人最初将与之交互。
...
- [Microsoft Graph 中的云通信 Api (beta) ](/graph/api/resources/communications-api-overview?view=graph-rest-beta)
