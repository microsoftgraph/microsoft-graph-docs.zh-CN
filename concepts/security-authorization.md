# <a name="authorization-and-the-security-api-in-microsoft-graph"></a>Microsoft Graph 中的授权和安全 API

通过 Microsoft Graph 中的安全 API 可访问的安全数据是很敏感的，它受到权限和 Azure Active Directory (Azure AD) 角色保护。
...
通过 Microsoft Graph 中的安全 API 提供的安全数据是很敏感的，必须由适当的身份验证和授权机制进行保护。 下表列出注册和创建可以访问安全 API 的客户端应用程序的步骤。
...
|租户管理员|向应用程序授予权限。|
|租户管理员|为用户分配角色。|
|应用程序开发人员|以用户身份登录并使用应用程序访问安全 API。|
...
4. 在“**Microsoft Graph 权限**”部分的“**委派权限**”下，选择“**添加**”。 在对话框中，选择所需的权限。 如需查看权限列表，请参阅[安全权限](../concepts/permissions_reference.md#security-permissions)。

    >安全 API 需要 GET 查询的 SecurityEvents.Read.All 作用域和 PATCH/POST 查询的 SecurityEvents.ReadWrite.All 作用域。
...
如需了解更多信息，请参阅[使用 Azure AD v2.0 终结点注册应用](../concepts/auth_register_app_v2.md)。
...
- **应用程序 ID** - 应用程序注册门户中的应用程序 ID。
- **重定向 URL** - 该 URL 发送来自 Azure AD 的身份验证响应。 请使用 http://localhost 或测试客户端 web 应用主页以开始。
...
|[桌面应用 - Android](https://docs.microsoft.com/zh-CN/azure/active-directory/develop/guidedsetups/active-directory-android)|[Microsoft 身份验证库 (MSAL)](http://javadoc.io/doc/com.microsoft.identity.client/msal)|
...
对于不使用任何现有库的应用程序，请参阅[代表用户获取访问权限](../concepts/auth_v2_user.md)。
