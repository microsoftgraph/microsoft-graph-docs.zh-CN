...
Microsoft Graph 提供单一 API 终结点，以便用户能够通过大量资源（如 [user](../api-reference/v1.0/resources/user.md) 和 [message](../api-reference/v1.0/resources/message.md)）访问以人为中心的丰富数据和见解。 现在，可以通过一种方法使用自己的应用数据_**扩展**_ Microsoft Graph。 无需使用外部数据存储，即可向 Microsoft Graph 资源添加自定义属性。 

例如，可能会决定扩展 **user** 资源，让应用处于轻量级水平，并将应用专属的用户配置文件数据存储在 Microsoft Graph 中。 或者，不妨保留应用的现有用户配置文件存储，并仅将应用专属的存储标识符添加到 **user** 资源。

Microsoft Graph 提供两种类型的扩展。请选择最能满足应用需求的扩展类型：
...
使用工作或学校帐户登录时，可以对所有这些资源使用扩展。 此外，使用个人帐户登录时，还可以对 **event**、**post**、**group**、**message**、**contact** 和 **user** 这些资源使用扩展。 
...
创建架构扩展定义时，必须针对 **id** 提供唯一名称。有两个命名选项：
