...
在邮件正文中发送 JSON 更改对象。 如果请求成功，Microsoft Graph 将返回 204 HTTP 状态代码。
...
> **注意：** API 会放弃在创建页面和更新页面请求的[输入 HTML](onenote_input_output_html.md) 中定义的所有 **id** 值。

以下示例显示了段落的生成 ID，以及页面的[输出 HTML](onenote_input_output_html.md) 中的图像。
...
有关权限范围及其工作方式的详细信息，请参阅 [OneNote 权限范围](permissions_reference.md)。
