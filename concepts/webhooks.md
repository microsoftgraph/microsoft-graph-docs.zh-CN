...
订阅的生命周期有限。 应用需要在订阅到期前续订订阅。 否则，需要新建订阅。 有关最长有效期的列表，请参阅[每个资源类型的最长订阅有效期](../api-reference/v1.0/resources/subscription.md#maximum-length-of-subscription-per-resource-type)。
...
`changeType`、`notificationUrl`、`resource` 和 `expirationDateTime` 属性是必需的。 如需属性定义和值，请参阅[订阅资源类型](../api-reference/v1.0/resources/subscription.md)。
...
如果成功，Microsoft Graph 将在正文中返回 `201 Created` 代码和 [subscription](../api-reference/v1.0/resources/subscription.md) 对象。
...
如果成功，Microsoft Graph 将在正文中返回 `200 OK` 代码和 [subscription](../api-reference/v1.0/resources/subscription.md) 对象。 subscription 对象包括新的 `expirationDateTime` 值。
...
| subscriptionExpirationDateTime | [dateTime](http://tools.ietf.org/html/rfc3339) | 订阅的过期时间。 |
