...
---
# <a name="tutorial-manage-access-to-resources-in-active-directory-entitlement-management-using-microsoft-graph-apis"></a>教程：使用 Microsoft Graph API 管理对 Active Directory 权利管理中的资源的访问权限

管理对员工所需的所有资源（如组、应用程序和网站）的访问权限是组织的一项重要功能。 您希望向员工授予高效工作所需的正确访问权限级别，并删除不再需要的访问权限。 [Azure Active Directory (Azure AD) ](/azure/active-directory/governance/entitlement-management-overview) 使用 Microsoft Graph API 进行权利管理，使你能够管理这种类型的访问。

本教程要求你开发代码，为内部用户可以自助请求的市场营销活动创建一个资源包。 请求不需要审批，用户的访问权限将在 30 天后过期。 对于本教程，市场营销活动资源只是单个组的成员身份，但它可能是组、应用程序或 SharePoint Online 网站的集合。

>**注意：** 为了可读性，本教程中显示的响应对象可能会缩短。 

## <a name="prerequisites"></a>先决条件

若要成功完成本教程，请确保你满足以下先决条件：
- Azure AD 权利管理需要特定许可证。 有关详细信息，请参阅许可证 [要求](/azure/active-directory/governance/entitlement-management-overview#license-requirements)。 租户中需要以下许可证：
    - Azure AD Premium P2
    - 企业移动性 + 安全性 (EMS) E5 许可证
- 本教程假定你使用的是 Microsoft Graph 资源管理器，但可以使用 Postman 或创建自己的客户端应用来调用 Microsoft Graph。 若要在本教程中调用 Microsoft Graph API，你需要将帐户与 全局管理员角色和相应权限一同使用。 对于本教程，需要具有 、 和 `User.ReadWrite.All` `Group.ReadWrite.All` `EntitlementManagement.ReadWrite.All` 委派权限。 完成以下步骤以在 Microsoft Graph 资源管理器中设置权限：
    1. 启动 [Microsoft Graph 浏览器](https://developer.microsoft.com/graph/graph-explorer)。
    2. 选择 **"使用 Microsoft 登录** "，然后使用 Azure AD 全局管理员帐户登录。 成功登录后，可以在左侧窗格中看到用户帐户详细信息。
    3. 选择用户帐户详细信息右边的设置图标，然后选择 **"选择权限"。**

        ![选择 Microsoft Graph 权限](./images/tutorial-access-package-api/set-permissions.png)
        
    4. 滚动权限列表，展开组 (`Group` **2**) ，选择 **Group.ReadWrite.All** 权限。 进一步向下滚动权限列表，展开"用户 (`User` **8) "，** 然后选择 **User.ReadWrite.All** 权限。

        ![搜索用户、组和权利管理权限](./images/tutorial-access-package-api/set-user-permission.png)
    
    5. 选择 **"** 同意"，然后选择" **接受** "以接受权限的同意。 无需代表组织同意这些权限。
    6. 搜索权限，展开 `EntitlementManagement` **EntitlementManagement (2) ，** 选择 **Entitlement.ReadWrite.All** 权限，然后选择"同意 **"。** 由于此权限需要管理员同意，并且需要你在此教程创建的用户帐户，因此你必须代表你的组织选择"同意 **"。**
...
在此步骤中，您将在目录中创建一个名为 **"** 营销资源"的组，该组是权利管理的目标资源。 您还可以创建设置为内部请求程序的用户帐户。

### <a name="create-a-user-account"></a>创建用户帐户

对于本教程，您将创建一个用户帐户，用于请求访问包中的资源的访问权限。 进行这些调用时，请 `contoso.onmicrosoft.com` 更改为租户的域名。 可以在 Azure Active Directory 概述页面上找到租户信息。 记录返回的 **id** 属性的值，以在本教程的稍后部分使用。
...
在本教程中，你将创建一个名为 **"** 营销资源"的组，该组是权利管理的目标资源。 如果已有一个组，可以使用现有组。 记录返回的 **id** 属性的值，以在本教程的稍后部分使用。 
...
访问 *包* 是团队或项目需要并受策略管理的资源捆绑包。 访问包在称为目录的容器中定义。 目录可以引用访问包中使用的资源，如组、应用和网站。 在此步骤中，您将 **在常规目录中** 创建市场营销活动访问包。 如果您有其他目录，请使用下一节中的名称。

### <a name="get-the-catalog-identifier"></a>获取目录标识符

若要向目录添加资源，必须先获取其标识符。 如果使用的是常规目录，请运行以下请求获取其标识符。 如果使用的是其他 calalog，则请求中的筛选器值更改为目录名称。 记录返回的 **id** 属性的值，以在本教程的稍后部分使用。
...
在请求 **中，提供** 你使用的目录的 ID。 记录组 **目录资源的 id** 属性的值。
...
访问包将用户分配给资源的角色。 组的典型角色是成员角色。 其他资源（如 SharePoint Online 网站和应用程序）可能有许多角色。 访问包中使用的组的典型角色是成员角色。 在本教程稍后向访问包添加资源角色时，将需要成员角色。 

在请求中，使用目录 **的 ID** 和所记录的目录中组资源的 **ID，** 获取 Member 资源角色的 **originId。** 记录 **originId** 属性的值，以在本教程的稍后部分使用。
...
如果成功，将返回一个值，该值代表该组的 Member 角色。 如果未返回任何角色，请检查 **目录的 ID** 值和访问包资源。

### <a name="create-the-access-package"></a>创建访问包

此时，您具有一个包含组资源的目录，并且知道您将在访问包中使用组成员的资源角色。 下一步是创建访问包。 获取访问包后，您可以向该包添加资源角色，并创建一个策略，说明用户如何请求访问该资源角色。 使用 **之前录制** 的目录的 ID 创建访问包。 记录访问包的 **ID，** 以在本教程的稍后部分使用。
...
将组资源的 Member 角色添加到访问包。 在请求中，提供 **访问** 包的 ID。 在请求正文中，提供 accessPackageResource 的组目录资源的 **ID，** 并提供以前记录的 Member 角色的 **originId。**
...
现在，你已创建访问包并添加了资源和角色，可以通过创建访问包策略决定谁可以访问它。 在本教程中，将启用您创建的 **Requestor1** 帐户以请求访问访问包中的资源。 对于此任务，需要以下值：
...
若要请求访问包中的资源访问权限，需要提供以下值：
- **为** **targetId** 属性的值创建的 **Requestor1** 用户帐户的 ID
-  **assignmentPolicyId** 属性值的分配策略 ID
-  **accessPackageId 属性值的访问包 ID**

在响应中，你可以看到接受 **的状态** 和已提交 **的状态**。 记录返回 **的 id** 属性的值，以稍后获取请求的状态。

如果尚未这样做，请注销你在 Microsoft Graph 资源管理器中使用的管理员帐户。 登录到您创建的 **Requestor1** 用户帐户。 如果密码是首次登录，将要求您更改密码。
...
使用请求 **的 id** 属性的值获取其当前状态。 在响应中，你可以看到状态更改为"已实现"，状态更改为"已送达 **"。** 
...
您还可以使用创建的访问包策略 **的 ID，** 以查看资源已分配给 **Requestor1** 用户帐户。
...
在此步骤中，删除所做的更改并删除 **市场营销活动** 访问包。

### <a name="remove-an-access-package-assignment"></a>删除访问包分配

必须先删除访问包的任何分配，然后才能将其删除。 使用 **之前** 记录的分配请求的 ID 将其删除。
...
- [使用 Azure AD 权利管理 API](/graph/api/resources/entitlementmanagement-root?view=graph-rest-beta)
