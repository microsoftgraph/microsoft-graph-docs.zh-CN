---
title: OneNote API 中 OneNote 页面上的输入和输出 HTML
description: 了解在创建或更新页面内容或分析返回的页面内容时如何使用输入和输出 HTML 的主体元素和属性。
...
---
# <a name="input-and-output-html-on-onenote-pages"></a>OneNote 页面中的输入和输出 HTML
...
|:------|:------|
| data-id | 元素的引用。<br/><br/>用于[更新页面内容](onenote-update-page.md)。 |
| id | 元素的唯一生成的 ID。 使用 `includeIDs=true` 查询选项时，由 [对页面的 *content* 终结点的 GET 请求](/graph/api/page-get)返回。<br/><br/>用于[更新页面内容](onenote-update-page.md)。 |
| style | div 的位置和大小属性。 |
...
> [!NOTE]
> div 的内容已被移至父 div 且嵌套 `<div>` 标记已被删除。 如果 div 定义了任何语义信息，例如 **data-id**（示例：`<div data-id="keep-me">`），则保留该 div。
...
| alt | 为此图像提供的替换文字。 |
| data-id | 元素的引用。<br/><br/>用于[更新页面内容](onenote-update-page.md)。 |
| data-render-src |需要 **data-render-src** 或 **src**。<br/><br/>在 OneNote 页上显示为位图图像的网页：<br/><br/> - `data-render-src="https://..."` 表示公共 URL。<br/><br/> - `data-render-src="name:BlockName"` 表示[多部分请求](/graph/api/section-post-pages#example)的“演示文稿”块中的图像部分。<br/><br/>如果网页比 OneNote 页能够忠实呈现的内容更为复杂，或者页面需要登录凭据，则此方法很有用。|
...
输出 **img** 元素包含图像文件资源的终结点和图像类型，如下所示。 可以发出单独的[图像资源终结点的 GET 请求](/graph/api/resource-get)，以检索其二进制内容。
...
> [!NOTE]
> 也可以 [使用 **object** 元素附加视频文件](onenote-images-files.md#adding-files)。
...
|输入属性|说明|
|:------|:------|
| data | 必需。表示 [多部分请求](/graph/api/section-post-pages#example) 中文件的部件的名称。 |
...
输出 **object** 元素包含链接到页面中的文件资源的终结点，如下所示。 可以发出单独的[文件资源终结点的 GET 请求](/graph/api/resource-get)，以检索其二进制内容。
...
> [!NOTE]
> 在输入 html 中不支持使用表格样式属性中的“**border**”属性。
...
> [!NOTE]
> [创建页面](onenote-create-page.md)或 [获取页面元数据](/graph/api/page-get)时，API 会返回 **contentUrl** 属性中的页面的 *content* 终结点 URL。
