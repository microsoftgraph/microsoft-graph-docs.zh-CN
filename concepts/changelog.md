...
| **更改类型** | **版本**   | **说明**                          |
| :-------------- | :------------ | :--------------------------------------- |
|添加项 |v1.0| 引入的新资源类型[团队](/graph/api/resources/team?view=graph-rest-1.0)。|
|添加项 |v1.0| 引入的新资源类型[通道](/graph/api/resources/channel?view=graph-rest-1.0)。|
|添加项 |v1.0| 引入的新资源类型[teamsApp](/graph/api/resources/teamsapp?view=graph-rest-1.0)。|
|添加项 |v1.0| 引入的新资源类型[teamsAppDefinition](/graph/api/resources/teamsappdefinition?view=graph-rest-1.0)。|
|添加项 |v1.0| 引入的新资源类型[teamsAppInstallation](/graph/api/resources/teamsappinstallation?view=graph-rest-1.0)。|
|添加项 |v1.0| 引入的新资源类型[teamsAsyncOperation](/graph/api/resources/teamsasyncoperation?view=graph-rest-1.0)。 |
|添加项 |v1.0| 引入新复杂类型[teamGuestSettings](/graph/api/resources/teamguestsettings?view=graph-rest-1.0)。 |
|添加项 |v1.0| 引入新复杂类型[teamMemberSettings](/graph/api/resources/teammembersettings?view=graph-rest-1.0)。 |
|添加项 |v1.0| 引入新复杂类型[teamMessagingSettings](/graph/api/resources/teammessagingsettings?view=graph-rest-1.0)。 |
|添加项 |v1.0| 引入新复杂类型[teamFunSettings](/graph/api/resources/teamfunsettings?view=graph-rest-1.0)。 |
|添加项 |v1.0| 引入的[克隆团队](/graph/api/team-clone?view=graph-rest-1.0)的新操作。 |
|添加项 |v1.0| 引入的新操作[存档团队](/graph/api/team-archive?view=graph-rest-1.0)。|
|添加项 |v1.0| 引入的新操作[Unarchive 团队](/graph/api/team-unarchive?view=graph-rest-1.0)。 |
|添加项         | Beta          | 添加到[克隆团队](/graph/api/team-clone?view=graph-rest-beta)权限支持的应用程序。 |
|添加项 |beta| 引入[/teams/ {id} / installedApps](/graph/api/resources/teamsappinstallation?view=graph-rest-beta)，用于将取代 /teams/ {id} / 有效负载中存在一些差异的应用程序。 |
|添加项 |beta| 引入[/appCatalogs/teamsApps / {id} / appDefinition](/graph/api/resources/teamsappdefinition?view=graph-rest-beta)，用于将取代[/appCatalogs/teamsApps / {id}](/graph/api/resources/teamsapp?view=graph-rest-beta)的版本属性。 |
|更改   |beta| 重命名[/appCatalogs/teamsApps](/graph/api/resources/teamsapp?view=graph-rest-beta)从 teamsCatalogApp 到 teamsApp 的类型。 |
|更改   |beta| 重命名的[/appCatalogs/teamsApps](/graph/api/resources/teamsapp?view=graph-rest-beta)从 teamsCatalogAppDistributionMethod 到 teamsAppDistributionMethod distributionMethod 属性类型  |
|删除 |beta| teamsCatalogAppDistributionMethod 已重命名为 teamsAppDistributionMethod  |
|添加项 |beta| 引入[/teams/ {id} / installedApps](/graph/api/resources/teamsappinstallation?view=graph-rest-beta)，用于将取代 /teams/ {id} / 有效负载中存在一些差异的应用程序。 |
|添加项 |beta| 添加到[teamsTab](/graph/api/resources/teamstab?view=graph-rest-beta)的 displayName 属性 |
|添加项 |beta| 添加到[teamsTab](/graph/api/resources/teamstab?view=graph-rest-beta)的 messageId 属性 |
|添加项 |beta| 添加到[teamsTab](/graph/api/resources/teamstab?view=graph-rest-beta)的 teamsApp 属性 |
...
| **更改类型** | **版本**   | **说明**                          |
| :-------------- | :------------ | :--------------------------------------- |
|添加项 |beta| 引入[riskyUsers API](/graph/api/resources/riskyuser?view=graph-rest-beta)，代表 Azure AD 用户风险，在检测到的 Azure AD 身份保护。 |
...
| **更改类型** | **版本**   | **说明**                          |
| :-------------- | :------------ | :--------------------------------------- |
|更改   |beta| 重命名属性`conditionalAccessPolicies`到`appliedConditionalAccessPolicy`。|
|添加项 |beta| 引入额外的风险属性中[登录 API](/graph/api/resources/signin?view=graph-rest-beta)，包括`riskDetail`， `riskLevelAggregated`， `riskLevelDuringSignIn`， `riskEventTypes`，和`riskState`。|
|添加项 |beta| 引入中[登录 API](/graph/api/resources/signin?view=graph-rest-beta)，其他登录属性包括`authenticationProcessingDetails`， `originalRequestID`， `isInteractive`， `tokenIssuerName`， `tokenIssuerType`， `correlationId`，和`processingTimeinMilliseconds`。|
|删除   |beta| 删除属性`isRisky`。|
...
| **更改类型** | **版本** | **说明**                  |
|:------------|:--------|:-----------------------------------------|
| 添加项    | Beta   | [DirectoryObject](/graph/api/directoryobject-delta?view=graph-rest-beta)的添加了的[增量查询](delta-query-overview.md)功能 |
| 更改      | v1.0 和 beta  | 要仅在[用户](/graph/api/user-delta?view=graph-rest-1.0)和[组](/graph/api/group-delta?view=graph-rest-1.0)的 JSON 响应中返回更改的属性的替代行为。 |
| 添加项    | v1.0   | 添加[directoryRole](/graph/api/resources/directoryrole?view=graph-rest-1.0)支持[修订使用增量查询](delta-query-overview.md)的[增量](/graph/api/directoryrole-delta?view=graph-rest-1.0)函数。 |
...
|更改类型|版本|说明|
|:---|:---|:---|
|添加项|v1.0|添加到[windows10GeneralConfiguration](/graph/api/resources/intune-deviceconfig-windows10generalconfiguration?view=graph-rest-1.0)实体**tenantLockdownRequireNetworkDuringOutOfBoxExperience**属性|
|添加项|v1.0|**V12_0**属性添加到[iosMinimumOperatingSystem](/graph/api/resources/intune-apps-iosminimumoperatingsystem?view=graph-rest-1.0)复杂类型|
|添加项|beta|添加到[deviceManagement](/graph/api/resources/intune-shared-devicemanagement?view=graph-rest-beta)实体**lastReportAggregationDateTime**属性|
|添加项|beta|添加的新实体：<br/>[intuneBrandingProfile](/graph/api/resources/intune-wip-intunebrandingprofile?view=graph-rest-beta)<br/>|
|添加项|beta|新增了复杂类型：<br/>[deviceAndAppManagementAssignedRoleIds](/graph/api/resources/intune-rbac-deviceandappmanagementassignedroleids?view=graph-rest-beta)<br/>|
|添加项|beta|添加的新枚举类型：<br/>[applicationGuardEnabledOptions](/graph/api/resources/intune-deviceconfig-applicationguardenabledoptions?view=graph-rest-beta)<br/>[autoRestartNotificationDismissalMethod](/graph/api/resources/intune-deviceconfig-autorestartnotificationdismissalmethod?view=graph-rest-beta)<br/>[meteredConnectionLimitType](/graph/api/resources/intune-deviceconfig-meteredconnectionlimittype?view=graph-rest-beta)<br/>|
|添加项|beta|[DeviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta)上添加[enableLegacyPcManagement](/graph/api/intune-deviceconfig-devicemanagement-enablelegacypcmanagement?view=graph-rest-beta)操作 |
|添加项|beta|[WindowsUpdateForBusinessConfiguration](/graph/api/resources/intune-deviceconfig-windowsupdateforbusinessconfiguration?view=graph-rest-beta)上添加[extendFeatureUpdatesPause](/graph/api/intune-deviceconfig-windowsupdateforbusinessconfiguration-extendfeatureupdatespause?view=graph-rest-beta)操作 |
|添加项|beta|[WindowsUpdateForBusinessConfiguration](/graph/api/resources/intune-deviceconfig-windowsupdateforbusinessconfiguration?view=graph-rest-beta)上添加[extendQualityUpdatesPause](/graph/api/intune-deviceconfig-windowsupdateforbusinessconfiguration-extendqualityupdatespause?view=graph-rest-beta)操作 |
|添加项|beta|[WindowsAutopilotDeviceIdentity](/graph/api/resources/intune-enrollment-windowsautopilotdeviceidentity?view=graph-rest-beta)上添加[unassignUserFromDevice](/graph/api/intune-enrollment-windowsautopilotdeviceidentity-unassignuserfromdevice?view=graph-rest-beta)操作 |
|添加项|beta|[DeviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta)上添加[getAssignedRoleIdsForLoggedInUser](/graph/api/intune-rbac-devicemanagement-getassignedroleidsforloggedinuser?view=graph-rest-beta)函数 |
|添加项|beta|对[用户](/graph/api/resources/intune-devices-user?view=graph-rest-beta)添加[getManagedDevicesWithAppFailures](/graph/api/intune-troubleshooting-user-getmanageddeviceswithappfailures?view=graph-rest-beta)函数 |
|添加项|beta|[ReportRoot](/graph/api/resources/intune-deviceconfig-reportroot?view=graph-rest-beta)上添加[managedDeviceEnrollmentAbandonmentSummary](/graph/api/intune-troubleshooting-reportroot-manageddeviceenrollmentabandonmentsummary?view=graph-rest-beta)函数 |
|添加项|beta|[ReportRoot](/graph/api/resources/intune-deviceconfig-reportroot?view=graph-rest-beta)上添加[managedDeviceEnrollmentAbandonmentDetails](/graph/api/intune-troubleshooting-reportroot-manageddeviceenrollmentabandonmentdetails?view=graph-rest-beta)函数 |
|删除|beta|从[androidForWorkCertificateProfileBase](/graph/api/resources/intune-deviceconfig-androidforworkcertificateprofilebase?view=graph-rest-beta)实体中删除**subjectAlternativeNameType**属性|
|添加项|beta|添加到[androidForWorkPkcsCertificateProfile](/graph/api/resources/intune-deviceconfig-androidforworkpkcscertificateprofile?view=graph-rest-beta)实体**subjectAlternativeNameType**属性|
|添加项|beta|添加到[androidForWorkScepCertificateProfile](/graph/api/resources/intune-deviceconfig-androidforworkscepcertificateprofile?view=graph-rest-beta)实体中的**certificateStore**、 **customSubjectAlternativeNames**和**subjectAlternativeNameType**属性|
|删除|beta|从[androidWorkProfileCertificateProfileBase](/graph/api/resources/intune-deviceconfig-androidworkprofilecertificateprofilebase?view=graph-rest-beta)实体中删除**subjectAlternativeNameType**属性|
|添加项|beta|添加到[androidWorkProfilePkcsCertificateProfile](/graph/api/resources/intune-deviceconfig-androidworkprofilepkcscertificateprofile?view=graph-rest-beta)实体**subjectAlternativeNameType**属性|
|添加项|beta|添加到[androidWorkProfileScepCertificateProfile](/graph/api/resources/intune-deviceconfig-androidworkprofilescepcertificateprofile?view=graph-rest-beta)实体中的**certificateStore**、 **customSubjectAlternativeNames**和**subjectAlternativeNameType**属性|
|添加项|beta|添加到[deviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta)实体**legacyPcManangementEnabled**属性|
|删除|beta|从[managedAppProtection](/graph/api/resources/intune-mam-managedappprotection?view=graph-rest-beta)实体中删除**pinRequiredOnLaunchInsteadOfBiometric**属性|
|添加项|beta|添加到[managedDeviceMobileAppConfiguration](/graph/api/resources/intune-apps-manageddevicemobileappconfiguration?view=graph-rest-beta)实体**roleScopeTagIds**属性|
|添加项|beta|添加到[windows10EndpointProtectionConfiguration](/graph/api/resources/intune-deviceconfig-windows10endpointprotectionconfiguration?view=graph-rest-beta)实体**applicationGuardEnabledOptions**属性|
|添加项|beta|添加到[windows10EnrollmentCompletionPageConfiguration](/graph/api/resources/intune-onboarding-windows10enrollmentcompletionpageconfiguration?view=graph-rest-beta)实体**selectedMobileAppIds**属性|
|添加项|beta|添加**engagedRestartDeadlineInDays**、 **engagedRestartSnoozeScheduleInDays**、 **engagedRestartTransitionScheduleInDays**、 **autoRestartNotificationDismissal**、 **scheduleRestartWarningInHours**和**scheduleImminentRestartWarningInMinutes**到[windowsUpdateForBusinessConfiguration](/graph/api/resources/intune-deviceconfig-windowsupdateforbusinessconfiguration?view=graph-rest-beta)实体的属性|
|添加项|beta|添加到[windowsWifiConfiguration](/graph/api/resources/intune-deviceconfig-windowswificonfiguration?view=graph-rest-beta)实体中的**preSharedKey**和**meteredConnectionLimit**属性|
|添加项|beta|添加到[deviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta)实体中的**intuneBrandingProfiles**导航属性|
|添加项|beta|**V6_0**、 **v7_0**、 **v7_1**、 **v8_0**、 **v8_1**和**v9_0**属性添加到[androidMinimumOperatingSystem](/graph/api/resources/intune-apps-androidminimumoperatingsystem?view=graph-rest-beta)复杂类型|
|添加项|beta|**V12_0**属性添加到[iosMinimumOperatingSystem](/graph/api/resources/intune-apps-iosminimumoperatingsystem?view=graph-rest-beta)复杂类型|
|删除|beta|从[win32LobAppPowerShellScriptDetection](/graph/api/resources/intune-apps-win32lobapppowershellscriptdetection?view=graph-rest-beta)复杂类型中移除**runAsLoggedOnUser**属性|
|添加项|beta|**LastUpdateDateTime**属性添加到[osVersionCount](/graph/api/resources/intune-devices-osversioncount?view=graph-rest-beta)复杂类型|
|添加项|beta|**LastUpdateDateTime**属性添加到[windowsMalwareCategoryCount](/graph/api/resources/intune-devices-windowsmalwarecategorycount?view=graph-rest-beta)复杂类型|
|添加项|beta|**LastUpdateDateTime**属性添加到[windowsMalwareExecutionStateCount](/graph/api/resources/intune-devices-windowsmalwareexecutionstatecount?view=graph-rest-beta)复杂类型|
|添加项|beta|**LastUpdateDateTime**属性添加到[windowsMalwareNameCount](/graph/api/resources/intune-devices-windowsmalwarenamecount?view=graph-rest-beta)复杂类型|
|添加项|beta|**LastUpdateDateTime**属性添加到[windowsMalwareStateCount](/graph/api/resources/intune-devices-windowsmalwarestatecount?view=graph-rest-beta)复杂类型|
...
| 添加项 | beta | 添加[privilegedroleassignmentrequest](/graph/api/resources/privilegedroleassignmentrequest?view=graph-rest-beta)实体和下列方法和操作：<br> [列出](/graph/api/privilegedroleassignmentrequest-list?view=graph-rest-beta) <br> [Create](/graph/api/privilegedroleassignmentrequest-post?view=graph-rest-beta) <br> [Cancel](/graph/api/privilegedroleassignmentrequest-cancel?view=graph-rest-beta) <br> [My](/graph/api/privilegedroleassignmentrequest-my?view=graph-rest-beta) |
| 添加项 | beta | 添加了的[更新](/graph/api/privilegedrolesettings-update?view=graph-rest-beta) [privilegedRoleSettings](/graph/api/resources/privilegedrolesettings?view=graph-rest-beta)|
| 删除 |beta| 弃用的[自我激活角色分配](/graph/api/privilegedrole_selfactivate?view=graph-rest-beta)|

### <a name="microsoft-teams-apis"></a>Microsoft Teams API

| **更改类型** | **版本**   | **说明**                          |
| :-------------- | :------------ | :--------------------------------------- |
|添加项|beta|添加到[存档团队](/graph/api/team-archive?view=graph-rest-beta)和[unarchive 团队](/graph/api/team-unarchive?view=graph-rest-beta)Api 的应用程序权限支持。|

### <a name="outlook-contacts"></a>Outlook 联系人

| **更改类型** | **版本** | **说明**                          |
| :-------------- | :---------- | :--------------------------------------- |
| 删除         | v1.0        | 这是对文档的更正： 从[联系人](/graph/api/resources/contact?view=graph-rest-1.0)实体主题删除**标志**属性。 属性已从不**联系人**实体中可用。|

### <a name="reports-apis"></a>报表 API
| 更改类型 | 版本 | 说明                              |
|:------------|:--------|:-----------------------------------------|
| 添加项    | v1.0    | 添加到[getSharePointSiteUsageDetail](/graph/api/reportroot-getsharepointsiteusagedetail?view=graph-rest-1.0)**网站 ID**属性。 |

## <a name="september-2018"></a>年 9 月 2018

### <a name="calls-and-online-meetings-api"></a>呼叫和联机会议 API

| **更改类型** | **版本** | **说明**                          |
| :-------------- | :---------- | :--------------------------------------- |
| 更改          | Beta        | [应用程序](/graph/api/resources/application?view=graph-rest-beta)资源已更新以添加呼叫集合。 |
| 更改          | Beta        | [操作](/graph/api/resources/operation?view=graph-rest-beta)资源已更新以支持长时间运行呼叫和会议 Api。 |
| 添加项        | Beta        | 添加用于管理音频/视频呼叫 （最初，在 Microsoft 团队中），包括 Api[创建呼叫](/graph/api/application-post-calls?view=graph-rest-beta)，[呼叫](/graph/api/resources/call?view=graph-rest-beta)资源[检索呼叫](/graph/api/call-get?view=graph-rest-beta)，[删除 （挂断） 呼叫](/graph/api/call-delete?view=graph-rest-beta)，[应答呼叫](/graph/api/call-answer?view=graph-rest-beta)，[拒绝呼叫](/graph/api/call-reject?view=graph-rest-beta)，[重定向呼叫](/graph/api/call-redirect?view=graph-rest-beta)，并且[将呼叫转接](/graph/api/call-transfer?view=graph-rest-beta)。 我们还添加了 Api 支持[IVR 方案](/graph/api/resources/calls-api-ivr-overview?view=graph-rest-beta)：[播放提示](/graph/api/call-playprompt?view=graph-rest-beta)、[记录呼叫](/graph/api/call-record?view=graph-rest-beta)、[取消处理媒体](/graph/api/call-cancelmediaprocessing?view=graph-rest-beta)，和[订阅触摸音调通知](/graph/api/call-subscribetotone?view=graph-rest-beta)。 |
| 添加项        | Beta        | 用于管理在音频/视频呼叫和会议，包括[检索参与者对象](/graph/api/participant-get?view=graph-rest-beta)、[配置为参与者音频混音器](/graph/api/participant-configuremixer?view=graph-rest-beta)、 静音[一个](/graph/api/participant-mute?view=graph-rest-beta)或[所有参与者添加[参与者](/graph/api/resources/call?view=graph-rest-beta)资源和 Api](/graph/api/participant-muteall?view=graph-rest-beta)参与者、 呼叫/会议和呼叫/会议[邀请参与者](/graph/api/participant-invite?view=graph-rest-beta)中[检索的参与者的列表](/graph/api/call-list-participants?view=graph-rest-beta)。 |
| 添加项        | Beta        | 添加了的 Api 的应用程序管理和参与呼叫和会议，包括[共享内容](/graph/api/call-changescreensharingrole?view=graph-rest-beta)、[静音和取消静音本身](/graph/api/call-unmute?view=graph-rest-beta)和[更新与呼叫相关联的元数据](/graph/api/call-updatemetadata?view=graph-rest-beta)的能力。 |
| 添加项        | Beta        | 用于管理在多方对话中，包括[创建音频路由组](/graph/api/call-post-audioroutinggroups?view=graph-rest-beta)、[检索这些列表](/graph/api/audioroutinggroup-get?view=graph-rest-beta)和[更新](/graph/api/audioroutinggroup-update?view=graph-rest-beta)的参与者之间的专用音频路由中添加[音频路由组](/graph/api/resources/audioroutinggroup?view=graph-rest-beta)资源和 Api 和[删除](/graph/api/audioroutinggroup-delete?view=graph-rest-beta)它们。 |
| 添加项        | Beta        | 用于管理的 Microsoft 团队联机会议中添加的[联机会议](/graph/api/resources/audioroutinggroup?view=graph-rest-beta)资源和 Api。 最初，没有只有一个 API 的联机会议，到[检索联机会议对象](/graph/api/onlinemeeting-get?view=graph-rest-beta)。 也已添加会议 （例如电话拨入式 URL、 密码和电话号码） 相关联的[音频会议信息](/graph/api/resources/audioconferencing?view=graph-rest-beta)的相关的资源。 |
| 添加项        | Beta        | 多个呼叫和会议 Api 的执行时间才能完成，因此这些长时间运行操作的资源添加：[呼叫特有的操作](/graph/api/resources/commsoperation?view=graph-rest-beta)、[播放音频提示](/graph/api/resources/playpromptoperation?view=graph-rest-beta)，并[记录](/graph/api/resources/recordoperation?view=graph-rest-beta)。  |

### <a name="dynamics-365-business-central-api"></a>Dynamics 365 业务中央 API

| **更改类型** | **版本**   | **说明**                          |
| :-------------- | :------------ | :--------------------------------------- |
| 添加项        | Beta          | 添加了的财务 Api Dynamics 365 业务中央。 有关详细信息，请参阅[财务 API 参考 （英文）](/graph/api/resources/dynamics-graph-reference?view=graph-rest-beta)

### <a name="microsoft-graph-data-connect"></a>Microsoft Graph 数据连接

| **更改类型** | **版本**   | **说明**                          |
| :-------------- | :------------ | :--------------------------------------- |
|添加项         | 不适用| 引入访问批量的 Office 365 数据的能力。 有关详细信息，请参阅[Microsoft Graph 数据连接 (preview)](data-connect-overview.md)。|
...
|添加项|beta|[OfficeClientConfiguration](/graph/api/resources/intune-cirrus-officeclientconfiguration?view=graph-rest-beta)上添加[分配](/graph/api/intune-cirrus-officeclientconfiguration-assign?view=graph-rest-beta)操作 |
|添加项|beta|[OfficeClientConfiguration](/graph/api/resources/intune-cirrus-officeclientconfiguration?view=graph-rest-beta)集上添加**updatePrioritie**操作 |
|添加项|beta|添加的新实体：<br/>[deviceConfigurationConflictSummary](/graph/api/resources/intune-deviceconfig-deviceconfigurationconflictsummary?view=graph-rest-beta)<br/>[importedWindowsAutopilotDeviceIdentityUpload](/graph/api/resources/intune-enrollment-importedwindowsautopilotdeviceidentityupload?view=graph-rest-beta)<br/>[win32LobApp](/graph/api/resources/intune-apps-win32lobapp?view=graph-rest-beta)<br/>|
...
|添加项|beta|添加的新枚举类型：<br/>[administratorConfiguredDeviceComplianceState](/graph/api/resources/intune-deviceconfig-administratorconfigureddevicecompliancestate?view=graph-rest-beta)<br/>[importedWindowsAutopilotDeviceIdentityUploadStatus](/graph/api/resources/intune-enrollment-importedwindowsautopilotdeviceidentityuploadstatus?view=graph-rest-beta)<br/>[microsoftStoreForBusinessPortalSelectionOptions](/graph/api/resources/intune-onboarding-microsoftstoreforbusinessportalselectionoptions?view=graph-rest-beta)<br/>[win32LobAppDetectionOperator](/graph/api/resources/intune-apps-win32lobappdetectionoperator?view=graph-rest-beta)<br/>[win32LobAppFileSystemDetectionType](/graph/api/resources/intune-apps-win32lobappfilesystemdetectiontype?view=graph-rest-beta)<br/>[win32LobAppMsiPackageType](/graph/api/resources/intune-apps-win32lobappmsipackagetype?view=graph-rest-beta)<br/>[win32LobAppRegistryDetectionType](/graph/api/resources/intune-apps-win32lobappregistrydetectiontype?view=graph-rest-beta)<br/>[win32LobAppReturnCodeType](/graph/api/resources/intune-apps-win32lobappreturncodetype?view=graph-rest-beta)<br/>[windows10AppsUpdateRecurrence](/graph/api/resources/intune-deviceconfig-windows10appsupdaterecurrence?view=graph-rest-beta)<br/>[windowsAppStartLayoutTileSize](/graph/api/resources/intune-deviceconfig-windowsappstartlayouttilesize?view=graph-rest-beta)<br/>[windowsAutopilotProfileAssignmentDetailedStatus](/graph/api/resources/intune-enrollment-windowsautopilotprofileassignmentdetailedstatus?view=graph-rest-beta)<br/>|
|添加项|beta|[ManagedDevice](/graph/api/resources/intune-devices-manageddevice?view=graph-rest-beta)上添加**overrideComplianceState**操作 |
|添加项|beta|[DeviceConfiguration](/graph/api/resources/intune-deviceconfig-deviceconfiguration?view=graph-rest-beta)集上添加**getTargetedUsersAndDevices**操作 |
|添加项|beta|[ImportedWindowsAutopilotDeviceIdentityUpload](/graph/api/resources/intune-enrollment-importedwindowsautopilotdeviceidentityupload?view=graph-rest-beta)上添加[autopilotDeviceStream](/graph/api/intune-enrollment-importedwindowsautopilotdeviceidentityupload-autopilotdevicestream?view=graph-rest-beta)函数 |
|添加项|beta|添加到[androidCompliancePolicy](/graph/api/resources/intune-deviceconfig-androidcompliancepolicy?view=graph-rest-beta)实体**restrictedApps**属性|
|添加项|beta|添加到[androidDeviceOwnerEnrollmentProfile](/graph/api/resources/intune-androidforwork-androiddeviceownerenrollmentprofile?view=graph-rest-beta)实体**tokenCreationDateTime**属性|
|删除|beta|从[androidForWorkCompliancePolicy](/graph/api/resources/intune-deviceconfig-androidforworkcompliancepolicy?view=graph-rest-beta)实体中删除**restrictedApps**属性|
|删除|beta|从[androidWorkProfileCompliancePolicy](/graph/api/resources/intune-deviceconfig-androidworkprofilecompliancepolicy?view=graph-rest-beta)实体中删除**restrictedApps**属性|
|更改|beta|更改[appleVpnConfiguration](/graph/api/resources/intune-deviceconfig-applevpnconfiguration?view=graph-rest-beta)实体上的以下属性：<br/>从**enablePerApp**所需为可选<br/>|
|添加项|beta|添加到[defaultManagedAppProtection](/graph/api/resources/intune-mam-defaultmanagedappprotection?view=graph-rest-beta)实体中的**disableProtectionOfManagedOutboundOpenInData**和**protectInboundDataFromUnknownSources**属性|
|添加项|beta|添加到[deviceAppManagement](/graph/api/resources/intune-shared-deviceappmanagement?view=graph-rest-beta)实体**microsoftStoreForBusinessPortalSelection**属性|
|添加项|beta|添加到[iosCompliancePolicy](/graph/api/resources/intune-deviceconfig-ioscompliancepolicy?view=graph-rest-beta)实体**passcodeMinutesOfInactivityBeforeScreenTimeout**属性|
|添加项|beta|添加到[iosEasEmailProfileConfiguration](/graph/api/resources/intune-deviceconfig-ioseasemailprofileconfiguration?view=graph-rest-beta)实体**useOAuth**属性|
|添加项|beta|添加**kioskModeBlockVolumeButtons**、 **classroomForceRequestPermissionToLeaveClasses**、 **keychainBlockCloudSync**、 **pkiBlockOTAUpdates**、 **privacyForceLimitAdTracking**、 **enterpriseBookBlockBackup**， **enterpriseBookBlockMetadataSync**、 **airPrintBlocked**、 **airPrintBlockCredentialsStorage**、 **airPrintForceTrustedTLS**、 **airPrintBlockiBeaconDiscovery**、 **blockSystemAppRemoval**和到[iosGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-iosgeneraldeviceconfiguration?view=graph-rest-beta)实体**vpnBlockCreation**属性|
|添加项|beta|添加到[iosManagedAppProtection](/graph/api/resources/intune-mam-iosmanagedappprotection?view=graph-rest-beta)实体中的**disableProtectionOfManagedOutboundOpenInData**和**protectInboundDataFromUnknownSources**属性|
|添加项|beta|添加到[macOSCompliancePolicy](/graph/api/resources/intune-deviceconfig-macoscompliancepolicy?view=graph-rest-beta)实体**gatekeeperAllowedAppSource**属性|
|添加项|beta|添加到[macOSGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-macosgeneraldeviceconfiguration?view=graph-rest-beta)实体**keychainBlockCloudSync**、 **airPrintBlocked**、 **airPrintForceTrustedTLS**和**airPrintBlockiBeaconDiscovery**属性|
|添加项|beta|添加到[managedAppRegistration](/graph/api/resources/intune-mam-managedappregistration?view=graph-rest-beta)实体中的**deviceModel**和**deviceManufacturer**属性|
|添加项|beta|添加到[resourceOperation](/graph/api/resources/intune-rbac-resourceoperation?view=graph-rest-beta)实体**enabledForScopeValidation**属性|
|添加项|beta|添加到[vppToken](/graph/api/resources/intune-onboarding-vpptoken?view=graph-rest-beta)实体**claimTokenManagementFromExternalMdm**属性|
|添加项|beta|添加到[windows10GeneralConfiguration](/graph/api/resources/intune-deviceconfig-windows10generalconfiguration?view=graph-rest-beta)实体**windows10AppsForceUpdateSchedule**属性|
|添加项|beta|添加到[windowsAutopilotDeviceIdentity](/graph/api/resources/intune-enrollment-windowsautopilotdeviceidentity?view=graph-rest-beta)实体**deploymentProfileAssignmentDetailedStatus**属性|
|添加项|beta|添加到[deviceManagement](/graph/api/resources/intune-shared-devicemanagement?view=graph-rest-beta)实体**deviceConfigurationConflictSummary**和**importedWindowsAutopilotDeviceIdentityUploads**导航属性|
|添加项|beta|添加到[windowsAutopilotDeviceIdentity](/graph/api/resources/intune-enrollment-windowsautopilotdeviceidentity?view=graph-rest-beta)实体中的**intendedDeploymentProfile**导航属性|
|添加项|beta|**StartLayoutTileSize**和**名称**属性添加到[windowsKioskAppBase](/graph/api/resources/intune-deviceconfig-windowskioskappbase?view=graph-rest-beta)复杂类型|
|添加项|beta|添加到[windowsKioskDesktopApp](/graph/api/resources/intune-deviceconfig-windowskioskdesktopapp?view=graph-rest-beta)复杂类型的**desktopApplicationId**和**desktopApplicationLinkPath**属性|
|删除|beta|从[windowsKioskDesktopApp](/graph/api/resources/intune-deviceconfig-windowskioskdesktopapp?view=graph-rest-beta)复杂类型中移除的**name**属性|
|添加项|beta|**DisallowDesktopApps**属性添加到[windowsKioskMultipleApps](/graph/api/resources/intune-deviceconfig-windowskioskmultipleapps?view=graph-rest-beta)复杂类型|
|更改|beta|更改[windowsKioskMultipleApps](/graph/api/resources/intune-deviceconfig-windowskioskmultipleapps?view=graph-rest-beta)复杂类型上的以下属性：<br/>从**startMenuLayoutXml**所需为可选<br/>|
|添加项|beta|**V10_1607**、 **v10_1703**、 **v10_1709**和**v10_1803**属性添加到[windowsMinimumOperatingSystem](/graph/api/resources/intune-apps-windowsminimumoperatingsystem?view=graph-rest-beta)复杂类型|
|添加项|beta|**PaloAltoGlobalProtect**成员添加到[androidWorkProfileVpnConnectionType](/graph/api/resources/intune-deviceconfig-androidworkprofilevpnconnectiontype?view=graph-rest-beta)枚举类型|
|添加项|beta|**RemoteLock**成员添加到[deviceComplianceActionType](/graph/api/resources/intune-deviceconfig-devicecomplianceactiontype?view=graph-rest-beta)枚举类型|
...
| **更改类型** | **版本**   | **说明**                          |
| :-------------- | :------------ | :--------------------------------------- |
|添加项|beta|[选项卡](/graph/api/resources/teamstab?view=graph-rest-beta)添加了的 API。|
|添加项|beta|[发布](/graph/api/resources/teamsapp?view=graph-rest-beta)应用程序的组织添加了的 API。|
|添加项|beta|[获取 /teams/ {id}](/graph/api/team-get?view=graph-rest-beta)到支持添加应用程序权限。 |
|添加项|beta|添加到应用程序权限支持[GET /teams/ {id} / 通道](/graph/api/group-list-channels?view=graph-rest-beta)。 |
|添加项|beta|[获取 /teams/ {id} /channels/ {id}](/graph/api/channel-get?view=graph-rest-beta)到支持添加应用程序权限。 |
|添加项|beta|添加了应用程序权限支持到[放置 /groups/ {id} / 团队](/graph/api/team-put-teams?view=graph-rest-beta)。 |
|添加项|beta|添加应用程序权限支持到[修补程序 /teams/ {id}](/graph/api/team-update?view=graph-rest-beta)。 |
|添加项|beta|添加到[创建通道](/graph/api/channel-post?view=graph-rest-beta)、[更新通道](/graph/api/channel-patch?view=graph-rest-beta)和[删除通道](/graph/api/channel-delete?view=graph-rest-beta)权限支持的应用程序。 |
|删除|beta| 从[teamsApp](/graph/api/resources/teamsapp?view=graph-rest-beta)的已删除的 isBlocks 和 installedState 属性。|
|更改| beta | [TeamsApp](/graph/api/resources/teamsapp?view=graph-rest-beta)上的上下文属性已被重命名为 distributionMethod。|

### <a name="outlook-mail"></a>Outlook 邮件

| **更改类型** | **版本**   | **说明**                          |
| :-------------- | :------------ | :--------------------------------------- |
| 添加项        | v1.0 和 beta | [Message](/graph/api/resources/message?view=graph-rest-1.0)实体的**internetMessageHeaders**属性现写上创建消息。 |


### <a name="project-rome-notifications-api"></a>项目 Rome 通知 API

| **更改类型** | **版本** | **说明**                          |
| :-------------- | :---------- | :--------------------------------------- |
| 添加项          | Beta        | 添加[通知](/graph/api/resources/projectrome-notification?view=graph-rest-beta)资源类型。 |
| 添加项          | Beta        | 添加 [创建和发布通知] (图/api/projectrome_notification_post /？ 视图 = 图-rest beta) API。|

### <a name="security-apis"></a>安全 API

| **更改类型** | **版本** | **说明**              |
| :-------------- | :---------- | :--------------------------------------- |
| 添加项        | Beta       | 添加到[安全 API](/graph/api/resources/securescore-api-overview?view=graph-rest-beta)，包括以下资源和操作安全分数 Api:<br/>[secureScores](/graph/api/resources/securescores?view=graph-rest-beta)（和相关的实体）<br/>[列表 secureScores](/graph/api/securescores-list?view=graph-rest-beta)<br/>[secureScoreControlProfiles](/graph/api/resources/securescorecontrolprofiles?view=graph-rest-beta)<br/>[列表 secureScoreControlProfiles](/graph/api/securescorecontrolprofiles-list?view=graph-rest-beta)<br/>[更新 secureScoreControlProfiles](/graph/api/securescorecontrolprofiles-update?view=graph-rest-beta)
...
| **更改类型** | **版本** | **说明**                          |
| :-------------- | :---------- | :--------------------------------------- |
| 添加项        | Beta        | 为在[driveItem](/graph/api/resources/driveitem?view=graph-rest-beta) [createUploadSession](/graph/api/driveitem-createuploadsession?view=graph-rest-beta)操作添加**deferCommit**参数|
| 添加项        | Beta        | 添加[storagePlanInformation](/graph/api/resources/storageplaninformation?view=graph-rest-beta)复杂类型 |
| 添加项        | Beta        | **StoragePlanInformation**属性添加到[配额](/graph/api/resources/quota?view=graph-rest-beta)复杂类型 |
| 添加项        | Beta        | 添加到[驱动器](/graph/api/resources/drive?view=graph-rest-beta)实体中的**以下**导航属性 |
| 添加项        | Beta        | 添加[driveItem](/graph/api/resources/driveitem?view=graph-rest-beta)上[执行](/graph/api/driveitem-follow?view=graph-rest-beta)操作 |
| 添加项        | Beta        | 添加[取消追随](/graph/api/driveitem-unfollow?view=graph-rest-beta)API |
| 添加项        | Beta        | **HasPassword**属性添加到[权限](/graph/api/resources/permission?view=graph-rest-beta)实体 |
| 添加项        | Beta        | **PreventsDownload**属性添加到[sharingLink](/graph/api/resources/sharinglink?view=graph-rest-beta)复杂类型 |
| 添加项        | Beta        | 添加到[sharedDriveItem](/graph/api/resources/shareddriveitem?view=graph-rest-beta)实体的**权限**导航属性 |
| 添加项        | Beta        | 添加到[columnDefinition](/graph/api/resources/columndefinition?view=graph-rest-beta)实体中的**地理位置**属性 |
| 添加项        | Beta        | 添加[geolocationColumn](/graph/api/resources/geolocationcolumn?view=graph-rest-beta)复杂类型 |
| 添加项        | Beta        | 添加到[driveItem](/graph/api/resources/driveitem?view=graph-rest-beta)实体中的**分析**属性 |
| 添加项        | Beta        | 添加到[网站](/graph/api/resources/site?view=graph-rest-beta)实体中的**分析**属性 |
| 添加项        | Beta        | 添加到[listItem](/graph/api/resources/listitem?view=graph-rest-beta)实体中的**分析**属性 |
| 添加项        | Beta        | [DriveItem](/graph/api/resources/driveitem?view=graph-rest-beta)实体添加**getActivitiesByInterval**函数 |
| 添加项        | Beta        | 在[网站](/graph/api/resources/site?view=graph-rest-beta)实体添加**getActivitiesByInterval**函数 |
| 添加项        | Beta        | [ListItem](/graph/api/resources/listitem?view=graph-rest-beta)实体添加**getActivitiesByInterval**函数 |
| 添加项        | Beta        | 添加[itemAnalytics](/graph/api/resources/itemanalytics?view=graph-rest-beta)实体 |
| 添加项        | Beta        | 添加[itemActivityStat](/graph/api/resources/itemactivity?view=graph-rest-beta)实体 |
| 添加项        | Beta        | 添加[itemActionStat](/graph/api/resources/itemactionstat?view=graph-rest-beta)复杂类型 |
| 添加项        | Beta        | 添加[accessAction](/graph/api/resources/accessaction?view=graph-rest-beta)复杂类型 |
| 添加项        | Beta        | 添加[incompleteData](/graph/api/resources/incompletedata?view=graph-rest-beta)复杂类型 |
| 添加项        | Beta        | **访问**属性添加到[itemActivity](/graph/api/resources/itemactivity?view=graph-rest-beta)复杂类型 |
| 添加项        | Beta        | **Location**属性添加到[itemActivity](/graph/api/resources/itemactivity?view=graph-rest-beta)复杂类型 |
| 添加项        | v1.0        | [DriveItem](/graph/api/resources/driveitem?view=graph-rest-1.0)实体添加**预览**操作 |
| 添加项        | v1.0        | 添加[itemPreviewInfo](/graph/api/resources/itempreviewinfo?view=graph-rest-1.0)复杂类型 |
...
| **更改类型** | **版本** | **说明**                          |
| :-------------- | :---------- | :--------------------------------------- |
| 添加项        | Beta        | Azure AD 中的以下实体的添加了的[增量查询](delta-query-overview.md)功能：<br/>[application](/graph/api/application-delta?view=graph-rest-beta)<br/>[directoryRole](/graph/api/directoryrole-delta?view=graph-rest-beta)<br/>[servicePrincipal](/graph/api/serviceprincipal-delta?view=graph-rest-beta) |

### <a name="directory-apis"></a>目录 API

| **更改类型** | **版本**   | **说明**                          |
| :-------------- | :------------ | :--------------------------------------- |
| 添加项 | v1.0 | 向 [Organization](/graph/api/resources/organization?view=graph-rest-beta) 资源添加了 isMultipleDataLocationsForServicesEnabled 属性，允许应用确认已为 Multi-Geo 功能启用了租户。 向 [user](/graph/api/resources/user?view=graph-rest-beta) 和 [group](/graph/api/resources/group?view=graph-rest-beta) 资源添加了 preferredDataLocation 属性，允许为用户和组设置首选数据位置。|
| 添加项 | v1.0 | 添加了的[onPremisesProvisioningErrors](/graph/api/resources/onpremisesprovisioningerror?view=graph-rest-1.0)属性设为[用户](/graph/api/resources/user?view=graph-rest-1.0)和[组](/graph/api/resources/group?view=graph-rest-1.0)实体值，该值代表目录同步错误时使用 Microsoft 时同步到 Azure Active Directory 的内部部署目录同步产品 （包括 Azure AD 连接、 目录同步和 MIM + 连接器）。|
| 添加项 | v1.0 | 添加到包含 15 个自定义扩展特性属性的[用户](/graph/api/resources/user?view=graph-rest-1.0)实体[onPremisesExtensionAttributes](/graph/api/resources/onpremisesextensionattributes?view=graph-rest-1.0)属性。 此组属性是 onPremisesSyncEnabled 用户掌握了内部部署 Active Directory 中和同步到 Azure AD 和是只读的。 仅限云用户 （其中 onPremisesSyncEnabled 为 false），这些属性的创建过程中可以设置或更新。|
|添加项|v1.0|添加到[用户](/graph/api/resources/user?view=graph-rest-1.0)实体的**onPremisesDomainName**、 **onPremisesSamAccountName**和**onPremisesUserPrincipalName**属性|

### <a name="microsoft-intune-apis"></a>Microsoft Intune API

|更改类型|版本|说明|
|:---|:---|:---|
|添加项|v1.0|添加了新实体：<br/>[androidWorkProfileCompliancePolicy](/graph/api/resources/intune-deviceconfig-androidworkprofilecompliancepolicy?view=graph-rest-1.0)<br/>[androidWorkProfileCustomConfiguration](/graph/api/resources/intune-deviceconfig-androidworkprofilecustomconfiguration?view=graph-rest-1.0)<br/>[androidWorkProfileGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-androidworkprofilegeneraldeviceconfiguration?view=graph-rest-1.0)<br/>|
...
|新增|v1.0|添加到[iosGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-iosgeneraldeviceconfiguration?view=graph-rest-1.0)实体**kioskModeBuiltInAppId**属性|
|添加项|v1.0|**NotAssigned**成员添加到[complianceStatus](/graph/api/resources/intune-shared-compliancestatus?view=graph-rest-1.0)枚举类型|
|添加项|v1.0|**PushNotification**成员添加到[deviceComplianceActionType](/graph/api/resources/intune-deviceconfig-devicecomplianceactiontype?view=graph-rest-1.0)枚举类型|
|添加项|v1.0|**UserAbandonment**成员添加到[deviceEnrollmentFailureReason](/graph/api/resources/intune-troubleshooting-deviceenrollmentfailurereason?view=graph-rest-1.0)枚举类型|
|添加项|v1.0|添加的**威胁**和**配置错误**成员为[managedDevicePartnerReportedHealthState](/graph/api/resources/intune-devices-manageddevicepartnerreportedhealthstate?view=graph-rest-1.0)枚举类型|
|添加项|v1.0|**AssignedToExternalMDM**成员添加到[vppTokenState](/graph/api/resources/intune-onboarding-vpptokenstate?view=graph-rest-1.0)枚举类型|
||
...
|添加项|beta|[DepOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta)上添加[uploadDepToken](/graph/api/intune-enrollment-deponboardingsetting-uploaddeptoken?view=graph-rest-beta)操作 |
|添加项|beta|[DepOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta)上添加[syncWithAppleDeviceEnrollmentProgram](/graph/api/intune-enrollment-deponboardingsetting-syncwithappledeviceenrollmentprogram?view=graph-rest-beta)操作 |
|添加项|beta|[EnrollmentProfile](/graph/api/resources/intune-enrollment-enrollmentprofile?view=graph-rest-beta)上添加[setDefaultProfile](/graph/api/intune-enrollment-enrollmentprofile-setdefaultprofile?view=graph-rest-beta)操作 |
|添加项|beta|[ImportedAppleDeviceIdentity](/graph/api/resources/intune-enrollment-importedappledeviceidentity?view=graph-rest-beta)集上添加**importAppleDeviceIdentityList**操作 |
|添加项|beta|[EnrollmentProfile](/graph/api/resources/intune-enrollment-enrollmentprofile?view=graph-rest-beta)上添加[updateDeviceProfileAssignment](/graph/api/intune-enrollment-enrollmentprofile-updatedeviceprofileassignment?view=graph-rest-beta)操作 |
|添加项|beta|[DepOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta)上添加[shareForSchoolDataSyncService](/graph/api/intune-enrollment-deponboardingsetting-shareforschooldatasyncservice?view=graph-rest-beta)操作 |
|添加项|beta|[DepOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta)上添加[unshareForSchoolDataSyncService](/graph/api/intune-enrollment-deponboardingsetting-unshareforschooldatasyncservice?view=graph-rest-beta)操作 |
|添加项|beta|[WindowsAutopilotDeviceIdentity](/graph/api/resources/intune-enrollment-windowsautopilotdeviceidentity?view=graph-rest-beta)上添加[assignUserToDevice](/graph/api/intune-enrollment-windowsautopilotdeviceidentity-assignusertodevice?view=graph-rest-beta)操作 |
|添加项|beta|[DeviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta)上添加[getRoleScopeTagsByResource](/graph/api/intune-rbac-devicemanagement-getrolescopetagsbyresource?view=graph-rest-beta)函数 |
|添加项|beta|[DeviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta)上添加[getRoleScopeTagsByIds](/graph/api/intune-rbac-devicemanagement-getrolescopetagsbyids?view=graph-rest-beta)函数 |
|添加项|beta|[DepOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta)上添加[getEncryptionPublicKey](/graph/api/intune-enrollment-deponboardingsetting-getencryptionpublickey?view=graph-rest-beta)函数 |
|添加项|beta|[EnrollmentProfile](/graph/api/resources/intune-enrollment-enrollmentprofile?view=graph-rest-beta)上添加[exportMobileConfig](/graph/api/intune-enrollment-enrollmentprofile-exportmobileconfig?view=graph-rest-beta)函数 |
|添加项|beta|[ImportedWindowsAutopilotDeviceIdentityUpload](/graph/api/resources/intune-enrollment-importedwindowsautopilotdeviceidentityupload?view=graph-rest-beta)上添加[autopilotDeviceStream](/graph/api/intune-enrollment-importedwindowsautopilotdeviceidentityupload-autopilotdevicestream?view=graph-rest-beta)函数 |
|删除|beta|删除**uploadDepToken**集合 |
|删除|beta|删除[depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta)集上的**syncWithAppleDeviceEnrollmentProgram**操作 |
|删除|beta|删除**getEncryptionPublicKey**函数在[depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta)集合 |
|添加项|beta|添加到[androidForWorkCompliancePolicy](/graph/api/resources/intune-deviceconfig-androidforworkcompliancepolicy?view=graph-rest-beta)实体**restrictedApps**属性|
|添加项|beta|添加到[androidForWorkGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-androidforworkgeneraldeviceconfiguration?view=graph-rest-beta)实体中的**vpnAlwaysOnPackageIdentifier**和**vpnEnableAlwaysOnLockdownMode**属性|
|删除|beta|从[androidForWorkMobileAppConfiguration](/graph/api/resources/intune-apps-androidforworkmobileappconfiguration?view=graph-rest-beta)实体中删除的**软件包名称**属性|
|添加项|beta|添加到[androidWorkProfileCompliancePolicy](/graph/api/resources/intune-deviceconfig-androidworkprofilecompliancepolicy?view=graph-rest-beta)实体**restrictedApps**属性|
|添加项|beta|添加到[androidWorkProfileGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-androidworkprofilegeneraldeviceconfiguration?view=graph-rest-beta)实体中的**vpnAlwaysOnPackageIdentifier**和**vpnEnableAlwaysOnLockdownMode**属性|
|添加项|beta|添加到[appleVpnConfiguration](/graph/api/resources/intune-deviceconfig-applevpnconfiguration?view=graph-rest-beta)实体**optInToDeviceIdSharing**属性|
|添加项|beta|添加到[depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta)实体**tokenType**、 **tokenName**、 **syncedDeviceCount**、 **defaultProfileDisplayName**和**dataSharingConsentGranted**属性|
|添加项|beta|添加到[deviceCompliancePolicy](/graph/api/resources/intune-deviceconfig-devicecompliancepolicy?view=graph-rest-beta)实体**roleScopeTagIds**属性|
|添加项|beta|添加到[deviceConfiguration](/graph/api/resources/intune-deviceconfig-deviceconfiguration?view=graph-rest-beta)实体中的**roleScopeTagIds**和**supportsScopeTags**属性|
|添加项|beta|添加到[deviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta)实体**windowsMalwareOverview**属性|
|更改|beta|更改[iosCertificateProfileBase](/graph/api/resources/intune-deviceconfig-ioscertificateprofilebase?view=graph-rest-beta)实体上的以下属性：<br/>将 **subjectAlternativeNameType** 从必需更改为可选<br/>|
|添加项|beta|添加到[iosCompliancePolicy](/graph/api/resources/intune-deviceconfig-ioscompliancepolicy?view=graph-rest-beta)实体**restrictedApps**属性|
|添加项|beta|添加到[iosScepCertificateProfile](/graph/api/resources/intune-deviceconfig-iosscepcertificateprofile?view=graph-rest-beta)实体中的**certificateStore**和**customSubjectAlternativeNames**属性|
|添加项|beta|添加到[iosUpdateConfiguration](/graph/api/resources/intune-deviceconfig-iosupdateconfiguration?view=graph-rest-beta)实体**enforcedSoftwareUpdateDelayInDays**属性|
|添加项|beta|添加到[iosVpnConfiguration](/graph/api/resources/intune-deviceconfig-iosvpnconfiguration?view=graph-rest-beta)实体中的**提供程序类型**、 **userDomain**、 **strictEnforcement**、 **cloudName**和**excludeList**属性|
|添加项|beta|添加**safariBlockAutofill**、 **cameraBlocked**、 **iTunesBlockMusicService**、 **spotlightBlockInternetResults**、 **keyboardBlockDictation**、 **definitionLookupBlocked**、 **appleWatchBlockAutoUnlock**， **iTunesBlockFileSharing**、 **iCloudBlockDocumentSync**、 **iCloudBlockMail**、 **iCloudBlockAddressBook**、 **iCloudBlockCalendar**、 **iCloudBlockReminders**、 **iCloudBlockBookmarks**， **iCloudBlockNotes**、 **airDropBlocked**、 **passwordBlockModification**和**passwordBlockFingerprintUnlock**属性设置为[macOSGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-macosgeneraldeviceconfiguration?view=graph-rest-beta)实体|
|添加项|beta|添加到[managedDevice](/graph/api/resources/intune-devices-manageddevice?view=graph-rest-beta)实体**roleScopeTagIds**、 **windowsActiveMalwareCount**、 **windowsRemediatedMalwareCount**、**注释**和**configurationManagerClientHealthState**属性|
|添加项|beta|添加到[mobileAppInstallStatus](/graph/api/resources/intune-apps-mobileappinstallstatus?view=graph-rest-beta)实体**installStateDetail**属性|
|添加项|beta|添加到[notificationMessageTemplate](/graph/api/resources/intune-notification-notificationmessagetemplate?view=graph-rest-beta)实体**roleScopeTagIds**属性|
|添加项|beta|添加到[officeSuiteApp](/graph/api/resources/intune-apps-officesuiteapp?view=graph-rest-beta)实体中的**targetVersion**和**updateVersion**属性|
|添加项|beta|添加到[resourceOperation](/graph/api/resources/intune-rbac-resourceoperation?view=graph-rest-beta)实体中的**资源**属性|
|添加项|beta|添加到[sharedPCConfiguration](/graph/api/resources/intune-deviceconfig-sharedpcconfiguration?view=graph-rest-beta)实体中的**localStorage**、 **setPowerPolicies**和**signInOnResume**属性|
|添加项|beta|添加到[windows10CompliancePolicy](/graph/api/resources/intune-deviceconfig-windows10compliancepolicy?view=graph-rest-beta)实体**configurationManagerComplianceRequired**属性|
|添加项|beta|添加**userRightsAccessCredentialManagerAsTrustedCaller**、 **userRightsAllowAccessFromNetwork**、 **userRightsBlockAccessFromNetwork**、 **userRightsActAsPartOfTheOperatingSystem**、 **userRightsLocalLogOn**， **userRightsBackupData**、 **userRightsChangeSystemTime**、 **userRightsCreateGlobalObjects**、 **userRightsCreatePageFile**、 **userRightsCreatePermanentSharedObjects**、 **userRightsCreateSymbolicLinks**， **userRightsCreateToken**、 **userRightsDebugPrograms**、 **userRightsRemoteDesktopServicesLogOn**、 **userRightsDelegation**、 **userRightsGenerateSecurityAudits**， **userRightsImpersonateClient**、 **userRightsIncreaseSchedulingPriority**、 **userRightsLoadUnloadDrivers**、 **userRightsLockMemory**、 **userRightsManageAuditingAndSecurityLogs**、 **userRightsManageVolumes**， **userRightsModifyFirmwareEnvironment**、 **userRightsModifyObjectLabels**、 **userRightsProfileSingleProcess**、 **userRightsRemoteShutdown**、 **userRightsRestoreData**， **userRightsTakeOwnership** **userRightsRegisterProcessAsService**、 **localSecurityOptionsMinimumSessionSecurityForNtlmSspBasedClients**、 **localSecurityOptionsMinimumSessionSecurityForNtlmSspBasedServers**， **lanManagerAuthenticationLevel**和**lanManagerWorkstationEnableInsecureGuestLogons**属性设置为[windows10EndpointProtectionConfiguration](/graph/api/resources/intune-deviceconfig-windows10endpointprotectionconfiguration?view=graph-rest-beta)实体|
|添加项|beta|添加到[windows10GeneralConfiguration](/graph/api/resources/intune-deviceconfig-windows10generalconfiguration?view=graph-rest-beta)实体中的**passwordMinimumAgeInDays**、 **tenantLockdownRequireNetworkDuringOutOfBoxExperience**和**dataProtectionBlockDirectMemoryAccess**属性|
|添加项|beta|添加到[windows10PkcsCertificateProfile](/graph/api/resources/intune-deviceconfig-windows10pkcscertificateprofile?view=graph-rest-beta)实体**extendedKeyUsages**属性|
|添加项|beta|添加到[windows10VpnConfiguration](/graph/api/resources/intune-deviceconfig-windows10vpnconfiguration?view=graph-rest-beta)实体中的**enableDnsRegistration**和**dnsSuffixes**属性|
|添加项|beta|添加到[windows81CertificateProfileBase](/graph/api/resources/intune-deviceconfig-windows81certificateprofilebase?view=graph-rest-beta)实体**customSubjectAlternativeNames**属性|
|添加项|beta|添加到[windowsAutopilotDeploymentProfile](/graph/api/resources/intune-enrollment-windowsautopilotdeploymentprofile?view=graph-rest-beta)实体中的**extractHardwareHash**和**deviceNameTemplate**属性|
|添加项|beta|添加到[windowsAutopilotDeviceIdentity](/graph/api/resources/intune-enrollment-windowsautopilotdeviceidentity?view=graph-rest-beta)实体中的**addressableUserName**和**userPrincipalName**属性|
|添加项|beta|添加到[windowsDeviceMalwareState](/graph/api/resources/intune-devices-windowsdevicemalwarestate?view=graph-rest-beta)实体**threatState**属性|
|添加项|beta|添加**qualityUpdatesPauseStartDateTime**、 **featureUpdatesPauseStartDateTime**、 **featureUpdatesRollbackWindowInDays**、 **qualityUpdatesWillBeRolledBack**、 **featureUpdatesWillBeRolledBack**、 **qualityUpdatesRollbackStartDateTime**和到[windowsUpdateForBusinessConfiguration](/graph/api/resources/intune-deviceconfig-windowsupdateforbusinessconfiguration?view=graph-rest-beta)实体**featureUpdatesRollbackStartDateTime**属性|
|添加项|beta|添加到[windowsWifiEnterpriseEAPConfiguration](/graph/api/resources/intune-deviceconfig-windowswifienterpriseeapconfiguration?view=graph-rest-beta)实体**trustedServerCertificateNames**属性|
|添加项|beta|添加到[depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta)实体**defaultIosEnrollmentProfile**、 **defaultMacOsEnrollmentProfile**、 **enrollmentProfiles**和**importedAppleDeviceIdentities**导航属性|
|添加项|beta|添加到[deviceAndAppManagementRoleAssignment](/graph/api/resources/intune-rbac-deviceandappmanagementroleassignment?view=graph-rest-beta)实体中的**roleScopeTags**导航属性|
|添加项|beta|添加到[deviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta)实体**advancedThreatProtectionOnboardingStateSummary**、 **roleScopeTags**和**importedWindowsAutopilotDeviceIdentityUploads**导航属性|
|添加项|beta|**SupportedDeviceTypes**属性添加到[mobileAppIntentAndStateDetail](/graph/api/resources/intune-troubleshooting-mobileappintentandstatedetail?view=graph-rest-beta)复杂类型|
|添加项|beta|**HideEscapeLink**属性添加到[outOfBoxExperienceSettings](/graph/api/resources/intune-enrollment-outofboxexperiencesettings?view=graph-rest-beta)复杂类型|
|添加项|beta|**ZscalerPrivateAccess**、 **f5Access2018**、 **citrixSso**和**paloAltoGlobalProtectV2**成员添加到[appleVpnConnectionType](/graph/api/resources/intune-deviceconfig-applevpnconnectiontype?view=graph-rest-beta)枚举类型|
|添加项|beta|**UserAbandonment**成员添加到[deviceEnrollmentFailureReason](/graph/api/resources/intune-troubleshooting-deviceenrollmentfailurereason?view=graph-rest-beta)枚举类型|
|添加项|beta|**阻止**成员添加到[enrollmentState](/graph/api/resources/intune-enrollment-enrollmentstate?view=graph-rest-beta)枚举类型|
|添加项|beta|**Microsoft365ManagedMdm**成员添加到[managementAgentType](/graph/api/resources/intune-devices-managementagenttype?view=graph-rest-beta)枚举类型|
|添加项|beta|**DomainNameService**成员添加到[subjectAlternativeNameType](/graph/api/resources/intune-deviceconfig-subjectalternativenametype?view=graph-rest-beta)枚举类型|
|添加项|beta|添加的**wpa2Personal**和**wpa2Enterprise**成员为[wiFiSecurityType](/graph/api/resources/intune-deviceconfig-wifisecuritytype?view=graph-rest-beta)枚举类型|
|添加项|beta|**EnterpriseUnwantedSoftware**、**然后**和**hipsRule**成员添加到[windowsMalwareCategory](/graph/api/resources/intune-devices-windowsmalwarecategory?view=graph-rest-beta)枚举类型|

### <a name="outlook-calendar"></a>Outlook 日历

| **更改类型** | **版本**   | **说明**                          |
| :-------------- | :------------ | :--------------------------------------- |
| 添加项 | Beta | 添加的[getSchedule](/graph/api/calendar-getschedule?view=graph-rest-beta)操作，以及[freeBusyError](/graph/api/resources/freebusyerror?view=graph-rest-beta)、 [scheduleInformation](/graph/api/resources/scheduleinformation?view=graph-rest-beta)和[scheduleItem](/graph/api/resources/scheduleitem?view=graph-rest-beta)复杂类型支持[获取用户、 通讯组列表和资源的闲/忙、 可用性信息时间的给定期间](outlook-get-free-busy-schedule.md)。 |

### <a name="outlook-mail"></a>Outlook 邮件

| **更改类型** | **版本**   | **说明**                          |
| :-------------- | :------------ | :--------------------------------------- |
| 添加项        | v1.0        | 要获得特定收件人的任何邮件提示的[getMailTips](/graph/api/user-getmailtips?view=graph-rest-1.0)操作添加了的支持。 添加以下资源： [automaticRepliesMailTips](/graph/api/resources/automaticrepliesmailtips?view=graph-rest-1.0)、[邮件提示](/graph/api/resources/mailtips?view=graph-rest-1.0)、 [mailTipsError](/graph/api/resources/mailtipserror?view=graph-rest-1.0)。 |

### <a name="reports-apis"></a>报表 API
| 更改类型 | 版本 | 说明                              |
|:------------|:--------|:-----------------------------------------|
| 添加项    | v1.0    | 添加到[getoffice365activationsuserdetail](/graph/api/reportroot-getoffice365activationsuserdetail?view=graph-rest-1.0)**激活上共享计算机**属性。 |
| 添加项    | v1.0    | 添加到[getoffice365activationsusercounts](/graph/api/reportroot-getoffice365activationsusercounts?view=graph-rest-1.0)**共享计算机激活**属性。 |

### <a name="security-apis"></a>安全 API

| **更改类型** | **版本** | **说明**              |
| :-------------- | :---------- | :--------------------------------------- |
| 添加项        | beta       | 添加了的**activityGroupName**、 **cloudAppStates**、**可信度**和**registryKeyStates**属性设置为[通知](/graph/api/resources/alert?view=graph-rest-beta )。 |
|删除|beta| 从[通知中](/graph/api/resources/alert?view=graph-rest-beta )的已删除的**activityGroupStates**、 **applicationStates**、 **malwareWasRunning**、 **riskScore**和**类型**属性。 |
|更改|beta| 已改变**注释**类型从`String`到`String collection`，并已改变**严重性**类型从`String`到在[通知](/graph/api/resources/alert?view=graph-rest-beta) [alertSeverity](/graph/api/resources/alertseverityenumtype?view=graph-rest-beta)枚举。 |
| 添加项        | beta       | 添加以下资源类型： <br/> [cloudAppSecurityState](/graph/api/resources/cloudappsecuritystate?view=graph-rest-beta) <br/> [fileHash](/graph/api/resources/filehash?view=graph-rest-beta) <br/> [registryKeyState](/graph/api/resources/registrykeystate?view=graph-rest-beta) |
|删除|beta| 删除下列资源类型： <br/> **activityGroupState**  <br/> **applicationSecurityState** |
| 新增        | beta       | 添加下列枚举： <br/> [alertSeverity](/graph/api/resources/alertseverityenumtype?view=graph-rest-beta) <br/> [connectionDirection](/graph/api/resources/connectiondirectionenumtype?view=graph-rest-beta) <br/> [connectionStatus](/graph/api/resources/connectionstatusenumtype?view=graph-rest-beta) <br/> [emailRole](/graph/api/resources/emailroleenumtype?view=graph-rest-beta) <br/> [fileHashType](/graph/api/resources/filehashtypeenumtype?view=graph-rest-beta) <br/> [registryHive](/graph/api/resources/registryhiveenumtype?view=graph-rest-beta)  <br/> [registryOperation](/graph/api/resources/registryoperationenumtype?view=graph-rest-beta) <br/> [registryValueType](/graph/api/resources/registryvaluetypeenumtype?view=graph-rest-beta)|
|删除|beta| 删除了以下枚举类型： <br/> **alertType** <br/> **applicationPermissionsRequired** |
| 添加项        | beta       | 添加到[fileSecurityState](/graph/api/resources/filesecuritystate?view=graph-rest-beta ) **fileHash**属性。|
|删除|beta| 从[fileSecurityState](/graph/api/resources/filesecuritystate?view=graph-rest-beta)的已删除的**authenticodeHash256**和**sha256**属性。 |
| 添加项 | beta | 添加到[hostSecurityState](/graph/api/resources/hostsecuritystate?view=graph-rest-beta) **os**属性。|
| 添加项 | beta | 添加了的**类别**、**家人**和**wasRunning**属性设置为[malwareState](/graph/api/resources/malwarestate?view=graph-rest-beta)。|
|删除|beta| 从[malwareState](/graph/api/resources/malwarestate?view=graph-rest-beta)删除**别名**属性。 |
|更改|beta| 从[通知](/graph/api/resources/alert?view=graph-rest-beta )的**malwareWasRunning**属性移至[malwareState](/graph/api/resources/malwarestate?view=graph-rest-beta)并重命名为**wasRunning**。 |
| 添加项        | beta       | 添加了的**应用程序名称**、 **destinationDomain**、**方向**、 **domainRegisteredDateTime**、 **localDnsName**、 **natDestinationAddress**、 **natDestinationPort**、 **natSourceAddress**、 **natSourcePort**， **riskScore**、**状态**和**urlParameters**属性设置为[networkConnection](/graph/api/resources/networkconnection?view=graph-rest-beta )。|
|更改|beta| 更改为**destinationUrl** [networkConnection](/graph/api/resources/networkconnection?view=graph-rest-beta )中的**uri**属性。 |
| 添加项        | beta       | 添加到[过程](/graph/api/resources/process?view=graph-rest-beta ) **fileHash**属性。|
|删除|beta| 从[进程](/graph/api/resources/process?view=graph-rest-beta )的已删除的**authenticodeHash256**和**sha256**属性。 |
| 添加项        | beta       | 添加了的**aadUserId**、 **emailRole**、 **isVpn**和**logonIp**属性设置为[userSecurityState](/graph/api/resources/usersecuritystate?view=graph-rest-beta)。|
|更改|beta| 更改为**logonIp**中[userSecurityState](/graph/api/resources/usersecuritystate?view=graph-rest-beta) **logonIpAddress**属性。 |
| 添加项        | beta       | 添加到[vulnerabilityState](/graph/api/resources/vulnerabilitystate?view=graph-rest-beta) **wasRunning**属性。|
|删除|beta| 从[vulnerabilityState](/graph/api/resources/vulnerabilitystate?view=graph-rest-beta)删除**name**属性。 |
...
| **更改类型** | **版本**   | **说明**                          |
| :-------------- | :------------ | :--------------------------------------- |
|更改|beta|更新的[chatmessage](/graph/api/resources/chatmessage?view=graph-rest-beta)资源|
...
| **更改类型** | **版本**   | **说明**                          |
| :-------------- | :------------ | :--------------------------------------- |
| 添加项 | Beta | 添加到[sychronizationStatus](/graph/api/resources/synchronization-synchronizationstatus?view=graph-rest-beta)允许客户端监视进度的同步作业**progress**属性。|
...
| **更改类型** | **版本**   | **说明**                          |
| :-------------- | :------------ | :--------------------------------------- |
| 添加项 | beta | 添加到[Azure AD](/graph/api/resources/azure-ad-overview?view=graph-rest-beta) [access 会检查](/graph/api/resources/accessreviews-root?view=graph-rest-beta)功能。 |
...
| 添加项 | v1.0 | 添加了的**ageGroup**、 **legalAgeGroupClassification**和**ConsentRequiredForMinor**属性设置为[用户](/graph/api/resources/user?view=graph-rest-1.0)资源
...
|删除|beta|删除 exportMobileConfig](/graph/api/intune_corpenrollment_enrollmentprofile_exportmobileconfig?view=graph-rest-beta) 函数在[enrollmentProfile](/graph/api/resources/intune-corpenrollment-enrollmentprofile?view=graph-rest-beta) |
...
| **更改类型** | **版本** | **说明**                          |
| :-------------- | :---------- | :--------------------------------------- |
| 添加        | v1.0        | 向 [organization](/graph/api/resources/organization?view=graph-rest-1.0) 实体添加了 **privacyProfile** 复杂类型。 |
...
| Addition        | v1.0        | 向 [webhook](/graph/api/resources/webhooks?view=graph-rest-1.0) 通知订阅添加了用户和组支持。 |
| 添加        | beta        | 向[目录（已删除项目）](/graph/api/resources/directory?view=graph-rest-beta)资源添加了[列出用户所拥有的已删除项](/graph/api/directory-deleteditems-user-owned?view=graph-rest-beta)操作 |
...
|Addition|v1.0|添加的新枚举类型：<br/>[managedDeviceOwnerType](/graph/api/resources/intune-devices-manageddeviceownertype?view=graph-rest-1.0)<br/>|
...
|添加|Beta|对 [managedDevice](/graph/api/resources/intune-devices-manageddevice?view=graph-rest-beta) 新增了 [wipe](/graph/api/intune-devices-manageddevice-wipe?view=graph-rest-beta) 操作 |
|Addition|Beta|对 [managedDevice](/graph/api/resources/intune-devices-manageddevice?view=graph-rest-beta) 新增了 [shutDown](/graph/api/intune-devices-manageddevice-shutdown?view=graph-rest-beta) 操作 |
...
|Addition|Beta|对 [deviceAppManagement](/graph/api/resources/intune-apps-deviceappmanagement?view=graph-rest-beta) 新增了 [syncMicrosoftStoreForBusinessApps](/graph/api/intune-onboarding-deviceappmanagement-syncmicrosoftstoreforbusinessapps?view=graph-rest-beta) 操作 |
...
|更改|Beta|向 [windowsInformationProtectionApp](/graph/api/resources/intune-mam-windowsinformationprotectionapp?view=graph-rest-beta) 复杂类型添加了 **denied** 属性|
...
| 添加项        | Beta        | 要获得特定收件人的任何邮件提示的[getMailTips](/graph/api/user-getmailtips?view=graph-rest-beta)操作添加了的支持。 添加以下资源： [automaticRepliesMailTips](/graph/api/resources/automaticrepliesmailtips?view=graph-rest-beta)、[邮件提示](/graph/api/resources/mailtips?view=graph-rest-beta)、 [mailTipsError](/graph/api/resources/mailtipserror?view=graph-rest-beta)。 |
...
|**更改类型**|**版本**|**说明**|
|:--------------|:-----------|:--------------|
|重大更改|Beta|对 findMeetingTimes API 的更改。 有关详细信息，请参阅[Microsoft Graph findMeetingTimes API 更新](https://dev.office.com/microsoft-graph-findmeetingtimes-api-update)博客文章。 此更改 2016 年 5 月 19，生效。
