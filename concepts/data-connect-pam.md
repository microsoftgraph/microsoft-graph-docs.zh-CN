...
4. 查找所有待处理的请求。
   >**注意：****Identity** 属性中的值将用于标识以及批准或拒绝请求。 记下此值，并在 -RequestId 参数中使用。 

   ```powershell
   Get-ElevatedAccessRequest | ?{$_.RequestStatus -eq 'Pending'}
   ```

4. 仔细查看你感兴趣的请求的 **context** 字段。 
...
5. 为 -RequestId 参数使用 **Identity** 的值来批准/拒绝请求。
...
若要撤销以前批准的请求，请选择需要撤销的已批准请求，并选择“**撤销**”。 下一次使用该审批转移数据的尝试将失败。 
