...
---
# <a name="reduce-missing-subscriptions-and-notifications-for-outlook-resources-preview"></a>减少 Outlook 资源的丢失订阅和通知（预览版） 

订阅 Outlook 资源通知的应用程序可能会删除其订阅并错过一些通知。 应用程序应实现逻辑以检测并从丢失中恢复，并恢复连续通知流程。
...
- 收到 `missed` 通知后，该应使用 Microsoft Graph 重新同步资源数据。
...
> **注意：** 目前，只能使用 Microsoft Graph API 的 `beta` 版本设置或读取 **lifecycleNotificationUrl** 属性。 但是，使用 `beta` 创建的订阅与 `v1.0` 存储在同一个生产环境中，因此除了经常使用的 `v1.0` 和其他订阅外，你还可以实现此处描述的新 Outlook 流程。
...
- 与资源通知类似，生命周期通知可以一起批处理（在**值**数组中），每个通知都有一个可能不同的 **lifecycleEvent** 值。 相应地处理批处理中的每个通知。
...
5. 创建新订阅后，从收到此资源通知的上次已知时间同步任何缺少的资源数据；例如：`GET https://graph.microsoft.com/v1.0/users/{id}/messages?$filter=createdDateTime+ge+{LastTimeNotificationWasReceived}`
