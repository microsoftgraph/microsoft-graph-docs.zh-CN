...
---
# <a name="create-absolute-positioned-elements-in-onenote-pages"></a>在 OneNote 页中创建绝对定位的元素

OneNote 页面的正文可以包含多个直接 `div`、`img` 和 `object` 子元素，可在页面上对其独立定位。

<a name="attributes"></a>

## <a name="attributes-and-positioning-behavior"></a>属性和定位操作

使用 `data-absolute-enabled` 和 [`style`](#supported-css-style-attributes) 属性在页面上创建绝对定位元素，如下所示：

- 主体元素必须指定`data-absolute-enabled="true"`。 如果省略或设置为 `false`，则所有正文内容在 API 创建的 `_default` 绝对定位 div 内呈现，所有位置设置将被忽略。

- 只有 `div`， `img`，和 `object` 元素可以是绝对定位的元素。 

- 绝对定位元素必须指定 `style="position:absolute"`。

- 绝对定位的元素必须是 `body` 元素的直接子元素。 任何非绝对定位的直接的子元素 `div`， `img`， 或`object`元素在绝对定位的`_default` div内呈现为静态内容。

- 绝对定位元素位于其指定的顶部和左侧坐标，与页面左上角（在标题区域上方）的 0:0 起始位置相关。

- 如果绝对定位元素忽略了顶部或左侧坐标，那么缺少的坐标将被设置为其默认值： `top:120px` 或者 `left:48px`。 这些默认坐标指定位于标题区域正下方的位置。 请注意，省略坐标可能会导致元素之间相互堆叠在彼此之上。

- 绝对定位的元素不能嵌套或包含定位元素。 API 忽略在绝对定位的 div 中嵌套元素上指定的任何位置设置，在绝对定位的父 div 中呈现此嵌套内容，并在响应中返回 **api.diagnostics** 属性中的警告。
...
该API呈现默认div中的非绝对定位div。 请注意，嵌套的 `<div>` 标记已被弃用，因为它们无法定义任何语义信息（例如，`data-id`）。
...
OneNote API 评估输入 HTML 并保留所有语义内容和受 OneNote 支持的任何结构信息。 结果页面呈现如下图所示（但没有div和图像的可见边框）。 

![生成具有绝对定位div和图像的页面](images/abs-pos.png)

注意输入HTML中非贡献的嵌套div的更改。 API保留div的内容但弃置 `<div>` 标签，因为div没有定义语义信息（比如 `data-id`）。
...
所有绝对定位元素都可以指定顶部和左侧位置。 div 和图像可以指定宽度，并且图像还可以指定高度。 例如：
...
| 属性 | 支持的元素 | 说明 |  
|:------|:------|:------|  
| top | div，图像，对象 | 仅以像素为单位的元素顶部边框的 y 轴坐标。 默认值是120像素。<br/><br/>示例： `top:140px` |  
| left |  div，图像，对象  | 仅以像素为单位的元素左边框的 x 轴坐标。 默认值是48像素。<br/><br/>示例： `left:95px` |  
| width |  div，图像  | 仅以像素为单位的元素的宽度。<br/><br/>示例： `width:480px` |  
| height | img | 仅以像素为单位的元素的高度。 对于 div，高度是在运行时计算的，且任何指定的高度值都将被忽略。<br/><br/>示例： `height:665px` |  
 
其他位置属性（如 `z-index`）都将被忽略。 绝对定位的图像可以使用 `data-render-src` 或者 `src` 的属性。
