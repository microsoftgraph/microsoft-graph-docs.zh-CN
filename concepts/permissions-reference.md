...
## <a name="granular-delegated-admin-privileges-gdap-permissions"></a>粒度委派管理员权限 (GDAP) 权限
...
|_OnlineMeetingTranscript.Read.All_|读取联机会议的所有脚本。|允许应用代表已登录用户读取联机会议的所有脚本。 |是|否|
...
|_OnlineMeetingTranscript.Read.All_|读取联机会议的所有脚本。|允许应用在没有已登录用户的情况下读取所有联机会议的所有脚本。 |是|
