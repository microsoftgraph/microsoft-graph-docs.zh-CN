...
单独请求还可以包含 `headers` 属性和 `body` 属性。两种属性通常都是 JSON 对象，如上一示例中所示。在某些情况下，`body` 可能是 base64 URL 编码的值，而不是一个 JSON 对象 - 例如，当正文为图像时。在这些情况下，`headers` 对象必须包含 `content-type` 的值。
...
## <a name="sequencing-requests-with-the-dependson-property"></a>使用 dependsOn 属性对请求排序
