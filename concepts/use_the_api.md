...
Microsoft Graph 一种是可让你访问 Microsoft 云服务资源的 REST 风格的 Web API.在你[注册应用](auth_register_app_v2.md)并[获取身份验证令牌以用于用户](auth_v2_user.md)或[服务](auth_v2_service.md)后，可以向 Microsoft Graph API 发送请求。
...
例如，添加以下过滤参数会将返回的电子邮件限制为仅 `emailAddress` 属性为 `jon@contoso.com` 的电子邮件。
...
你可以随时开始使用和运行 Microsoft Graph。若要了解详细信息，请转到 [Graph 资源管理器](https://developer.microsoft.com/en-us/graph/graph-explorer)尝试发出某些请求、尝试[快速启动](https://developer.microsoft.com/en-us/graph/quick-start)，或使用 [SDK 和代码示例](https://developer.microsoft.com/en-us/graph/code-samples-and-sdks)之一开始使用。