...
- 本教程假定你使用的是 Microsoft Graph 资源管理器，但可以使用 Postman，也可以创建自己的客户端应用来调用 Microsoft Graph。 若要在本教程中调用 Microsoft Graph API，你需要使用具有 全局管理员角色 和相应权限的帐户。 完成以下步骤以在 Microsoft Graph 资源管理器中设置权限：
...
对于本教程，您将创建用于测试风险检测的用户帐户。 在请求正文中， `contoso.com` 更改为租户的域名。 可以在 Azure Active Directory 概述页面上找到租户信息。
...
2. 使用之前创建的 **MyTestUser1** 帐户的凭据登录到门户。 将要求您更改现有密码。

### <a name="list-risk-detections"></a>列出风险检测

使用匿名浏览器登录 Azure 门户时， `anonymizedIPAddress` 检测到风险事件。 查询参数可用于仅获取与 `$filter` **MyTestUser1** 用户帐户关联的风险检测。
...
您可以利用组织中的条件访问策略，以允许用户在检测到风险时自行修正。 利用自我修正，用户可以在完成策略提示后自行取消阻止，从而安全地访问其资源。 在此步骤中，您将创建一个条件访问策略，要求用户在出现中等或高风险检测时使用 MFA 登录。

### <a name="set-up-multi-factor-authentication"></a>设置多重身份验证

为 MFA 设置帐户时，可以从多种方法中选择用于验证用户。 选择最适合你的情况的方法来完成本教程。 

1. 使用 **MyTestUser1** [帐户](https://aka.ms/MFASetup)登录到帐户安全网站。
...
因为 MFA 已完成。 现在，当您列出风险检测时 **，riskState** 将事件显示为 `remediated` 。
...
你可以阻止用户登录，而不是让用户自行修正。 在此步骤中，您将创建一个新的条件访问策略，在出现中等或高风险检测时阻止用户登录。 策略中的区别在于 **builtInControls** 设置为 `block` 。
...
如果您认为用户没有风险，并且不希望强制实施条件访问策略，您可以手动消除存在风险的用户。
...
消除风险用户后，可以在列出风险用户时看到响应中 **MyTestUser1** 用户帐户现在具有风险级别和 `none` `dismissed` riskState。
